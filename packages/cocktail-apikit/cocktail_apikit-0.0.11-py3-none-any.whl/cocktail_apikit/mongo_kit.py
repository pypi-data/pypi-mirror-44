...
from .constants import (LIMIT_KEY, MONGO_LOOKUPS_MAPPINGS, PAGE_KEY,
                        RECORD_ACTIVE_FLAG_FIELD, REQUEST_DESC_SORT_MARK,
                        REQUEST_LOOKUP_MARK, SORT_KEY)
...
class BottleMongoQueryBuilder(object):
    ...
    def __init__(self, request: BaseRequest = None, schema: Schema = None):
        ...
        self.limit = self._get_limit(self._bottle_params.pop(
...
class MongoDBManager(object):
    ...
    def _convert_uuid_condition_value(self, condition):
        ...
        if condition.get(MONGO_ID_FIELD_NAME, None):
            condition[MONGO_ID_FIELD_NAME] = _uuid_string_to_mongo_uuid(
                condition.get(MONGO_ID_FIELD_NAME))
        return condition

    def _extract_set_values(self, update:dict=None):
        """
    ...
    def find_one_and_update(self, condition: dict=None, changed_value: dict=None, projection: dict=None,
        ...
        set_values.update( {'updated_at': datetime.now().astimezone(timezone.utc)})
        changed_value.update({'$set': set_values})
    ...
    def filter(self, query: Union[MongoQuery, Dict]=None, projection=None,  soft_delete: bool=True) -> (Cursor, int):
        ...
        results=self.find(
            condition=query, sort=sort_fields, skip=skip, limit=limit)
        return results, count

    def find(self, condition: dict=None, projection: dict=None, skip=0, limit=0, no_cursor_timeout=False, cursor_type=CursorType.NON_TAILABLE,
             sort=None, allow_partical_results=False, oplog_replay=False, modifiers=None, batch_size=0, manipulate=True, collation=None, hint=None,
             max_scan=None, max_time_ms=None, max=None, min=None, return_key=False, show_record_id=False, snapshot=False, comment=None, session=None):
        """
        complete style of pymongo's collection.find method's signature
        """

        return self.collection.find(condition, projection, skip=skip, limit=limit, no_cursor_timeout=no_cursor_timeout, cursor_type=cursor_type,
                                    sort=sort, allow_partial_results=allow_partical_results, oplog_replay=oplog_replay, modifiers=modifiers, batch_size=batch_size, manipulate=manipulate,
                                    collation=collation, hint=hint, max_scan=max_scan, max_time_ms=max_time_ms, max=max, min=min, return_key=return_key, show_record_id=show_record_id,
