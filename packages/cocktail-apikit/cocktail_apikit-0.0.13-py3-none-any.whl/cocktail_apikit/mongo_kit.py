...
class DictMongoQueryBuilder(MongoQueryBuilderMixin):
    ...
    def __init__(self, query_data: dict = None, schema: Schema = None):
        ...
        self.limit = self._get_limit(self._query_data.pop(LIMIT_KEY, DefaultSettings.API_DEFAULT_LIMIT))
        self.skip = (self.page - 1) * self.limit
        self.projections = self._get_projection(self._query_data.pop(PROJECTION_KEY, None))
        self.build_filter()
...
class MongoDBManager(object):
    ...
    def filter(self, query: Union[MongoQuery, Dict] = None, projection=None, soft_delete: bool = True) -> (Cursor, int):
        ...
        results = self.find(condition=query, projection=projection, sort=sort_fields, skip=skip, limit=limit)
