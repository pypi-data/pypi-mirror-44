    ...
    def clear(
        ...
        try:
            if start_date and end_date:
                start = f'{self.date_column_type}("{start_date.isoformat()}")'
                end = f'{self.date_column_type}("{end_date.isoformat()}")'
                query = f"""
                        DELETE FROM `{self.fqtable_name}`
                        WHERE `{self.date_column_name}` BETWEEN {start} AND {end}
                        """
            else:
            ...
            self.logger.exception(f'BQTable.clear.failed: {str(ex)}')
            raise
