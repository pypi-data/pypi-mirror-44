...
import mungo.version

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("-j", "--jobs", type=int, help="number of jobs", default=4)
    parser.add_argument('--version', action='version', version=f'mungo {mungo.version.mungoversion}')
    ...
    if command == "create":
        create_environment(name, channels, packages, pip, jobs, dag, ask_for_confirmation, offline)
    elif command == "install":
        install_packages(name, channels, packages, pip, jobs, dag, ask_for_confirmation, offline)
