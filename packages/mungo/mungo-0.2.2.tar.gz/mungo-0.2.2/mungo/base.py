...
def create_dag(channels, packages, local_repodata, njobs=4, offline=False):
    repodata_chunks = get_repository_data(channels, njobs, offline)
...
def print_dag(all_nodes, root, to_install):
    dot = Digraph()
...
def create_environment(name, channels, packages, pip, jobs=1, dag=False, ask_for_confirmation=False, offline=False):
    all_nodes, root = create_dag(channels, packages, [], jobs, offline)
...
def install_packages(name, channels, packages, pip, jobs=1, dag=False, ask_for_confirmation=False, offline=False):
    local_repodata = Repodata.from_environment(name)
    all_nodes, root = create_dag(channels, packages, [local_repodata], jobs, offline)
