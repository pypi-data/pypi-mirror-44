...
class BarakatTS2018Data(OutsideData):
    ...
    def __init__(self, source="BarakatTS2018", **kwargs):
        ...
        try:
            ...
            self.data = self._open_file(enhancer_file)
        except KeyError:
...
class InoueF2017Data(OutsideData):
    ...
    def __init__(self, source="InoueF2017"):
        super().__init__()
        self.data_source = source
        self.data = self._open_file(self.data_source, sep="\t", header=None, names=["temp", "sequence"])
        self._data_cleaner()
...
class BroadPeak(OutsideData):
    ...
    def __init__(self, source=False):
        ...
        self.data = self._open_file(self.data_source, sep="\t", header=None, usecols=use_cols,
                                    names=names)
        self._data_cleaner()
...
class MergeDataSets:
    """
    object1 and object2 must be a object with an attribute "data" which has chromosome coordinates as the index.
    This class provides methods to return data1, with chromosome coordinates merged with those of data2.
    Methods for merging can be Union or Interception.
    """
    def __init__(self, object1, object2, method):
        self.data = self._merge(object1.data, object2.data, method)

    def _merge(self, data1, data2, method):
        if "range" not in data1.columns:
            data="s"
        return data
