...
class PaginatedHttpGetRequest:
    ...
    def responses(self):
        ...
        while self.additional_data_key in out and additional_data_test(
            ...
            out = response.json()
            logging.info(
                "Paginator expects more pages: "
                + str(additional_data_test(out[self.additional_data_key]))
                + " "
                + endpoint_url
            )
            logging.info(
                "                              "
                + str(out[self.additional_data_key])
                + " "
                + endpoint_url
            )
            logging.info(
                "                              "
                + str(out[self.pagination_key])
                + " "
                + endpoint_url
            )
            logging.info(
                ">>>>>>>>>>>>>>>>>>>>>>>>>>>>> "
                + str(
                    self.additional_data_key in out
                    and additional_data_test(out[self.additional_data_key])
                )
            )
            yield out
