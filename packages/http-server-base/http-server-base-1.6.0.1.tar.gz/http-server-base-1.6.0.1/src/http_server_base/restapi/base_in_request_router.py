...
from typing import List, Union, Dict, Type, Tuple, Any, Pattern, Callable, TypeVar, Optional, Iterator
...
class BaseInRequestRouter(IInRequestRouter):
    # Dictionary: class_full_name (with module) -> method_lower -> path -> rest_method scope dict
    method_mapper: Dict[str, Dict[str, IInRequestRouter.mapper_type[Dict[str, Any]]]] = dict()
    ...
    @classmethod
    def get_class_mapper(cls, x: Union[Type[IRest_RequestHandler], str]) -> Dict[str, IInRequestRouter.mapper_type[Any]]:
        class_name = None
    ...
    (
        ...
        **kwargs,
        ...
        def rest_method__registrator(func, http_method=None):
            ...
            paths = (path, key)
            cls.method_mapper[_class_name][_method][key] = locals()
            cls.method_mapper[_class_name][_method][key]['action'] = rest_method__registrator__action
