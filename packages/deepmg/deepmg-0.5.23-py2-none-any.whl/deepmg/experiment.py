...
def para_cmd ():
    ...
    #Learning
    parser.add_option('-k','--n_folds',help='number of k folds (default:10), if k == 1, training on whole *_x.csv, and testing on *_zx.csv')
...
def write_para(options,configfile):
    ...
    Config.set('model','rf_n_estimators',options.rf_max_depth)
...
def validation_para(options):
    ...
    if not (options.model in ['svm_model','rf_model','gbc_model','knn_model','model_cnn','model_pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d']):
        print 'you entered --model=' + str(options.model)
        print utils.textcolor_display('--model') + " only supports 'svm_model','rf_model','gbc_model','model_cnn','model_pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d'"
        valid = 0
...
def name_log_final(arg, n_time_text):
    ...
    #model for 1D
        ...
        if arg.rf_max_depth in [-1,'-1']:
            ...
            prefix='rf_' +  str(arg.rf_n_estimators) + 'dp'+ str(arg.rf_max_depth)+'_'+mid 

    elif arg.model=="gbc_model": #gbc
        if arg.rf_max_depth in [-1,'-1']:
            prefix='gbc_' +  str(arg.rf_n_estimators) + '_'+mid 
        else:
            prefix='gbc_' +  str(arg.rf_n_estimators) + 'dp'+ str(arg.rf_max_depth)+'_'+mid 
