...
def imprimeDEF():
        ...
        if english:
                print('; Database for DAAD 2.40 - 2.42 generated by Trizio2sce.', file=f)
        else:        
                print('; Base de datos para DAAD V2.40 - 2.42 generada por Trizio2sce.', file=f)
        print(';', file=f)
        if english:
...
def imprimeMTX():
        ...
        print(';', file=f)
        print('; Los mensajes 0 a 13 se usan por Trizio2sce para su rutina de listado de salidas (process 10).', file=f)
        print(';', file=f)
        print('/0',file=f)
        ...
        print('salir ',file=f)
        print(';', file=f)
        print('; El mensaje 14 se usa por Trizio2sce como texto de introducción del juego.', file=f)
        print(';', file=f)
        print('/14', file=f)
        print(history, file=f)
        print(';', file=f)
        print('; El mensaje 15 se usa por Trizio2sce para imprimir los turnos en la barra de estado.', file=f)
        print(';', file=f)
        print('/15', file=f)
        print('Turnos: ', file=f)
        print(';', file=f)
        print('; Tras el mensaje 15, Trizio2sce usará un nº variable de mensajes para los nombres de localidad en la barra de estado.', file=f)
        print(';', file=f)
...
def imprimeMTX_ENG():
        ...
        print(';', file=f)
        print('; Messages 0 to 13 are used by Trizio2sce in its exits listing routine (process 10).', file=f)
        print(';', file=f)
        print('/0',file=f)
        ...
        print('out ',file=f)
        print(';', file=f)
        print('; Message 14 is used by Trizio2sce as an introduction text.', file=f)
        print(';', file=f)
        print('/14', file=f)
        print(history, file=f)
        print(';', file=f)
        print('; Message 15 is used by Trizio2sce while printing the numbre of turns at the status line.', file=f)
        print(';', file=f)
        print('/15', file=f)
        print('Turns: ', file=f)
        print(';', file=f)
        print('; After message 15, Trizio2sce will use a variable amount of messages as location names at the status line.', file=f)
        print(';', file=f)
...
def imprimePRO5(helpMessage1, helpMessage2, listRooms2):
        ...
        for y in listRooms2:
                if y['dark']==True:
                        x=True
        if x:
                print('; Entradas añadidas por Trizio2sce para tratar las localidades sin luz.', file=f) 
                print('', file=f)
        for y in listRooms2:
                for x in y['con']:
                        a=x.split()
                        ...
                        if e==True and e2==False:
                                ...
                                print('                NOTDONE', file=f)
                                print('', file=f)
        print('; Entrada añadida por Trizio2sce para pantallas de ayuda.', file=f)
        print('', file=f)
...
def imprimePRO5_ENG(helpMessage1, helpMessage2, listRooms2):
        ...
        for y in listRooms2:
                if y['dark']==True:
                        x=True
        if x:
               print('; Entries added by Trizio2sce for dealing with dark locations.', file=f) 
               print('', file=f)
        for y in listRooms2:
                for x in y['con']:
                        a=x.split()
                        ...
                        if e==True and e2==False:
                                ...
                                print('                NOTDONE', file=f)
                                print('', file=f)
        print('; Entry added by Trizio2sce to display help screens.', file=f)
        print('', file=f)
...
def imprimePRO6():
        ...
        if md:
                print('', file=f)
                if english:
                        print('; Trizio2sce optionally (-md) adds conditional code to activate', file=f)
                        print('; "Invisible drawing" mode in Amstrad CPC.', file=f)
                else:
                        print('; Trizio2sce añade opcionalmente (-md) código condicional para activas el modo', file=f)
...
def imprimeTodo():
        ...
        print(';', file=f)
        if english:
                print('; After location names messages, Trizio2sce uses 2 messages for the help screen.', file=f)
        else:
                print('; Tras los mensajes con los nombres de las localidades, Trizio2sce usa 2 mensajes para la pantalla de ayuda.', file=f)
        print(';', file=f)
        ...
        print(';', file=f)
        if english:
                print('; After help messages, Trizio2sce uses one more message as status line text in dark locations.', file=f)
        else:
                print('; Tras los mensajes de ayuda, Trizio2sce usa un mensaje más para la barra de estado en localidades sin luz.', file=f)
        print(';', file=f)
...
print('Trizio2sce version 1.0.5 190410 (c) 2018 Pedro Fernández')     
...
parser.add_argument('-md', help="Añade código condicional para activas el modo de dibujo invisible en CPC", action="store_true")
...
if out_file==None:
        ...
        if out_file.endswith('.json'):
                out_file=out_file.replace('.json','.sce')
        else:
...
if english:
        title='Test adventure generated with trizio2sce.'
        author='Anonymous'
        description='Test adventure for the conversion script trizio2sce (trizbort.io maps to DAAD SCE source code).'
        history='Adventure introduction text (edit in message 14).'
else:
        title='Aventura de prueba generada con trizio2sce.'
        author='Anónimo'
        description='Aventura de prueba del script conversor trizio2sce (mapas de trizbort.io a código fuente SCE del DAAD).'
...
with open(out_file, 'w', encoding='850', newline='\r\n') as f:
        imprimeTodo()
