...
{model_name}.__bento_module_path = _module_path
...
class BentoModel(BentoService):
    ...
    env = BentoServiceEnv()  # Default to empty env

    artifacts = []  # Default to 0 artifacts
    ...
    def artifacts(self):
        try:
            ...
            self.__artifacts = ArtifactCollection()
            for artifact in self.__class__.artifacts:
                self.__artifacts[artifact.name] = artifact.create()
            return self.__artifacts
    ...
    def env(self):
        try:
            ...
            if isinstance(self.__class__.env, dict):
                self.__env = BentoServiceEnv.fromDict(dict)
            else:
                self.__env = self.__class__.env

    @property
    def name(self):
        if self.__class__.__bento_model_name is not None:
            # TODO: verify self.__class__._model_name format, can't have space in it
            #  and can be valid folder name
            return self.__class__.__bento_model_name
        else:
    ...
    def version(self):
        try:
            ...
            raise BentoMLException("Only BentoModel loaded from archive has version attribute")

    def save(self, base_path, version=None, env=None, project_base_dir=None,
        ...
        with open(os.path.join(path, self.name, '__init__.py'), "w") as f:
            f.write(
                INIT_PY_TEMPLATE.format(model_name=self.name, module_name=module_name,
                                        model_version=self.__class__.__python_package_version))

        # write setup.py, make exported model pip installable
        setup_py_content = BENTO_MODEL_SETUP_PY_TEMPLATE.format(
            name=self.name, version=self.__class__.__python_package_version,
    ...
    def load(self, path=None):
        ...
        if self.__class__.__bento_module_path is not None:
            ...
            path = self.__class__.__bento_module_path
            artifacts_path = path
        else:
            if path is None:
