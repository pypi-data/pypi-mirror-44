...
#
def import_jsons(api_instance, dataset, json_iter, processes=4):
    ...
    pool_output = pool.imap_unordered(import_json, json_iter)
    pool.close()
    pool.join()
...
#
def import_files(api_instance, dataset, file_iter, processes=4):
    # partial() function not work well with pool, use global variables as a workaround
    global global_api_instance
    global global_dataset
    ...
    # import_json_partial = partial(import_json123, api_instance, dataset)
    pool = Pool(processes=processes)
    pool_output = pool.imap_unordered(import_file, file_iter)
    pool.close()
    pool.join()
...
#
def import_urls(api_instance, dataset, url_iter, processes=4):
    # partial() function not work well with pool, use global variables as a workaround
    global global_api_instance
    global global_dataset
    ...
    # import_json_partial = partial(import_json123, api_instance, dataset)
    pool = Pool(processes=processes)
    pool_output = pool.imap_unordered(import_url, url_iter)
    pool.close()
    pool.join()
