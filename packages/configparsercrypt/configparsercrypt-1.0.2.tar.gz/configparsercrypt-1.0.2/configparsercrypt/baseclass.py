...
__doc__ = '''SecureConfig base class for simplifying load of encrypted config files (default: serialized dict).
    ...
    * Supply readonly=True to protect configuration from .set and .write
    ...
    For .ini-style files, use ConfigParserCrypt.
'''
...
class SecureConfig(cryptkeeper_access_methods):
    ...
    exclusively in plaintext and throws ConfigParserCryptException if it cannot be parsed.
    ...
    `readonly` param ensures that .set() and .write() cannot be used. 
        ...
        :param ck:         CryptKeeper object (see configparsercrypt.cryptkeeper)
    ...
    def __init__(self, filepath='', rawtxt='', readonly=False, **kwargs):
        ...
        if filepath and rawtxt:
            raise ConfigParserCryptException('Supply either filepath or rawtxt (not both).')
        ...
        if self.ck and rawtxt:
            ...
            try:
                ...
                raise ConfigParserCryptException('bad data or missing encryption key')                  
        else:
    ...
    def add_section(self, section):
        ...
        if self.cfg.get(section, None):
            raise ConfigParserCryptException('specified section already exists')
        else:
            self.cfg[section] = {}            
