...
class Agility_Docker(object):
    def __init__(self, debug=False):
        if not debug:
            ...
            if cmd.wait() == 1:
                DockerInitRunError('__main__.主机没有docker.server')
...
if __name__ == '__main__':
    """
    for i in cmd_ps_a().keys():
        print(i, cmd_ps_a()[i])
    cmd_pull_images('daocloud.io/centos')
    cmd_run_vessel(images_name='daocloud.io/centos', name='test', cpu_shares=512, cpuset_cpus=2, blkio_weight=65535)

    a = docker.docker_history(all=True)
    for i in a:
        print(i, a[i])
    """

    docker = Agility_Docker(debug=False)
    docker.dockerfile(RUN=['apt update', 'apt install python3', 'pip3 install tornado'],
                      CMD=['apt install httpd', 'yum install tree'],
                      COPY=[[['EPC_test', '/'], ['README', '/home']], [3, 5]],
                      ADD=[[['bunch.tar.gz', '.'], ['nginx-1.14.2.tar', '.']], [6, 7]],
                      ENTRYPOINT=[['/bin/echo', 'Hello World'], [2, 3]],
                      ENV=[
        ['WELCOME "You are in my container, welcome!"',
         'name Cloud Man ENTRYPOINT echo "Hello, $name"'],
        [2, 3]
    ],
        EXPOSE=80, VOLUME=["/data1", "/data2"])
    pass
