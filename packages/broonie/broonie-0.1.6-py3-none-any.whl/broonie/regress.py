...
class regressDataClass(object):
    ...
    def __init__(self, kNN, inrawfilename, indexfilename, distancefilename, outputRfilename, outputSfilename, batchfilename=False, origScaling=True, logScaledResults=False):
        self.kNN = kNN
    ...
    def runProcess(self):
        ...
        t0=tm.time()
        RinData = pd.read_csv(self.rawfilename,delim_whitespace=True)
        NNindex = pd.read_csv(self.xtildeNeighborIndicesFilename,delim_whitespace=True,index_col=0)
        NNdistance = pd.read_csv(self.xtildeNeighborDistancesFilename,delim_whitespace=True,index_col=0)
        print(tm.time()-t0)
        ...
        print('min value oif regressed R is '+str(df_new_R.values.min()))
        ...
        df_new_S = self.scaleResults(df_new_R)
