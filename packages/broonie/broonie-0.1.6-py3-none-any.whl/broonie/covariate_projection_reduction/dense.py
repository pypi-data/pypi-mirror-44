...
# Options for computing the betas required for Z projection
    ...
    def fetchBeta(self, X, Z, iflag=0):
        ...
        if (iflag !=0 and iflag!=1):
            print('Wrong iflag choice: changing to iflag=1 natural eqn solvers')
            iflag=1
        if (iflag==0 and Z.shape[1]==1):
...
## Can choose to zero out betas (temp) here
        ...
        self.d = d
        lmX = np.matmul(U,d) 
        temp = self.fetchBeta(lmX,Z,iflag=1)
        self.beta = temp
        newz = np.concatenate((np.ones((Z.shape[0],1)),Z),axis=1)
    ...
    def computeUandS_ZscoredEigenvaluesReturnReconstructed(self,betaZeroList):
        ...
        np.fill_diagonal(d, s)
        lmX = np.matmul(U,d)
        temp = self.fetchBeta(lmX,Z,iflag=1) # Might want to regularized
        tempZeros = temp.copy()
        zerorow = [0.0]*k
