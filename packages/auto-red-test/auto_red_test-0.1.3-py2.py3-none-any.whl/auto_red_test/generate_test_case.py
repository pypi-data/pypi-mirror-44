...
def __get_service_info_from_thrift(root_path, idl_service, need_test_methods):
    ...
    idl_method_request = dict([
        (name[:-5], obj) for name, obj in idl_service
...
def __get_request_info_from_thrift(idl_request, method_request):
    ...
    request_obj_name__obj = dict([
        (name, obj)for name, obj in idl_request if inspect.isclass(obj)])
...
def __write_pytest_request(root_path, test_request_name, request_name_obj):
    ...
    with open(os.path.join(
        ...
        pytest_requests.append('from ' + test_request_name + ' import *\n\n\n')
        for name, obj in request_name_obj.items():
...
def generate_red_test(root_path, test_handler, test_service, test_request, test_request_name):
    """自动生成Red Test"""
    ...
    __write_pytest_request(root_path, test_request_name, request_name_obj)
    __write_test_methods(root_path, method_request, existed_handler_methods)


# if __name__ == '__main__':
#     测试案例
#     StockageHandler = inspect.getmembers(StockageHandler)
#     whole_service = inspect.getmembers(whole_service)
#     all_request_name = all_request.__name__
#     all_request = inspect.getmembers(all_request)
#     generate_red_test(ENV['root'], StockageHandler, whole_service, all_request, all_request_name)
