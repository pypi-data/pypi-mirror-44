...
class Client:
    ...
    def _determine_url_for_version(self):
        ...
        if cloudcix.conf.settings.CLOUDCIX_API_VERSION == 2:
            ...
            # Check if the chosen application is potentially running in py2 and py3 simultaneously
            if self.application.lower() in FULLY_DEPLOYED:
                return url

            # If it's not fully deployed, check which version we need to use
    ...
    def request(self, method, token=None, pk=None, data=None, params=None, files=None, **kwargs):
        ...
        if span is not None:
            ...
            tracer = getattr(cloudcix.conf.settings, 'TRACER', None)
            if tracer is None:
                warnings.warn('A span was passed to the client but `settings.TRACER` is not set')
            else:
