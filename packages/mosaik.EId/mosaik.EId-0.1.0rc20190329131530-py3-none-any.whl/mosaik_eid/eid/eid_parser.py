...
from mosaik_eid.eid.eid_registry import EidRegistry
from mosaik_eid.eid.entity_exception import EntityIndexException, EntityException, EntityTypeException
...
class EidParser(object):
    ...
    def parse_entity_type(cls, e_type):
        ...
        """
        try:
            e_type = EidRegistry.types[e_type.name]
        except AttributeError:
            try:
                e_type = EidRegistry.types[e_type]
            except KeyError:
                raise EntityTypeException('No such entity type was registered:', e_type)
