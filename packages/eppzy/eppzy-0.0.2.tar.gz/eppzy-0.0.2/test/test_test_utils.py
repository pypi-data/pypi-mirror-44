...
from test_contact import MockTransport, data_file_contents
...
def test_overlayed():
    ...
    mrwt = MockTransport(rw_checks)

    def ro_checks(body):
        assert b'info' in body
        return data_file_contents('rfc5733/contact_info_example.xml')
    mrot = MockTransport(ro_checks)
    rws = {'Contact': RequestWrapper(Contact(mrwt))}
    ros = {'Contact': RequestWrapper(Contact(mrot))}
    o = overlayed(rws, ros)
