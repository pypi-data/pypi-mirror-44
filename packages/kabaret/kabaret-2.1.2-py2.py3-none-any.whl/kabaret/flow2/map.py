...
from .relations import OrderedStringSetParam, HashParam, Parent, Child, Computed
...
class DynamicMap(Object):
    ...
    def rows(self):
        ...
        The _style value in the row must be a dict with those optional keys:
            ...
            - <col_name>_icon: the icon in column <col_name>
...
#---OLD
        ...
        if not self.has_mapped_name(name):
            ...
            raise ValueError('No %r mapped item declared under %s' %
                             (name, self.oid()))
            # BN: Shouldn't this raise an Exception from flow2.exceptions instead of ValueError ???
