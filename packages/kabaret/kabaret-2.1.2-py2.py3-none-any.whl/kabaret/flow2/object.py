...
#---OLD
    ...
    def destroy(self):
        print('----> DESTROYING', self.oid(), '. Should we notify refs ?')
        self.destroyed = True
        self.parent = None
...
#---OLD
    ...
    def remove_ref(self, object):
        ...
        try:
            ...
            print('WARNING: Removing ref %r from %r: ref not in list!')
            return
        refs = sorted(refs)
        self.value_store.set(self.oid() + '.refs', refs)
...
class _Relation(object):
    ...
    def is_hidden(self):
        return self._ui.get('hidden', False)
...
# (anything with a touch() mechanism)
    def summary(self):
        return None

    def pprint(self):
        print(self._mng.pformat())
