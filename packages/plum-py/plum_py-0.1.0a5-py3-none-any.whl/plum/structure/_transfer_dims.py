...
def __create__(cls, iterable=None):
...
def __touchup__(cls, parent):
    array_name = cls._member.name
    if parent[array_name] is None:
        dims = parent[cls._member.dimsmember]
        try:
            dims = [int(dims)]
        except TypeError:
            dims = list(dims)

        parent[array_name] = cls._member.cls._make_instance(None, dims)
