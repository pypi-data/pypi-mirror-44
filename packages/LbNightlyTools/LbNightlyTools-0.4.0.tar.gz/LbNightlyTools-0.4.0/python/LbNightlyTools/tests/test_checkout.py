...
def test_Project():
    ...
    assert p.baseDir == 'LBSCRIPTS/LBSCRIPTS_HEAD'
...
def test_checkout():
    ...
    with TemporaryDir(chdir=True):
        ...
        assert_git_branch(join('GAUDI', 'GAUDI_v23r6'), None)
...
def test_checkout_git_merge():
    ...
    with TemporaryDir(chdir=True):
        shutil.rmtree(join('GAUDI', 'GAUDI_mr150'), ignore_errors=True)
        Project('Gaudi', 'mr150', checkout_opts={'commit': 'd7ecf7ef4~',
                                                 'merges': 150}).checkout()
        check([join('GAUDI', 'GAUDI_mr150', join(*x))
               for x in [('Makefile',),
                         ('CMakeLists.txt',),
                         ('cmt', 'project.cmt'),
                         ('GaudiRelease', 'cmt', 'requirements')]])
        assert_git_branch(join('GAUDI', 'GAUDI_mr150'), None)
        parents = check_output(['git', 'log', '-n', '1', '--format=%P'],
                               cwd=join('GAUDI', 'GAUDI_mr150')).split()
    ...
    with TemporaryDir(chdir=True):
        ...
        check([join('BRUNEL', 'BRUNEL_v44r1', join(*x))
               for x in [('Makefile',),
                         ...
                         ('cmt', 'project.cmt'),
                         ('BrunelSys', 'cmt', 'requirements')]])
        assert not exists(join('BRUNEL', 'BRUNEL_v44r1', '.git'))
