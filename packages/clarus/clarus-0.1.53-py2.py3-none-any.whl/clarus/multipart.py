...
class PartWrapper(object):
    def __init__(self, part, response):
        ...
        stats = PartWrapper.merge_stats(part, response)
    ...
    @staticmethod
    def merge_stats(part, response):
        if api.STATS in part.headers:
            part_stats = PartWrapper.parse_stats_header(part.headers.get(api.STATS))
        else:
            part_stats = {}
            
        if api.STATS in response.headers:
            response_stats = PartWrapper.parse_stats_header(response.headers.get(api.STATS))
        else:
            response_stats = {}
