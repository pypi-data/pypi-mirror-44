...
class ArithmeticPath(object):
    ...
    def read_from(self, root_namespace):
        self.actions.set_namespace(root_namespace)
        try:
            result = parse(self.expression, actions=self.actions)
    ...
    def write_to(self, root_namespace, value):
        ...
        try:
            ...
            parse(self.expression, actions=self.actions)
        except ParseError as e:
            raise RookInvalidArithmeticPath(self.expression, e)

        # execute the operation chain.
        size = len(self.actions.operations)
        if size == 0:
            raise RookInvalidArithmeticPath(self.expression)
        ...
        for _ in range(i, size - 1):
            namespace = self.actions.operations[i].read(namespace, True)
            i += 1

        self.actions.operations[i].write(namespace, value)
