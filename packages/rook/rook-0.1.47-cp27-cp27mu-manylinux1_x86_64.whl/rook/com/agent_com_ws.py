...
from tornado.concurrent import Future, future_set_result_unless_cancelled, future_set_exc_info
...
class AgentCom(object):
    def __init__(self, host, port, token, labels):
    ...
    def _connect(self):
        ...
        while not self._cancel.cancelled():
            try:
                try:
                    if connected and time.time() >= last_successful_connection + 3*60.0:
                        retry = 0
                        backoff = AgentComConfiguration.BACK_OFF
                ...
                logger.info('Connection failed; reason = %s, retry = #%d, waiting %.3fs', e.message or e, retry, backoff)
    ...
    def _create_connection(self):
        ...
        return websocket_connect(request, ping_interval=10, ping_timeout=30)
