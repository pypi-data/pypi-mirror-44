...
def grid(ctx, command, args, params, provider, machine_type, pip_packages, requirements_file, apt_packages,
         ...
         conda_file, force, verbose, local_caching, idempotent, github_url, github_ref, **kwargs):
    """
...
@hyper.command(name="bayesian",
               short_help="Execute a hyperparameter bayesian search", hidden=True)
@click.argument("command")
@click.argument("args", nargs=-1)
@click.option("-n", "--num-runs", "num_runs", required=True, type=int,
...
@click.option("-p", "--param", "params", multiple=True,
              metavar='NAME=VALUE[,VALUE,VALUE,...] | NAME=MIN:MAX[:linear[:int|float]]',
              help="Specify a hyperparameter for the run in the form: a range (MIN:MAX) "
                   "An optional type ('int' or 'float'), only linear scaling currently supported for bayesian searches "
