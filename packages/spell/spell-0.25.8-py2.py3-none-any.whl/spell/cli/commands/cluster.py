...
def model_server_create(ctx, aws_profile, cluster_id,
                        ...
                        nodes_min, nodes_max, node_volume_size, auth_api_url):
    """
    ...
    try:
        ...
        ca_yaml_path = os.path.join(tmp_dir, "spell-ca.yaml")
        with open(ca_yaml_path, "w") as f:
            f.write(ca_yaml)
        subprocess.check_call(("kubectl", "apply", "--filename", ca_yaml_path))
        os.remove(ca_yaml_path)
    ...
    try:
        ...
        with open(ambassador_path, "w") as f:
            f.write(generate_eks_cluster_ambassador_yaml(auth_api_url))
    ...
    try:
        ...
        secrets_yaml_path = os.path.join(tmp_dir, "spell-secrets.yaml")
        with open(secrets_yaml_path, "w") as f:
            f.write(secret_yaml)
        subprocess.check_call(("kubectl", "apply", "--filename", secrets_yaml_path))
        os.remove(secrets_yaml_path)
