...
def get_contours(sample_points, samples, level):
    # we assume that sample_points gives us directional coordinates on the sphere [(theta, phi), ...]
    ...
    connected_line_segments = connect_line_segments(simplex_lines)

    lines_in_contours = np.unique([li for index_contour in connected_line_segments for li in index_contour])

    #for li in xrange(len(lines)):
    for li in lines_in_contours:
	p0, p1 = lines[li]
	(p0, y0), (p1, y1) = sorted([(p0, samples[p0]), (p1, samples[p1])], key=lambda x: x[1])
	x = (level-y0) / (y1-y0)

        v0, v1 = np.array(sample_points[p0]), np.array(sample_points[p1])
    ...
    return [np.array([line_ps[li] for li in index_contour]) for index_contour in connected_line_segments]
