...
class ConfModifyView(ScrollFrame) :
    def __init__(self, *args, **dArgs) :
        ScrollFrame.__init__(self, *args, **dArgs) 

        self.supportTypes = ('int', 'float', 'bool', 'string', 'version')
    ...
    def editCell(self, column, row) :
        ...
        values = self.__getTVByRow(row)
        edit = ConfEditView(self, callback=self.__onEditEnd, width=240, height=250)
        edit.setData(
...
class ConfEditView(tk.Toplevel):
    ...
    def __init__(self, *args, **dArgs):
        ...
        combobox = Combobox(ft, callbcak=self.changeType, values=('int', 'float', 'bool', 'string', 'version', 'dict', 'list'))
