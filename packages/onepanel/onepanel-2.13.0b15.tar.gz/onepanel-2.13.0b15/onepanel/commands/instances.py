...
@click.option(
    '-c', '--cpu',
    type=float,
    help='Threshold for CPU. 0.40 = 40%. Anything below or equal to this is considered an inactive CPU'
)
@click.option(
    '-g', '--gpu',
    type=float,
    help='Threshold for GPU. 0.40 = 40%. Anything below or equal to this is considered an inactive GPU'
)
@click.option(
    '-d', '--duration',
    type=int,
...
@click.pass_context
def monitor_activity(ctx, identifier, cpu=None, gpu=None, duration=None, time_repeat=None, verbose=None):
    vc = ctx.obj['vc']
    ...
    machine_activity = ThresholdMachineActivity(cpu, gpu, dead_duration=timedelta(seconds=duration))
