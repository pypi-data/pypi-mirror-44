...
class DatasetDownloader(Thread):
    ...
    def start_downloading_dataset(self, connection, path, account_uid, dataset_uid, dataset_version):
        # Authenticate for S3
        # todo update authentication to be more generic, based on cloud provider
        cloud_provider = os.getenv('CLOUD_PROVIDER', 'GCP')
        if cloud_provider == 'GCP':
            gcsauth = GCSAuthenticator(connection)
            creds = gcsauth.get_gcs_credentials(account_uid, 'datasets', dataset_uid)
            util = GCPUtility()
        ...
        return util.download_all_background(path, s3_from_dir)
