...
def set_site_wgd_cert(name, cert):
    """Set a sites wgd certificate

    :param name: site name
    :param cert: TLS cetificate file to set
    """

    with open(cert) as fd:
        data = fd.read()
        return do_post('sites/'+name+'/wgdcert', {
            'cert': data
        })
    return None
...
def materialize_detach(pid, eid, rid, mid, key, addr):
