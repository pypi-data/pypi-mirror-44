...
class Jira2Peach:
    ...
    def _jiraJsonRequestGet(self, opts, url):
        ...
        try:
            ...
            print("Error calling Jira API: %s" % url)
            print("  Status Code: %s" % e.response.status_code)
            print("  Response text: %s" % e.response.text)
    ...
    def _jiraCreateIssue(self, opts):
        ...
        try:
            ...
            print("Error calling Jira API while creating a new issue.")
            print("  Status Code: %s" % e.response.status_code)
            print("  Response text: %s" % e.response.text)
    ...
    def _jiraAttachFileToIssue(self, opts, issue, name, data):
        ...
        try:
            ...
            print("Error calling Jira API while attaching file to issue")
            print("  Status Code: %s" % e.response.status_code)
            print("  Response text: %s" % e.response.text)
    ...
    def _jiraAddComment(self, opts, issue, comment):
        ...
        try:
            ...
            print("Error calling Jira API while adding comment")
            print("  Status Code: %s" % e.response.status_code)
            print("  Response text: %s" % e.response.text)
    ...
    def _newIssue(self, opts):
        ...
        if len(opts['issue']['fields']['summary']) > 254:
            opts['issue']['fields']['summary'] = opts['issue']['fields']['summary'][:254]
    ...
    def _as_str(self, data):
        ...
        header_buff = buf[0:boundary]

        try:
            headers = header_buff.decode('utf-8')

        except UnicodeDecodeError:
            
            for i in range(len(header_buff)):

                try:
                    chr_ord = ord(header_buff[i])
                except:
                    chr_ord = header_buff[i]

                if chr_ord == ord('\n') or chr_ord == ord('\r'):
                    continue
                elif chr_ord > 126 or chr_ord < 32:
                    header_buff = self._replace_bytes_index(header_buff, i, b'?')
    ...
    def _replace_bytes_index(self, data, index, replacement):
        return b'%s%s%s'%(data[:index],replacement,data[index+1:])
