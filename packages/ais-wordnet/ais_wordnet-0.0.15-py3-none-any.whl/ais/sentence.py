...
class Sentence:
    ...
    def get_similar_sentences_as_words(self,wordnet):
        ...
        for each in tokenized_sentence:
            if each[1] in self.tag_list:
                multiple_choice.append(wordnet.get_ez_synsets_as_row(each[0],each[1].lower()))  # nếu từ loại của từ đầu vào không nằm trong wordnet thì mặc định là danh từ
            else:
    ...
    def get_similar_sentences_with_score(self,wordnet):
        sentences = self.get_similar_sentences_as_list(wordnet)
        #doc2vec model
        return sentences
