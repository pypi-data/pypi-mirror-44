...
class Client:
  ...
  def delete_node(self, uuid):
    # Delete node
    # Throw exception if powered on
    try:
      # Get node info
      node = self.req.get('/servers/{}/info/full'.format(uuid))
    except Exception as e:
      raise Exception('Node does not exist: {}'.format(uuid))

    if node['status'] == 'active':
      raise Exception('Node is powered on, cannot delete')
    print(node['status'])
