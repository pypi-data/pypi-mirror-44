...
class Consumer:
    ...
    def register_regular_call(self, frequency, callback):
        regular_call = {'frequency': frequency, 'callback': callback}
        self._schedule_regular_call(regular_call)

    def _schedule_regular_call(self, regular_call):
        next_call_time = time.monotonic() + regular_call['frequency']
        heapq.heappush(self._regular_calls, (next_call_time, regular_call))

    def _maybe_run_regular_call(self):
        if not self._regular_calls:
            return

        if self._regular_calls[0][0] > time.monotonic():
            return

        (regular_call_due, regular_call) = heapq.heappop(self._regular_calls)

        log.debug("Calling regular call")
        call_start_time = time.monotonic()
        regular_call['callback']()
        call_time = time.monotonic() - call_start_time
        log.debug("Regular call done in {:f}s".format(call_time))
    ...
    def _calculate_sleep_time(self):
        ...
        if self._regular_calls:
            next_event_due_at = min(next_event_due_at, self._regular_calls[0][0])
    ...
    def run(self):
        ...
        while True:
            ...
            # with a higher priority than keeping up with regular calls
            if replication_message:
                continue

            self._maybe_run_regular_call()
