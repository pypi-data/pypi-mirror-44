...
*** Keywords ***
Click Element Confirmed By New Element
    [Tags]
    ...
    # récupère le WebElement 'sonde' pour pouvoir le comparer ensuite
    ${webelm_probe_st}  ${webelm_probe} =  Run Keyword And Ignore Error  Get WebElement  ${elm_probe}
    ...
    \
    \  # attente de l'apparition du second élément pendant 5 secondes max
    \  ${probe_visible}=  Run Keyword And Return Status
    \  ...  Wait Until Keyword Succeeds  5  ${RETRY_INTERVAL}  Element Should Be Visible  ${elm_probe}
    \
    \  # récupère (à nouveau) le WebElement 'sonde'
    \  ${webelm_probe_status}  ${webelm_probe_new} =  Run Keyword And Ignore Error  Get WebElement  ${elm_probe}
    \
    \  # compare les deux éléments sonde
    \  ${probe_diff}=  Run Keyword And Return Status  Should Not Be Equal  ${webelm_probe}  ${webelm_probe_new}
    \  
    \  # si l'élément sonde est devenu visible ou les WebElements sont différents, on sort de la boucle
    \  Run Keyword If  ${probe_visible} or ${probe_diff}  Return From Keyword
    Run Keyword If  ${INDEX} == 3  Fail  Le clic sur '${elm_clicked}' a échoué

Click Element Confirmed By No More Element
    [Tags]
    ...
    # récupère le WebElement 'sonde' pour pouvoir le comparer ensuite
    ${webelm_probe} =  Get WebElement  ${elm_gone}
    ...
    \  Wait Until Keyword Succeeds     3     ${RETRY_INTERVAL}    Click Element  ${elm_clicked}
    \
    \  # attente de la disparition du second élément pendant 5 secondes max
    \  ${probe_not_visible}=  Run Keyword And Return Status
    \  ...  Wait Until Keyword Succeeds  5  ${RETRY_INTERVAL}  Element Should Not Be Visible  ${elm_gone}
    \
    \  # récupère (à nouveau) le WebElement 'sonde'
    \  ${webelm_probe_status}  ${webelm_probe_new} =  Run Keyword And Ignore Error  Get WebElement  ${elm_gone}
    \
    \  # compare les deux éléments sonde
    \  ${probe_diff}=  Run Keyword And Return Status  Should Not Be Equal  ${webelm_probe}  ${webelm_probe_new}
    \  
    \  # si on a détecté la disparition de l'élément sonde ou les WebElement sont différent, on sort de la boucle
    \  Run Keyword If  ${probe_not_visible} or ${probe_diff}  Return From Keyword
    Run Keyword If  ${INDEX} == 3  Fail  Le clic sur '${elm_clicked}' a échoué

Click Element Confirmed By New Window
    [Tags]
...
Click Element Confirmed By Message
    [Tags]
    ...
    \  Wait Until Keyword Succeeds     3     ${RETRY_INTERVAL}    Click Element  ${elm_clicked}
    \
    \  # attente de l'apparition du message pendant 5 secondes max
    \  ${msg_found}=  Run Keyword And Return Status
    \  ...  Run Keyword If  len("${elm_message}") > 0
    \  ...     Wait Until Keyword Succeeds  5  ${RETRY_INTERVAL}  One Of Messages Should Be  ${message}
    \  ...  ELSE
    \  ...     Wait Until Keyword Succeeds  5  ${RETRY_INTERVAL}  Element Should Contain  ${elm_message}  ${message}
    \
...
Click On Add Button
    [Tags]
    Click Element Confirmed By No More Element    css=span.add-16
...
Click On Submit Button
    [Tags]
    Click Element Confirmed By No More Element  css=#formulaire div.formControls input[type="submit"]
    La page ne doit pas contenir d'erreur

Click On Submit Button Confirmed By Message
    [Tags]
    [Documentation]  Clic sur le bouton de confirmation jusqu'à ce qu'un message
    ...              apparaisse
    [Arguments]  ${message}
    Click Element Confirmed By Message  css=#formulaire div.formControls input[type="submit"]  ${message}

Click On Submit Button In Subform
    [Tags]
    Click Element Confirmed By No More Element  css=#sformulaire div.formControls input[type="submit"]
    La page ne doit pas contenir d'erreur

Click On Submit Button In Subform Confirmed By Message
    [Tags]
    [Documentation]  Clic sur le bouton de confirmation jusqu'à ce qu'un message
    ...              apparaisse
    [Arguments]  ${message}
    Click Element Confirmed By Message  css=#sformulaire div.formControls input[type="submit"]  ${message}

Click On Submit Button In Overlay
    [Tags]
    [Arguments]    ${obj}
    Click Element Confirmed By No More Element  css=#form-${obj}-overlay div.formControls input[type="submit"]
    La page ne doit pas contenir d'erreur
...
Click On Form Tab
    [Tags]
    Click Element Confirmed By No More Element  main
    La page ne doit pas contenir d'erreur
...
Click On Httpclick Element
    ...
    Click Element Confirmed By No More Element  css=.field-type-httpclick #${element_id}
    La page ne doit pas contenir d'erreur
...
Click On Tab
    ...
    Click Element Confirmed By New Element  ${locator}  ${tab_container_id}
    La page ne doit pas contenir d'erreur
...
Add File
    ...
    Click Element Confirmed By New Element  css=#${field}_upload + a.upload > span.ui-icon  css=.ui-dialog #upload-container
    Wait Until Keyword Succeeds  ${TIMEOUT}  ${RETRY_INTERVAL}  Choose File  css=#upload-form > input.champFormulaire    ${PATH_BIN_FILES}${file}
    Click Element Confirmed By No More Element  css=form#upload-form input.ui-button
...
Add File and Expect Error Message Be
    ...
    Click Element Confirmed By New Element  css=#${field}_upload + a.upload > span.ui-icon  css=.ui-dialog #upload-container
    Wait Until Keyword Succeeds  ${TIMEOUT}  ${RETRY_INTERVAL}  Choose File  css=#upload-form > input.champFormulaire    ${PATH_BIN_FILES}${file}
    Click Element Confirmed By Message  css=form#upload-form input.ui-button  ${message}  css=#upload-container div.message.ui-state-error p span.text
    ...
    Click Element Confirmed By No More Element  css=form#upload-form a.linkjsclosewindow
...
Click On Portlet Action
    ...
    Run Keyword If  '${mode}' == 'new_window'  Click Element Confirmed By New Window       ${selector}
    ...    ELSE IF  '${mode}' == 'modale'      Click Element Confirmed By New Element      ${selector}  css=.ui-widget-overlay
    ...    ELSE IF  '${mode}' == 'message'     Click Element Confirmed By Message          ${selector}  ${message}
    ...    ELSE                                Click Element Confirmed By No More Element  ${selector}
    La page ne doit pas contenir d'erreur
...
Open Fieldset
    ...
    Click Element Confirmed By New Element  css=#fieldset-form-${obj}-${fieldset} > legend.collapsible  css=#fieldset-form-${obj}-${fieldset} > .fieldsetContent
...
Open Fieldset In Subform
    ...
    [Documentation]    Déplie un fieldset et attend qu'il soit ouvert
    [Arguments]    ${obj}    ${fieldset}
    Click Element Confirmed By New Element  css=#fieldset-sousform-${obj}-${fieldset} > legend.collapsible  css=#fieldset-sousform-${obj}-${fieldset} > .fieldsetContent
