...
class TheLogger(IndentedLoggerAdapter,Folder):
	...
	def with_logging(self,log_level=logging.DEBUG,atomic_print=False,show_inputs=False,*log_names,**log_name_kwargs):
		...
		def second_wrapper(func):
			...
			def wrapper(*args,**kwargs):
				...
				try:
					if log_name_kwargs:
						logger.create().get_filepath(*log_names,**log_name_kwargs).addHandler()
...
def with_logging(log_level=logging.INFO,atomic_print=False,show_inputs=False,*log_names,**log_name_kwargs):
	...
	return logger.with_logging(log_level,atomic_print=atomic_print,show_inputs=show_inputs,*log_names,**log_name_kwargs)
