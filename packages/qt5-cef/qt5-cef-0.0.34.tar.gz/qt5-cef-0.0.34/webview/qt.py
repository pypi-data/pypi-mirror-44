...
class LoadHandler(object):
    ...
    def OnLoadStart(self, browser, frame):
        if frame.IsMain():
            ...
            # set_javascript_bindings(self.uid)
...
class BrowserView(QMainWindow):
    ...
    def reload_url_by_cid(self, cid, url):
        uid = self.get_uid_by_cid(cid)
        if uid in BrowserView.instances.keys():
            BrowserView.instances[uid].view.ExecuteFunction('window.python_cef.loadUrl', url)
...
def listen_process_by_hwnd(hwnd, cid, client_window, pid):
    if hwnd == 0:
        return
    else:
        if platform.system() == 'Windows':
...
def launch_third_party_client(application_title,
    ...
    if platform.system() == 'Windows':
        ...
        t = Thread(target=listen_process_by_hwnd, args=(child_process_hwnd, cid, client_window, child_process.pid))
        t.start()
...
def nest_third_party_application(target_uid='master',
    ...
    t = Thread(target=launch_third_party_client,
               args=(third_party_application_title, application_path, launch_params, target_window,
                     third_party_wrapper_window, cid))
    t.start()
    t.join()
