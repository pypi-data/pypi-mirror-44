...
class Swagger(object):
    def __init__(self, host, basePath='/api'):
    ...
    def addDefinition(self, name, sgProp):
        ...
        assert name not in self.definitions, name
...
class SwaggerPropertie(object):
    ...
    def setPropertieItem(self, item):
...
def _getGoStruct(lines):
    ...
    for line in lines:
        ...
        i = (tag, isArray, typ, typObj, comment)
        si[cs].append(i)
    return si
...
def genSwaggerYaml(savePath, url, title, description, version):
    ...
    for item in ds.iteritems():
        ...
        for prop in props:
            param, requried, typ, refObj, comment = prop
            if requried:
    ...
    for group in apiModel.groups:
        ...
        for api in group.apis:
            ...
            sg.addPath('/api/' + api.group + '/' + api.apiRouterName(), path)
