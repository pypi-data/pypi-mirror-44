...
$Id: utils.py 31294 2008-11-18 16:19:27Z dbinger $
...
def write(f, s):
    """write as bytes the value of ``s`` into f (file-like object)"""
    assert f is not None, 'invalid socket: %s' % f
    s_bytes = as_bytes(s)
    if TRACE:
    ...
    if f_write is not None:
        ...
        if TRACE:
            sys.stdout.write('s_bytes len: %s\n' % len(s_bytes))
        failures = 0
        n = failures
        while s_bytes:
            try:
