...
class ImageSlice(Prop, Dataset):
    ...
    def __setstate__(self, s):
        ...
        self._Dataset.DeepCopy(io.loads(s['_Dumps']['_Dataset']))
        self._LookupTable.DeepCopy(io.loads(s['_Dumps']['_LookupTable']))
        del s['_Dumps']

        for kw in s['_VTKGets']:
            io.vtkset(getattr(self, kw), s['_VTKGets'][kw])
        del s['_VTKGets']
    ...
    def __getstate__(self):
        ...
        s['_Dumps'] = {'_Dataset': io.dumps(self._Dataset),
                       '_LookupTable': io.dumps(self._LookupTable)}
        for kw in s['_Dumps']:
            del s[kw]

        s['_VTKGets'] = {name: io.vtkget(getattr(self, name)) for name in
