...
import vio
from vidget import Mouse, Widget
...
class ImageSlice(_Prop, _Dataset):
    """场景表示，图像数据的断层表示
    vtk.vtkImageData -> vtk.vtkImageSlice"""

    def __init__(self, vi: Widget):
        _Prop.__init__(self, vi)
...
class ImageVolume(_Prop, _Dataset):
    """场景表示，图像数据的立体表示
...
class PolyActor(_Prop, _Dataset):
    """场景表示，聚合数据的表示
