...
class Reception(threading.Thread):
    ...
    def run(self):
        ...
        # initialisation des messages reçus
        part = ''
        # boucle infinie de réception
        while True:
            try:
                # attente (bloquante) d'une nouvelle partie de message
                newpart    = self.__socket.recv(4096)
                # décodage de la nouvelle partie
                newpart = newpart.decode(encoding='utf8')
                # si la partie reçue est vide
                if not newpart:
                    # la connexion est perdue
                    break
                # on concatène la nouvelle partie à la partie existante
                part += newpart
                # si le dernier caractère n'est pas un séparateur
                # il faut attendre la partie suivante
                if not newpart.endswith(server.SEP):
                    continue
                # on sépare les messages au cas où
                # on en aurait récupéré plusieurs d'un coup
                lstmsg = part.split(server.SEP)[0:-1]
                # remise à zéro des messages reçus
                part = ''
