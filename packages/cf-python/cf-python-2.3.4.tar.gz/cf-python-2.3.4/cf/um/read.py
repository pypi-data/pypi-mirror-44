...
if netCDF4.__version__ < '1.4':
    ...
    import cftime
    netCDF4_netcdftime_datetime = cftime.datetime
...
'''
    def __init__(self, var, fmt, byte_ordering, word_size, um_version,
                 set_standard_name, height_at_top_of_model, _debug,
                 **kwargs):
        '''
...
#            self.cell_method_axis_name = {'area': 'area'}
            ...
            if cell_methods is not None:
                field.insert_cell_methods(cell_methods)
...
:Returns:
    ...
    out : cf.DimensionCoordinate or None
        ...
        # Insert new Z axis
        field.insert_axis(DomainAxis(self.nz), key=zdim)
        ...
        key_a = field.insert_aux(ac, axes=[zdim], copy=False)
        ...
        if not TOA_height:
            ...
            field.insert_dim(dc, key=zdim, copy=False)
        #--- End: if
        ...
        key_b = field.insert_aux(ac, axes=[zdim], copy=False)

        if bool(dc):
#           self.cell_method_axis_name['z'] = dc.identity()
           
           # atmosphere_hybrid_height_coordinate coordinate reference
           ref = CoordinateReference(name='atmosphere_hybrid_height_coordinate',
                                     ancillaries={'a': key_a, 'b': key_b, 'orog': None})
#           ref.inspect()
#                                     a=key_a, b=key_b)
#               coordinates=(key_a, key_b), coord_terms=('a', 'b'))
           
           field.insert_ref(ref, copy=False)
        #--- End: if
...
:Returns:
    ...
    out: `cf.Coordinate`
        ...
        if standard_name is not None:
            coord.setprop('standard_name', standard_name)
...
:Returns:
    ...
    out: `cf.Data`
        ...
        if len(recs) == 1:
            ...
            if self._debug:
                print '    location =',yx_shape,'subarray[...].max() =', data.partitions[()].subarray[...].max()
        else:
            # --------------------------------------------------------
            ...
            if pmndim == 1:
                ...
                for i, rec  in enumerate(recs):
                    ...
                    if self._debug:
                        print '    header_offset =', rec.hdr_offset, 'location =',location,'subarray[...].max() =', subarray[...].max()
                #--- End: for
                ...
                for i, rec  in enumerate(recs):
                    ...
                    if self._debug:
                        print '    location =',location,'subarray[...].max() =', subarray[...].max()
                #--- End: for
...
#                self.cell_method_axis_name['z'] = c.identity()
            ...
            if aux:
                ...
                self.field.insert_aux(c, axes=[_axis['z']], copy=False)
            else:
...
:Returns:
    ...
    out: `cf.DimensionCoordinate`
        ...
        '''
        if self._debug:
            print 'Creating Z coordinates and bounds from BLEV, BRLEV and BRSVD1:'
...
'''
        if self._debug:
            print 'Creating Z reference coordinates from BRLEV'
