"""This module provides many useful functions used by Davte when programming in python.
"""
...
def markdown_check(text, symbols):
    """Return True if all `symbols` occur an even number of times in `text`.
    """
    for s in symbols:
...
class MyOD(collections.OrderedDict):
    """Subclass of OrderedDict featuring `get_by_val` and `get_by_key_val` methods.
    """
    ...
    def get_by_val(self, val, case_sensitive=True):
        """Get key pointing to given val. Can be case-sensitive or insensitive.
