...
class ApproximateLayerStackBuilder(LayerStackBuilder):
    ...
    def add_region(self, region: pya.Region,
        ...
        for poly in region.each():
            ...
            approx_hull = approximate_polygon(poly.each_point_hull(), self.approx_error)
            approx_holes = [approximate_polygon(poly.each_point_hole(i), self.approx_error)
                            for i in range(n_holes)]

            points_hull = [(p.x, p.y) for p in approx_hull]
            points_holes = [[(p.x, p.y) for p in h] for h in approx_holes]

            self.builder.add_polygon(points_hull, points_holes,
                                     z_offset=z_offset,
                                     thickness=thickness,
                                     material=material)
