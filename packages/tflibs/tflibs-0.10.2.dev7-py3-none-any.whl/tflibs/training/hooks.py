...
class EvaluationRunHook(tf.train.SessionRunHook):
    ...
    def begin(self):
        if self._best_model_exporter_args is not None:
            # self.best_metric = tf.get_variable('best_metric', shape=(), dtype=tf.float32, trainable=False)
            # self.saver = tf.train.Saver(var_list=[self.best_metric])
            pass
    ...
    def _run_evaluation(self, run_context: tf.train.SessionRunContext):
        if self._lock.acquire(False):
            ...
            try:
                ...
                # metric_name = self._best_model_exporter_args.metric_name
                #
                # if metrics not in metrics:
                #     raise ValueError('Metric variable named `{}` is not found'.format(metric_name))
                #
                # metric = metrics[metric_name]

                # self.saver.restore(run_context.session, os.path.join(self.estimator.model_dir, 'eval', 'best-model'))
                ...
                tf.logging.info('Release lock')
                self._lock.release()

    def end(self, session: tf.Session):
        # eval_dir = os.path.join(self.estimator.model_dir, 'eval')
        # self.saver.save(session, os.path.join(eval_dir, 'best-model'))
        session.
        pass
...
class EvalSummaryHook(tf.train.SessionRunHook):
    ...
    def after_run(self,
        ...
        if run_context.stop_requested:
            tf.logging.info('Stop Requested')
...
class BestModelExporterHook(tf.train.SessionRunHook):
    ...
    def begin(self):
        metrics = tf.get_collection(self.metric_name)
    ...
    def after_run(self,
        ...
        tf.logging.info(metric_val)
        if run_context.stop_requested:
            tf.logging.info('LAST METRIC: {}'.format(metric_val))
