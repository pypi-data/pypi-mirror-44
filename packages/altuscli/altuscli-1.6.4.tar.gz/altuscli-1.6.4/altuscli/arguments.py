...
from altuscli.argprocess import ParamShorthand
from altuscli.argprocess import unpack_cli_arg
from altuscli.model import ShapeResolver
from altuscli.schema import SchemaTransformer


def create_argument_model_from_schema(schema):
    # Given a JSON schema (described in schema.py), convert it
    # to a shape object from `altus.model.Shape` that can be
...
class CLIArgument(BaseCLIArgument):
    ...
    def __init__(self,
        ...
        # dict we send to altus.  While we can change the .name attribute
        # which is the name exposed in the CLI, the serialized name we use
        # for altus is invariant and should not be changed.
    ...
    def add_to_params(self, parameters, value):
        if value is None:
            ...
            # something that can be consumed by altus.  Many times this is
