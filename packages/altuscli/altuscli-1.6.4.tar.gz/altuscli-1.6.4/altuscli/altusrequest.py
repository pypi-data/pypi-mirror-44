...
from altuscli.compat import HTTPResponse
from altuscli.compat import six
from altuscli.compat import urlsplit
from altuscli.compat import urlunsplit
from altuscli.exceptions import UnseekableStreamError
from altuscli.thirdparty.requests import models
from altuscli.thirdparty.requests.packages.urllib3.connection import HTTPConnection
from altuscli.thirdparty.requests.packages.urllib3.connection import VerifiedHTTPSConnection  # noqa
from altuscli.thirdparty.requests.packages.urllib3.connectionpool import HTTPConnectionPool   # noqa
from altuscli.thirdparty.requests.packages.urllib3.connectionpool import HTTPSConnectionPool  # noqa
from altuscli.thirdparty.requests.sessions import REDIRECT_STATI
...
def prepare_request_dict(request_dict, endpoint_url, user_agent_header):
    r = request_dict
...
def create_request_object(request_dict):
    return AltusRequest(method=request_dict['method'],
                        url=request_dict['url'],
                        data=request_dict['body'],
                        headers=request_dict['headers'])


class AltusHTTPResponse(HTTPResponse):
...
class AltusRequest(models.RequestEncodingMixin, models.Request):
    def __init__(self, *args, **kwargs):
        self.auth_path = None
    ...
    def prepare(self):
        """Constructs a :class:`AltusPreparedRequest <AltusPreparedRequest>`."""
        # Eventually I think it would be nice to add hooks into this process.
        p = AltusPreparedRequest(self)
...
class AltusPreparedRequest(models.PreparedRequest):
    ...
    def __init__(self, original_request):
        ...
        super(AltusPreparedRequest, self).__init__()
    ...
    def prepare_body(self, data, files, json=None):
        ...
        super(AltusPreparedRequest, self).prepare_body(data, files, json)
        ...
        if 'Content-Length' not in self.headers:
            if hasattr(data, 'seek') and hasattr(data, 'tell'):
                ...
                # Altus Services so remove it if it is added.
                if 'Transfer-Encoding' in self.headers:
                    self.headers.pop('Transfer-Encoding')


class AltusHTTPConnection(HTTPConnection):
        ...
        try:
            ...
            if self._is_100_continue_status(maybe_status_line):
                ...
                response_class = functools.partial(
                    AltusHTTPResponse, status_tuple=status_tuple)
...
class AltusHTTPSConnection(VerifiedHTTPSConnection):
    pass


HTTPSConnectionPool.ConnectionCls = AltusHTTPSConnection
HTTPConnectionPool.ConnectionCls = AltusHTTPConnection
