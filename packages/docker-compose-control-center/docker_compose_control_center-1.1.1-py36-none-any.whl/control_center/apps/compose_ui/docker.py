                ...
                try:
                    ...
                    if yml_file:
                        _cache["config"] = ComposeProjectConfig(
                            original_file=stream, config=yml_file, project_name=settings.COMPOSE_PROJECT
                        )
...
def client() -> DockerClient:
    ...
    if hasattr(settings, "PRIVATE_DOCKER_REPOSITORY") and settings.PRIVATE_DOCKER_REPOSITORY["available"]:
        docker_client.login(
            username=settings.PRIVATE_DOCKER_REPOSITORY["username"],
            password=settings.PRIVATE_DOCKER_REPOSITORY["password"],
            registry=settings.PRIVATE_DOCKER_REPOSITORY["url"],
        )
...
def execute_docker_command(args: List[str], debug: bool = True) -> str:
    try:
        arguments = ["docker", "--log-level", "ERROR"] + args
        if debug:
            logger.debug("command:\n" + str(arguments))
        output = subprocess.check_output(arguments, stderr=subprocess.STDOUT).decode()
        if debug:
        ...
        error_output = error.output.decode()
        logger.exception(f"error running docker command: {error_output}")
        raise error
