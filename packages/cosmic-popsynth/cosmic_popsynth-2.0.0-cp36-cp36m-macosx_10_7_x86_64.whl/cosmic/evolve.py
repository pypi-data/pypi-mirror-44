...
class Evolve(Table):
    ...
    def evolve(cls, initialbinarytable, BSEDict, **kwargs):
        ...
        if 'neta' not in list(initialbinarytable.keys()):
            initialbinarytable['neta'] = BSEDict['neta']
        if 'bwind' not in list(initialbinarytable.keys()):
            initialbinarytable['bwind'] = BSEDict['bwind']
        if 'hewind' not in list(initialbinarytable.keys()):
            initialbinarytable['hewind'] = BSEDict['hewind']
        if 'alpha1' not in list(initialbinarytable.keys()):
            initialbinarytable['alpha1'] = BSEDict['alpha1']
        if 'lambdaf' not in list(initialbinarytable.keys()):
            initialbinarytable['lambdaf'] = BSEDict['lambdaf']
        if 'cekickflag' not in list(initialbinarytable.keys()):
            initialbinarytable['cekickflag'] = BSEDict['cekickflag']
        if 'cemergeflag' not in list(initialbinarytable.keys()):
            initialbinarytable['cemergeflag'] = BSEDict['cemergeflag']
        if 'cehestarflag' not in list(initialbinarytable.keys()):
            initialbinarytable['cehestarflag'] = BSEDict['cehestarflag']
        if 'ceflag' not in list(initialbinarytable.keys()):
            initialbinarytable['ceflag'] = BSEDict['ceflag']
        if 'tflag' not in list(initialbinarytable.keys()):
            initialbinarytable['tflag'] = BSEDict['tflag']
        if 'ifflag' not in list(initialbinarytable.keys()):
            initialbinarytable['ifflag'] = BSEDict['ifflag']
        if 'wdflag' not in list(initialbinarytable.keys()):
            initialbinarytable['wdflag'] = BSEDict['wdflag']
        if 'ppsn' not in list(initialbinarytable.keys()):
            initialbinarytable['ppsn'] = BSEDict['ppsn']
        if 'bhflag' not in list(initialbinarytable.keys()):
            initialbinarytable['bhflag'] = BSEDict['bhflag']
        if 'nsflag' not in list(initialbinarytable.keys()):
            initialbinarytable['nsflag'] = BSEDict['nsflag']
        if 'mxns' not in list(initialbinarytable.keys()):
            initialbinarytable['mxns'] = BSEDict['mxns']
        if 'pts1' not in list(initialbinarytable.keys()):
            initialbinarytable['pts1'] = BSEDict['pts1']
        if 'pts2' not in list(initialbinarytable.keys()):
            initialbinarytable['pts2'] = BSEDict['pts2']
        if 'pts3' not in list(initialbinarytable.keys()):
            initialbinarytable['pts3'] = BSEDict['pts3']
        if 'ecsnp' not in list(initialbinarytable.keys()):
            initialbinarytable['ecsnp'] = BSEDict['ecsnp']
        if 'ecsn_mlow' not in list(initialbinarytable.keys()):
            initialbinarytable['ecsn_mlow'] = BSEDict['ecsn_mlow']
        if 'aic' not in list(initialbinarytable.keys()):
            initialbinarytable['aic'] = BSEDict['aic']
        if 'sigma' not in list(initialbinarytable.keys()):
            initialbinarytable['sigma'] = BSEDict['sigma']
        if 'sigmadiv' not in list(initialbinarytable.keys()):
            initialbinarytable['sigmadiv'] = BSEDict['sigmadiv']
        if 'bhsigmafrac' not in list(initialbinarytable.keys()):
            initialbinarytable['bhsigmafrac'] = BSEDict['bhsigmafrac']
        if 'polar_kick_angle' not in list(initialbinarytable.keys()):
            initialbinarytable['polar_kick_angle'] = BSEDict['polar_kick_angle']
        if 'beta' not in list(initialbinarytable.keys()):
            initialbinarytable['beta'] = BSEDict['beta']
        if 'xi' not in list(initialbinarytable.keys()):
            initialbinarytable['xi'] = BSEDict['xi']
        if 'acc2' not in list(initialbinarytable.keys()):
            initialbinarytable['acc2'] = BSEDict['acc2']
        if 'epsnov' not in list(initialbinarytable.keys()):
            initialbinarytable['epsnov'] = BSEDict['epsnov']
        if 'eddfac' not in list(initialbinarytable.keys()):
            initialbinarytable['eddfac'] = BSEDict['eddfac']
        if 'gamma' not in list(initialbinarytable.keys()):
            initialbinarytable['gamma'] = BSEDict['gamma']
        if 'bconst' not in list(initialbinarytable.keys()):
            initialbinarytable['bconst'] = BSEDict['bconst']
        if 'ck' not in list(initialbinarytable.keys()):
            initialbinarytable['ck'] = BSEDict['ck']
        if 'merger' not in list(initialbinarytable.keys()):
            initialbinarytable['merger'] = BSEDict['merger']
        if 'windflag' not in list(initialbinarytable.keys()):
        ...
        if pd.Series(natal_kick_columns).isin(list(initialbinarytable.keys())).all() and 'natal_kick_array' not in list(initialbinarytable.keys()):
            initialbinarytable['natal_kick_array'] = initialbinarytable[natal_kick_columns].values.tolist()
        if 'natal_kick_array' not in list(initialbinarytable.keys()):
        ...
        if pd.Series(qcrit_columns).isin(list(initialbinarytable.keys())).all() and 'qcrit_array' not in list(initialbinarytable.keys()):
            initialbinarytable['qcrit_array'] = initialbinarytable[qcrit_columns].values.tolist()

        if 'qcrit_array' not in list(initialbinarytable.keys()):
