...
class AzureContext(BackendContext, CloudConnector):
    ...
    def _authorize_app(self, service_principal):
        click.echo('(6/7) Authorizing MissingLink App to manage virtual machines')
    ...
    def _create_key(self):
        click.echo('(5/7) Creating a key in the Key Vault')
    ...
    def _assign_role_key_vault_perms(self):
        click.echo('(3/7) Giving permissions to access Key Vault to worker instances')
    ...
    def _get_and_deploy_template(self):
        click.echo('(1/7) Applying template for Resource Group, Network and Key Vault. This may take a few minutes')
    ...
    def _setup_default_storage(self):
        click.echo('(2/7) Setting up default container for artifacts')
        storage_client = get_client_from_cli_profile(StorageManagementClient)
    ...
    def _register_compute_provider(self):
        click.echo('(4/7) Registering compute service in Azure subscription')
        rm_client = get_client_from_cli_profile(ResourceManagementClient)
        rm_client.providers.register('Microsoft.Compute')
        click.echo('Done')

    def _create_cloud_connector(self):
        click.echo('(7/7) Saving state to MissingLink servers')
        template = {
