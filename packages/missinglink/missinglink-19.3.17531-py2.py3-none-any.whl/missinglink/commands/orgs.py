...
def org_members(ctx, org):
    ...
    output_result(ctx, result.get('members', []), ['user_id', 'email', 'first_name', 'last_name'])
...
@orgs_commands.command('leave')
@click.option('--org', required=True, multiple=True)
@click.pass_context
def user_leave_orgs(ctx, org):

    for single_org in list(org):
        data = {}

        add_to_data_if_not_none(data, single_org, 'org_id')

        result = ApiCaller.call(ctx.obj, ctx.obj.session, 'post', 'orgs/{org}/leave'.format(org=single_org), data)
