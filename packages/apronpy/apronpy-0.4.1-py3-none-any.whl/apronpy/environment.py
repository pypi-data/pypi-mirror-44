...
class PyEnvironment:
    ...
    def __init__(self, int_vars: List[PyVar] = None, real_vars: List[PyVar] = None):
        if int_vars:
            ...
            int_arr = typ(*(x.var.encode('utf-8') for x in int_vars))
        else:
            int_size = 0
            int_arr = None
        if real_vars:
            ...
            real_arr = typ(*(x.var.encode('utf-8') for x in real_vars))
        else:
            real_size = 0
            real_arr = None
    ...
    def rename(self, old_vars: List[PyVar], new_vars: List[PyVar]):
        ...
        old_arr = old_typ(*(x.var.encode('utf-8') for x in old_vars))
        new_typ: Type = c_char_p * o_size
        new_arr = new_typ(*(x.var.encode('utf-8') for x in new_vars))
        p = DimPerm()
    ...
    def remove(self, del_vars: List[PyVar] = None):
        if del_vars:
            ...
            arr = typ(*(x.var.encode('utf-8') for x in del_vars))
        else:
            size = 0
            arr = None
