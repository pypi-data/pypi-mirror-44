...
class PyLinexpr1:
    ...
    def get_coeff(self, var: PyVar):
        coeff = libapron.ap_linexpr1_coeffref(self, var.var.encode('utf-8'))
        if coeff.contents.discr == CoeffDiscr.AP_COEFF_INTERVAL:
            ...
            if coeff.contents.val.scalar.contents.discr == ScalarDiscr.AP_SCALAR_MPQ:
                ...
                result = PyDoubleScalarCoeff()
        libapron.ap_linexpr1_get_coeff(result.coeff, self, var.var.encode('utf-8'))
        return result

    def set_coeff(self, var: PyVar, coeff: PyCoeff):
        coeffref = libapron.ap_linexpr1_coeffref(self, var.var.encode('utf-8'))
        libapron.ap_coeff_set(coeffref, coeff.coeff)
