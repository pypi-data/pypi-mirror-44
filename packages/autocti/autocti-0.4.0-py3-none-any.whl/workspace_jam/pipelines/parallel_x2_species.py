...
def make_pipeline(phase_folders=None, mask_function=msk.Mask.empty_for_shape):
    ...
    phase2 = ParallelPhase(phase_name='phase_2_x2_species_init', phase_folders=phase_folders,
                           parallel_species=[prior_model.PriorModel(arctic_params.Species),
    ...
    class ParallelHyperModelFixedPhase(ph.ParallelHyperPhase):
        ...
        def pass_priors(self, results):
            ...
            self.parallel_species = results.from_phase('phase_2_x2_species_init').constant.parallel_species
            self.parallel_ccd = results.from_phase('phase_2_x2_species_init').constant.parallel_ccd
    ...
    class ParallelHyperFixedPhase(ph.ParallelHyperPhase):
        ...
        def pass_priors(self, results):
            ...
            self.parallel_species = results.from_phase('phase_2_x2_species_init').variable.parallel_species
            self.parallel_ccd = results.from_phase('phase_2_x2_species_init').variable.parallel_ccd
            self.parallel_ccd.well_fill_alpha = 1.0
            self.parallel_ccd.well_fill_gamma = 0.0
