...
%
The study of continuous scalar functions often centers about their expansion in terms of monomials.
%
This is made rigorous by the Generalised Stone-Weierstrass Theorem \cite{Hassani:2013}. Consider a continuous function $f(\vec{x}) \equiv f(x_0, x_1, ..., x_N)$ in $N+1$ variables. Then $f(\vec{x})$ can be represented as
%
\begin{align}
  \label{eq:mvt}
  f( \vec{x}) &= \sum_{\substack{d_i=0 \\ i \in \{0 \ldots N\}}}^{\infty} c_{d_0 d_1 \ldots d_N} x^{d_0} x^{d_1} \ldots x^{d_N}
  \\ \nonumber
  &\approx \sum_{\substack{d_i=0 \\ i \in \{0 \ldots N\}}}^{K} c_{d_0 d_1 \ldots d_N} x^{d_0} x^{d_1} \ldots x^{d_N}
\end{align}
%
From the first to second line of \eqn{eq:mvt} the equality has been replaced by an approximation as we have limited the linear representation to monomials with combined exponents no greater than $K(N+1)$. This implication follows from the Stone-Weierstrass Theorem \cite{Lang:1983} and tells us that a continuous function may be uniformly approximated by a polynomial.
%
\par This is key to the perspective of \gmvp{}:
%
given training data thought to be related to a continuous multivariate function, it may, particularly on small scales, be well approximated by a polynomial in an appropriate coordinate basis.
%
% GMVP
\hspace{1cm}
{\scriptsize
