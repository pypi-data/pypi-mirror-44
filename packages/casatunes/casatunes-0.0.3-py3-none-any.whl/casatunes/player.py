...
class CasaTunesPlayer:
    def __init__(self, source, url, logo_prefix_url):
        self._source = source
    ...
    async def update_now_playing(self):
        response = await async_request(self._url_now_playing)
    ...
    async def _set_property(self, prop, value):
        ...
        response = await async_request(self._url_now_playing, props)
        if response is False:
            return False
        self._set_now_playing_dict(response)
        return True
    ...
    async def _send_action(self, action):
        ...
        response = await async_request(url, json=False)
        if response is False:
            return False
        elif response == '"OK"':
            return True
