...
def make_pipeline(phase_folders=None):
    ...
    class SubhaloPhase(ph.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.shear = results.from_phase('phase_5_subhalo').variable.lens.shear
            self.lens_galaxies.subhalo.mass = results.from_phase('phase_5_subhalo').variable.subhalo.mass
            self.source_galaxies.source.pixelization = results.from_phase('phase_5_subhalo').variable.variable.pixelization
            self.source_galaxies.source.regularization = results.from_phase('phase_5_subhalo').best_result.variable.regularization

            self.lens_galaxies.lens.mass.centre_0 = results.from_phase('phase_5_subhalo').variable.lens.mass.centre_0
            self.lens_galaxies.lens.mass.centre_1 = results.from_phase('phase_5_subhalo').variable.lens.mass.centre_1
            self.lens_galaxies.lens.mass.axis_ratio = results.from_phase('phase_5_subhalo').variable.lens.mass.axis_ratio
            self.lens_galaxies.lens.mass.phi = results.from_phase('phase_5_subhalo').variable.lens.mass.phi

            einstein_radius_value = results.from_phase('phase_5_subhalo').constant.lens.mass.einstein_radius
            self.lens_galaxies.lens.mass.einstein_radius = prior.GaussianPrior(mean=einstein_radius_value,  sigma=0.2)

    phase5 = SubhaloPhase(phase_name='phase_6_power_law', phase_folders=phase_folders,
                          lens_galaxies=dict(lens=gm.GalaxyModel(mass=mp.EllipticalPowerLaw,
                                                                 shear=mp.ExternalShear),
                                             subhalo=gm.GalaxyModel(mass=mp.SphericalTruncatedNFWChallenge)),
                          source_galaxies=dict(source=gm.GalaxyModel(pixelization=pix.AdaptiveMagnification,
                                                                     regularization=reg.Constant)),
                          interp_pixel_scale=0.05, sub_grid_size=2,
                          optimizer_class=nl.MultiNest)
