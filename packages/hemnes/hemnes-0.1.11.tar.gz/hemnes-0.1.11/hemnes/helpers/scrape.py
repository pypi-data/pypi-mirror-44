        ...
        for product in products_on_page:
            ...
            if product_overview is not None:
                ...
                if required is not None:
                    ...
                    if not strict:
                        logger.log_if_active('\t>>NOTE: potential product #%d failed required keywords - skipping' % (relative_rank+(page_num*products_per_page)))
                if strict:
                    ...
                    if next_product is not None:
                        ...
                        logger.log_if_active('\t>>processed product #%d' % (relative_rank+(page_num*products_per_page)))
                        relative_rank += 1
    except Exception:
        logger.log_if_active('ERROR: unexpected exception in hemnes.helpers.scrape.get_products_from_page_of_search_results')
    finally:
        return products
...
def get_products_for_query(query, driver, logger, prod_tag, required, sleep_time, products_per_page = __PRODUCTS_PER_PAGE):
    ...
    try:
        ...
        logger.log_if_active('found %d possible result(s) over %d page(s) for query: %s' % (num_of_results, num_of_pages, query))
        for page_num in range(num_of_pages):
            # avoid reloading the first page - already done above
            logger.log_if_active('\n>>processing products on page #%d of %d' % (page_num+1, num_of_pages))
            if page_num != 0:
