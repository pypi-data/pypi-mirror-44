...
def get_products(query, data_path = None, keywords = None, tag = None, sleep_time=None, log=False):
    ...
    """
    results = [] # outside of try-except to enable partial result return
    driver = webdriver.Chrome() # globally passed driver is faster & enables early termination due to uncaught exceptions
    logger = Logger()
    
    try:
        # optional param setup
        if log: logger.open()
        ...
        if data_path is not None: json.dump([result.to_dict() for result in results], open(data_path, 'w+'), sort_keys=True, indent=4)
    except Exception:
        logger.log_if_active('ERROR: unexpected exception in hemnes.get_products')
        logger.log_if_active('returning %d results found before the error' % len(results))
    finally:
        driver.close()
        driver.quit()
