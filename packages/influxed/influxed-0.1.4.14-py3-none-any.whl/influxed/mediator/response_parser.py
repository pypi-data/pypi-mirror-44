...
class json_parse(response_parser):
    ...
    def from_json_string(self, response):
    ...
    def parse_response(self, response):
        ...
        """
        return self.from_json_string(response.body)

class msgpack_parse(response_parser):

    def from_msgpack_string(self, response):
...
class dataframe_parser(response_parser):
    ...
    def parse_response(self, response):
        ...
        """
        response_converted = self.response_to_dataframe(response.body)
    ...
    def response_to_dataframe(self, response):
        ...
        """
        response_converted = [json.loads(x) for x in response.decode("utf-8").split('\n') if x]
