...
def main(filename, pypi_username, password):
    ...
    if os.name == 'nt':
        ...
        subprocess.run(['python', str(setup_file), 'sdist', 'bdist_wheel'], stdout=subprocess.PIPE)
        subprocess.run(['twine', 'upload', folder / f"dist/*{version}*", '-u', pypi_username, '-p', password], stdout=subprocess.PIPE)
    else:
        subprocess.run(['black', f'"{str(setup_file)}"'], stdout=subprocess.PIPE)
        subprocess.run(['black', f'"{str(python_file)}"'], stdout=subprocess.PIPE)
        subprocess.run(['python', f'"{str(setup_file)}"', 'sdist', 'bdist_wheel'], stdout=subprocess.PIPE)
        subprocess.run(['twine', 'upload', f'"{folder} / dist/*{version}*"', '-u', f'"{pypi_username}"', '-p', f'"{password}"'], stdout=subprocess.PIPE)
