...
def main(filename, pypi_username, password):
    ...
    if os.name == 'nt':
        ...
        subprocess.run(['black', quote(f'{str(setup_file)}')], stdout=subprocess.PIPE)
        subprocess.run(['black', quote(f'{str(python_file)}')], stdout=subprocess.PIPE)
        subprocess.run(['python', quote(f'{str(setup_file)}'), 'sdist', 'bdist_wheel'], stdout=subprocess.PIPE)
        subprocess.run(['twine', 'upload', quote(f'{str(version_path)}'), '-u', f'"{pypi_username}"', '-p', f'"{password}"'], stdout=subprocess.PIPE)
