...
def main():
    ...
    if args.tunnel or args.join:
        fh          = open('procout', "w")
        comm        = [ 'python', os.path.dirname(os.path.abspath(__file__)) + './3rdparty/pagekite.py', '--nocrashreport', '--clean', '--frontend=' + APP_HOST + ':' + str(LT_PORT), '--service_on=http:' + APP_HOST + ':localhost: ' + str(args.port) + ':2MagicDav1d' ]
