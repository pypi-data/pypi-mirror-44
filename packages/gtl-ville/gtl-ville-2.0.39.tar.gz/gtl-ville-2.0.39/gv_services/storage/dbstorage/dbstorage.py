...
DATAPOINTEIDS = enums.AttId.datapointeids
...
class DbStorage:
    ...
    async def update_road(self, roads, fields):
        ...
        for road in await dbrequest.update_road(self.dbpool, modifiedroads,
            ...
            for field in fields:
                if field in road:
                    self.roads[eid][field] = road[field]
    ...
    async def update_zone(self, zones, fields):
        ...
        for zone in await dbrequest.update_zone(self.dbpool, modifiedzones,
            ...
            for field in fields:
                if field in zone:
                    self.zones[eid][field] = zone[field]
    ...
    async def get_roads(self, eids=None):
        ...
        for roadeid, road in roads.items():
            datapointeids = self.roadsdatapoints.get(roadeid)
            if datapointeids:
                road[enums][WEBATT][DATAPOINTEIDS] = datapointeids
        return roads
