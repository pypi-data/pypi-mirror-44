...
from .objects import HealthCheckResult, StatusList
...
def healthcheck(url: str, timeout: Union[float, int],
                find_data: Optional[str] = None) -> HealthCheckResult:
    ...
    Returns a HealthCheckResult object with the requested URL, its status and
    checks results.
    """
    ...
    result = HealthCheckResult(url, alive, ok)
    if alive and ok:
        status = 'alive and OK'
    elif alive:
...
async def check_urls(urls: list, timeout, validations: Optional[list]) -> StatusList:
