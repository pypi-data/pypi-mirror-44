...
from .objects import StatusList
from .settings import GITHUB_API_TOKEN, REQUESTS_TIMEOUT, GITHUB_BRANCH, \
    ...
    URLS_VALIDATION, NOTIFY_HEADERS, NOTIFY_PAYLOAD
...
def _to_github(repo_name: str, github_api_token: str, filename: str,
               statuses: StatusList, branch: str = GITHUB_BRANCH,
               author_email: str = GITHUB_COMMITTER_EMAIL):
...
def _notify(url: str, statuses: StatusList, payload: Optional[str] = None,
            headers: Optional[dict] = None):
    failed_urls = ','.join([status.url for status in statuses if not status])
    if payload:
        if 'HEALTHCHECKER_FAILED_URLS' in payload:
    ...
    logger.debug('Notifying %s with headers: %s and payload: %s',
                 url, headers, notify_payload)
    if notify(url, payload, headers):
...
async def main():
    ...
    if notify_url and not statuses:
        _notify(notify_url, statuses, notify_payload, notify_headers)
