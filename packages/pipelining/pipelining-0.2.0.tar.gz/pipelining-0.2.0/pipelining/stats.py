...
def __yield_throughput(pipeline: Pipeline, time_delta: float):
    for name, stage in pipeline.stages.items():
        yield dict(name=name, throughput=stage.ticks.get() / time_delta)

def compute_pipeline_throughput(pipeline: Pipeline):
    if not pipeline.ticks_ts:
        raise Exception("Pipeline hasn't started")

    time_delta = time() - pipeline.ticks_ts
    throughput = __yield_throughput(pipeline, time_delta)
