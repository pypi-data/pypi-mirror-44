...
def code_to_type(code):
    ...
    strobe_types = [EVTYPE_STRON, EVTYPE_STROFF, EVTYPE_MARK]
    scalar_types = [EVTYPE_SCALAR]
    snip_types = [EVTYPE_SNIP]

    if code in strobe_types:
        s = 'epocs'
    elif code in snip_types:
        s = 'snips'
    elif code & EVTYPE_MASK == EVTYPE_STREAM:
        s = 'streams'
...
def check_ucf(code):
    ...
    return code & EVTYPE_UCF == EVTYPE_UCF
...
def epoc_to_type(code):
    ...
    strobe_on_types = [EVTYPE_STRON, EVTYPE_MARK]
    strobe_off_types = [EVTYPE_STROFF]
    t = 'unknown'
    ...
    for file in file_list:
        ...
        with open(file['fullname'], 'rb') as sev:
            ...
            if stream_header.file_version < 4:
                ...
                data_format &= 0b111
                stream_header.data_format = ALLOWED_FORMATS[data_format]
    ...
    for this_event in event_names:
        ...
        if channel > 0:
            try:
                ...
                warnings.warn('channel {0} not found in {1} event'.format(channel, temp_event_name), Warning)
                continue
        else:
            search_ch = min_chan
        ...
        for jjj in hour_values:
            ...
            if len(temp_num) < 1:
                raise Exception('matching file not found for hour {0} channel {1}'.format(jjj, matching_ch))
            elif len(temp_num) > 1:
                raise Exception('too many matches found for hour {0} channel {1}'.format(jjj, matching_ch))
            temp_num = temp_num[0]
            ...
            for chan in channels:
                ...
                for kk in range(start_hour_file[ii], end_hour_file[ii] + np.uint64(1)):
                    ...
                    with open(file_list_temp[file_num]['fullname'], 'rb') as f:
                        ...
                        if len(channels) > 1:
                            data[varname].data[ii][chan-1, int(chan_index):int(chan_index + read_size)] = ddd
                        else:
                            data[varname].data[ii][0, int(chan_index):int(chan_index + read_size)] = ddd
...
def read_block(block_path, *, bitwise='', channel=0, combine=None, headers=0,
    ...
    if 'all' in evtype:
        ...
        for given_type in evtype:
            if given_type not in ALLOWED_EVTYPES:
                print('Unrecognized type: {0}\nAllowed types are: {1}'.format(given_type, ALLOWED_EVTYPES))
                return None
    evtype = list(set(evtype))
    
    use_outside_headers = 0
    do_headers_only = 0
    if isinstance(headers, StructType) or isinstance(headers, dict):
        use_outside_headers = 1
        header = headers
    else:
        header = StructType()
        if headers == 1:
    ...
    if not use_outside_headers:
        tsq_list = get_files(block_path, '.tsq')
        ...
        if len(tsq_list) < 1:
            ...
            if 'streams' in evtype:
                ...
                return read_sev(block_path, channel=channel, event_name=store, t1=t1,
                    t2=t2, ranges=ranges, verbose=verbose)
        ...
        try:
            for sort_id in os.listdir(sort_path):
                if os.path.isdir(os.path.join(sort_path, sort_id)):
                    ...
                    sort_files = get_files(os.path.join(sort_path, sort_id), '.SortResult')
    ...
    data = StructType()
    data.epocs = StructType()
    data.snips = StructType()
    data.streams = StructType()
    data.scalars = StructType()
    data.info = StructType()
    
    # set info fields
    [data.info.tankpath, data.info.blockname] = os.path.split(os.path.normpath(block_path))
    data.info.start_date = datetime.fromtimestamp(header.start_time)
    ...
    if not np.isnan(header.stop_time):
        data.info.stop_date = datetime.fromtimestamp(header.stop_time)
        ...
        for note_line in notes_txt_lines:
            ...
            if do_once:
                if '-' in data.info.start:
                    ...
                    yearfmt = '%m/%d/%Y'
                rec_start = datetime.strptime(data.info.start.lower(), '%I:%M:%S%p ' + yearfmt)
            ...
            if len(note_line) >= eee + 2:
                if note_line.startswith(test_str):
                    ...
                    note_dt = datetime.strptime(note_time.lower(), '%I:%M:%S%p')
    ...
    if ~use_outside_headers:
        ...
        while True:
            ...
            seen = set()
            store_codes = []
            for counter, x in enumerate(codes):
                if not (x in seen or seen.add(x)):
                    store_codes.append({'code'     : x,
                                       'type'      : heads[1, counter],
                                       'type_str'  : code_to_type(heads[1, counter]),
                                       'ucf'       : check_ucf(heads[1, counter]),
                                       'epoc_type' : epoc_to_type(heads[1, counter]),
                                       'dform'     : heads[8, counter],
                                       'size'      : heads[0, counter],
                                       'buddy'     : heads[3, counter],
                                       'temp'      : heads[:, counter]})
            
            
            for store_code in store_codes:
                if store_code['code'] in [EVMARK_STARTBLOCK, EVMARK_STOPBLOCK]:
                    continue
                ...
                store_code['var_name'] = fix_var_name(store_code['name'], verbose)                
                ...
                if not var_name in header.stores.keys():
                    if store_code['type_str'] != 'epocs':
                        header.stores[var_name] = StructType(name=store_code['name'],
                ...
                if store_code['type_str'] != 'epocs':
                    ...
                    header.stores[var_name].ts.append(np.reshape(heads[[[4],[5]], valid_ind].T, (-1,1)).T.view(np.float64) - header.start_time)
                        ...
                        if len(custom_sort_codes) > 0 and var_name in custom_sort_event:                            
                            ...
                            for tempp in range(len(custom_sort_event)):
                                if type(custom_sort_codes[tempp]) == np.ndarray and header.stores[var_name].name == custom_sort_event[tempp]:
                                    ...
                                    header.stores[var_name].sortcode.append(custom_sort_codes[tempp][valid_ind+(len(header.stores[var_name].sortcode))*len(codes)])
                    ...
                    epocs.ts[loc] = np.append(epocs.ts[loc], np.reshape(heads[[[4],[5]], valid_ind].T, (-1,1)).T.view(np.float64) - header.start_time)
        ...
        for ii in range(len(epocs.name)):
            ...
            if epocs.type[ii] == 'onset':
                var_name = fix_var_name(epocs.name[ii])
                header.stores[var_name] = StructType()
        ...
        for ii in range(len(epocs.name)):
            if epocs.type[ii] == 'offset':
                var_name = fix_var_name(epocs.buddies[ii])
    ...
    for var_name in header.stores.keys():
        if header.stores[var_name].type_str not in evtype:
            continue
        ...
        if 'ts' in header.stores[var_name].keys():
            if header.stores[var_name].type_str == 'streams':
                header.stores[var_name].start_time = [[] for jj in range(num_ranges)]
            else:
                header.stores[var_name].filtered_ts = [[] for jj in range(num_ranges)]
            if hasattr(header.stores[var_name], 'chan'):
                header.stores[var_name].filtered_chan = [[] for jj in range(num_ranges)]
            if hasattr(header.stores[var_name], 'sortcode'):
                header.stores[var_name].filtered_sort_code = [[] for jj in range(num_ranges)]
            if hasattr(header.stores[var_name], 'data'):
                header.stores[var_name].filtered_data = [[] for jj in range(num_ranges)]
            
            filter_ind = [[] for i in range(num_ranges)]
            for jj in range(num_ranges):
                start = valid_time_range[0,jj]
                stop = valid_time_range[1,jj]
                ind1 = header.stores[var_name].ts >= start
                ind2 = header.stores[var_name].ts < stop
                ...
                if len(filter_ind[jj]) == 0:
                    ...
                    if header.stores[var_name].type_str == 'streams':
                        ...
                        if len(ind2) > 0:
                            ...
                            nchan = max(header.stores[var_name].chan)
                            if ind2 - nchan > 0:
                                filter_ind[jj] = ind2 - np.arange(nchan-1, -1, -1)
                                temp = header.stores[var_name].ts[filter_ind[jj]]
                                header.stores[var_name].start_time[jj] = temp[0]
                                bSkip = 1
                ...
                if len(filter_ind[jj]) > 0:
                    ...
                    if header.stores[var_name].type_str == 'streams':
                        ...
                        if not bSkip:
                            ...
                            temp = filter_ind[jj]
                            if temp[0] - nchan > 0:
                                filter_ind[jj] = np.concatenate(-np.arange(nchan-1, -1, -1) + temp[0], filter_ind[jj])
                            temp = header.stores[var_name].ts[filter_ind[jj]]
                            header.stores[var_name].start_time[jj] = temp[0]
                    else:
                        header.stores[var_name].filtered_ts[jj] = header.stores[var_name].ts[filter_ind[jj]]
                    
                    if hasattr(header.stores[var_name], 'chan'):
                        if len(header.stores[var_name].chan) > 1:
                            header.stores[var_name].filtered_chan[jj] = header.stores[var_name].chan[filter_ind[jj]]
                        else:
                            header.stores[var_name].filtered_chan[jj] = [header.stores[var_name].chan]
                    if hasattr(header.stores[var_name], 'sortcode'):
                        header.stores[var_name].filtered_sort_code[jj] = header.stores[var_name].sortcode[filter_ind[jj]]
                    if hasattr(header.stores[var_name], 'data'):
                        header.stores[var_name].filtered_data[jj] = header.stores[var_name].data[filter_ind[jj]]
            
            if header.stores[var_name].type_str == 'streams':
                delattr(header.stores[var_name], 'ts')
                delattr(header.stores[var_name], 'data')
                delattr(header.stores[var_name], 'chan')
                if not hasattr(header.stores[var_name], 'filtered_chan'):
                    header.stores[var_name].filtered_chan = [[] for i in range(num_ranges)]
                if not hasattr(header.stores[var_name], 'filtered_data'):
                    header.stores[var_name].filtered_data = [[] for i in range(num_ranges)]
                if not hasattr(header.stores[var_name], 'start_time'):
                    header.stores[var_name].start_time = [-1]
            else:
                # consolidate other fields
                if hasattr(header.stores[var_name], 'filtered_ts'):
                    header.stores[var_name].ts = np.concatenate(header.stores[var_name].filtered_ts)
                    delattr(header.stores[var_name], 'filtered_ts')
                else:
                    header.stores[var_name].ts = []
                if hasattr(header.stores[var_name], 'chan'):
                    if hasattr(header.stores[var_name], 'filtered_chan'):
                        try:
                            header.stores[var_name].chan = np.concatenate(header.stores[var_name].filtered_chan)
                        except:
                            header.stores[var_name].chan = header.stores[var_name].filtered_chan[0]
                        delattr(header.stores[var_name], 'filtered_chan')
                    else:
                        header.stores[var_name].chan = []
                if hasattr(header.stores[var_name], 'sortcode'):
                    if hasattr(header.stores[var_name], 'filtered_sort_code'):
                        header.stores[var_name].sortcode = np.concatenate(header.stores[var_name].filtered_sort_code)
                        delattr(header.stores[var_name], 'filtered_sort_code')
                    else:
                        header.stores[var_name].sortcode = []
                if hasattr(header.stores[var_name], 'data'):
                    if hasattr(header.stores[var_name], 'filtered_data'):
                        header.stores[var_name].data = np.concatenate(header.stores[var_name].filtered_data)
                        delattr(header.stores[var_name], 'filtered_data')
                    else:
                        header.stores[var_name].data = []
        else:
            # handle epoc events
            ...
            for jj in range(num_ranges):
                start = valid_time_range[0,jj]
                stop = valid_time_range[1,jj]
                ind1 = header.stores[var_name].onset >= start
                ind2 = header.stores[var_name].onset < stop
                filter_ind.append(np.where(ind1 & ind2)[0])

            filter_ind = np.concatenate(filter_ind)
            if len(filter_ind) > 0:
                header.stores[var_name].onset = header.stores[var_name].onset[filter_ind]
                header.stores[var_name].data = header.stores[var_name].data[filter_ind]
                header.stores[var_name].offset = header.stores[var_name].offset[filter_ind]
                if var_name == 'Note':
                    header.stores[var_name].notes = note_text[filter_ind]
                # fix time ranges
                if header.stores[var_name].offset[0] < header.stores[var_name].onset[0]:
                    if header.stores[var_name].onset[0] > firstStart:
                        header.stores[var_name].onset = np.concatenate([[firstStart], header.stores[var_name].onset])
                if header.stores[var_name].offset[-1] > last_stop:
                    header.stores[var_name].offset[-1] = last_stop
            else:
                # default case is no valid events for this store
                header.stores[var_name].onset = []
                header.stores[var_name].data = []
                header.stores[var_name].offset = []
                if var_name == 'Note':
                    header.stores[var_name].notes = []
        
        # see if there are any notes to add
        if hasattr(header.stores, var_name):
            try:
                loc = notes.name.index(header.stores[var_name].name)
                header.stores[var_name].notes = StructType()
                ts = notes.ts[loc][0]
                note_index = np.array(notes.index[loc])
                ind1 = ts >= firstStart
                ind2 = ts < last_stop
                valid_ind = np.where(ind1 & ind2)[0]
                header.stores[var_name].notes.ts = ts[valid_ind]
                header.stores[var_name].notes.index = note_index[valid_ind]
                header.stores[var_name].notes.notes = note_str[note_index[valid_ind]-1] # zero-based indexing
            except:
                pass
    ...
    for current_name in header.stores.keys():
        if header.stores[current_name].type_str not in evtype:
            continue
        current_size = header.stores[current_name].size
        current_type_str = header.stores[current_name].type_str
        current_data_format = ALLOWED_FORMATS[header.stores[current_name].dform]
        if hasattr(header.stores[current_name], 'fs'):
            current_freq = header.stores[current_name].fs
        ...
        if current_type_str == 'epocs':
            pass
            '''
            if len(header.stores[current_name].data) > 0:
                header.stores[current_name].data = header.stores[current_name].data[np.newaxis].T
                header.stores[current_name].onset = header.stores[current_name].onset[np.newaxis].T
                header.stores[current_name].offset = header.stores[current_name].offset[np.newaxis].T
            if hasattr(header.stores[current_name], 'notes') and current_name != 'Note':
                header.stores[current_name].notes.notes = header.stores[current_name].notes.notes[np.newaxis].T
                header.stores[current_name].notes.index = header.stores[current_name].notes.index[np.newaxis].T
                header.stores[current_name].notes.ts = header.stores[current_name].notes.ts[np.newaxis].T
            '''
        elif current_type_str == 'scalars':
            nchan = np.max(header.stores[current_name].chan)
            ...
            if nchan > 1:
                ...
                for xx in range(nchan):
                    ind.append(np.where(header.stores[current_name].chan == xx+1)[0])
                    min_length = min(len(ind[-1]), min_length)
                    max_length = max(len(ind[-1]), max_length)
                if min_length != max_length:
                    warnings.warn('Truncating store {0} to {1} values (from {2})'.format(current_name, min_lengths, max_lengths), Warning)
                    ind = [ind[xx][:min_length] for xx in range(nchan)]
                if not nodata:
                    header.stores[current_name].data = header.stores[current_name].data[np.concatenate(ind)].reshape(nchan, -1)
                
                # only use timestamps from first channel
                header.stores[current_name].ts = header.stores[current_name].ts[ind[0]]
                
                # remove channels field
                delattr(header.stores[current_name], 'chan')
            if hasattr(header.stores[current_name], 'notes') and current_name != 'Note':
                header.stores[current_name].notes.notes = header.stores[current_name].notes.notes[np.newaxis].T
                header.stores[current_name].notes.index = header.stores[current_name].notes.index[np.newaxis].T
                header.stores[current_name].notes.ts = header.stores[current_name].notes.ts[np.newaxis].T
            ...
            header.stores[current_name].name = current_name
            header.stores[current_name].fs = current_freq
            
            if channel > 0:
                # test if chan is a list
                if len(header.stores[current_name].chan) > 1:
                    valid_ind = header.stores[current_name].chan == channel
                    if not nodata:
                        all_offsets = header.stores[current_name].data[valid_ind]
                    header.stores[current_name].chan = header.stores[current_name].chan[valid_ind]
                    header.stores[current_name].sortcode = header.stores[current_name].sortcode[valid_ind]
                    header.stores[current_name].ts = header.stores[current_name].ts[valid_ind]
                else:
                    # it's a single value
                    if channel != header.stores[current_name].chan[0]:
                        # there is only one channel and we want it..
                        raise Exception('channel {0} not found'.format(channel))
                    if not nodata:
                        all_offsets = header.stores[current_name].data
            else:
                if not nodata:
                    all_offsets = header.stores[current_name].data
            
            if len(header.stores[current_name].chan) > 1:
                header.stores[current_name].chan = header.stores[current_name].chan[np.newaxis].T
            header.stores[current_name].sortcode = header.stores[current_name].sortcode[np.newaxis].T
            header.stores[current_name].ts = header.stores[current_name].ts[np.newaxis].T
            ...
            if not nodata:                
                ...
                header.stores[current_name].data = []
                ...
                for f in range(len(arr)):
                    ...
                    uniqueVal, uniqueInd = np.unique(ind[:,0], return_index=True)
                    if len(uniqueInd) != len(ind[:,0]):
                        # only keep uniques
                        ind = ind[uniqueInd, :]
                        # remove last row
                        ind = ind[:-1,:]
                        warnings.warn('data missing from tev file for store:{0} time:{1}s'.format(current_name, np.round(header.stores[current_name].ts[event_count + len(ind)][0],3)), Warning)
                        if len(ind) == 0:
                            continue

                    # add data to big array
                    if min(ind.shape) == 1:
                        header.stores[current_name].data.append(tev_data[ind.flatten().astype(np.uint64)])
                    else:
                        header.stores[current_name].data.append(tev_data[ind.flatten().astype(np.uint64)].reshape((-1, npts)))
                    event_count += len(ind)
                
                # convert cell array for output
                if len(header.stores[current_name].data) > 1:
                    header.stores[current_name].data = np.concatenate(header.stores[current_name].data)
                else:
                    header.stores[current_name].data = header.stores[current_name].data[0]
                totalEvents = len(header.stores[current_name].data)
                if len(header.stores[current_name].chan) > 1:
                    header.stores[current_name].chan = header.stores[current_name].chan[:totalEvents]
                header.stores[current_name].sortcode = header.stores[current_name].sortcode[:totalEvents]
                header.stores[current_name].ts = header.stores[current_name].ts[:totalEvents]
            ...
            if current_name in sev_names:
                ...
                if len(block_notes) > 0:
                    for storeNote in block_notes:
                        if storeNote['StoreName'] == header.stores[current_name].name:
                ...
                header.stores[current_name] = d[current_name]
                header.stores[current_name].start_time = 0
            else:
                # make sure SEV files are there if they are supposed to be
                if header.stores[current_name].ucf == 1:
                    warnings.warn('Expecting SEV files for {0} but none were found, skipping...'.format(current_name), Warning)
                    continue
                
                header.stores[current_name].data = [[] for i in range(num_ranges)]
                for jj in range(num_ranges):
                    fc = header.stores[current_name].filtered_chan[jj]
                    if len(fc) < 1:
                        continue                    
                    if channel > 0:
                        if np.all(header.stores[current_name].filtered_chan[jj] == 1):
                            # there is only one channel here, use them all
                            valid_ind = np.arange(len(header.stores[current_name].filtered_data[jj]))
                        else:
                            valid_ind = fc == channel
                            if not np.any(valid_ind):
                                raise Exception('channel {0} not found in store {1}'.format(channel, current_name))
                            header.stores[current_name].filtered_chan[jj] = fc[valid_ind]
                        nchan = np.uint64(1)
                    else:
                        valid_ind = np.arange(len(header.stores[current_name].filtered_data[jj]))
                        nchan = np.uint64(np.max(fc))
                    chan_index = np.zeros(nchan, dtype=np.uint64)                    
                    
                    fc = header.stores[current_name].filtered_chan[jj]
                    these_offsets = header.stores[current_name].filtered_data[jj]
                    these_offsets = these_offsets[valid_ind]
                                        
                    # preallocate data array
                    npts = (current_size - np.uint32(10)) * np.uint32(4) // sz
                    header.stores[current_name].data[jj] = np.zeros([nchan, npts * np.uint64(len(these_offsets)) // nchan], dtype=current_data_format)
                    ...
                    for f in range(len(arr)):
                        ...
                        for kk in range(len(relative_offsets)):
                            if nchan > 1:
                                chan = fc[channel_offset]-1
                            else:
                                chan = 0
                            channel_offset += 1
                            if not np.any(ind[kk,:] <= len(tev_data)):
                                if not found_empty:
                                    warnings.warn('Data missing from tev file for store:{0} channel:{1} time:{2}s'.format(current_name, chan, np.round(t1 + chan_index / current_freq, 2)), Warning)
                                    found_empty = True
                                chan_index[chan] += npts
                                continue
                            found_empty = False
                            header.stores[current_name].data[jj][chan, chan_index[chan]:(chan_index[chan] + npts)] = tev_data[ind[kk,:].flatten().astype(np.uint64)]
                            chan_index[chan] += npts
                            #    header.stores[current_name].data.append(tev_data[ind.flatten().astype(np.uint64)].reshape((-1, npts)))
                    ...
                    minSample = np.uint64(np.maximum(np.ceil((valid_time_range[0,jj]-header.stores[current_name].start_time[jj]) * current_freq),0))
                    maxSample = np.uint64(np.minimum(np.maximum(np.floor((valid_time_range[1,jj]-header.stores[current_name].start_time[jj]) * current_freq),0) + 1, header.stores[current_name].data[jj].shape[1]))
                    header.stores[current_name].data[jj] = header.stores[current_name].data[jj][:,minSample:maxSample]
                    header.stores[current_name].start_time[jj] = header.stores[current_name].start_time[jj] + (minSample-1) / current_freq
                    
                if channel > 0:
                    header.stores[current_name].channel = channel
                delattr(header.stores[current_name], 'filtered_chan')
                delattr(header.stores[current_name], 'filtered_data')
            if len(header.stores[current_name].data) == 1:
                header.stores[current_name].data = header.stores[current_name].data[0]
                if header.stores[current_name].data.shape[0] == 1:
                    header.stores[current_name].data = header.stores[current_name].data[0]
                header.stores[current_name].start_time = header.stores[current_name].start_time[0]
        data[current_type_str][current_name] = header.stores[current_name]
    ...
    if 'streams' in evtype:
        for sev_name in sev_names:
            ...
            if sev_name not in [data.streams[fff].name for fff in data.streams.keys()]:
                ...
                data.streams[sev_name].startTime = 0
    ...
    if bitwise != '':
        ...
        data[bitwisetype][bitwise].bitwise = StructType()
            ...
            for ind in changes:
                ...
                if curr_state[ind]:
                    ...
                    if hasattr(data[bitwisetype][bitwise].bitwise, ffield):
                        ...
                        data[bitwisetype][bitwise].bitwise[ffield] = StructType()
