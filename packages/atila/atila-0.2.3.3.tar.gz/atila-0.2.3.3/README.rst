...
*Atila* is simple and minimal framework integrated with `Skitai App Engine`_ for exporting component and backend services.
  ...
  def test_futures (app, dbpath):
    ...
    def index (was):
        def response (was, rss, stock):
            stock.announcements = rs [0].data
            return was.API (stock) 
        
        stock_req = was.backend ("@sqlite").select (stocks").get ("*").filter (symbol='RHAT').execute ()
        stock = stock_req.getwait ().data
        if not stock:
          return was.response ("404 Not Found")        
                
        reqs = [stock_req = was.backend ("@sqlite").select (announcements").get ("*").filter (id=stock.id).execute ()]
...
.. code:: python
  ...
  def bearer_handler (was, token):
    claims = was.dejwt (token) #was.request.JWT will be created
    if "err" in claims:
      return claims ["err"]
