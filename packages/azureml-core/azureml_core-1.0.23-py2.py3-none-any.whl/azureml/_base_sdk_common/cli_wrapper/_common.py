...
def get_default_subscription_id(auth):
    ...
    :rtype: str
    """
    if isinstance(auth, ArmTokenAuthentication):
        if AZUREML_SUBSCRIPTION_ID in os.environ:
            ...
            raise AuthenticationException("The default subscription id cannot be determined for "
                                          "auth object of type = {}".format(auth.__class__))
...
def get_workspace_or_default_name(workspace_name, throw_error=False, auth=None, project_path=None):
    """
    Order is
    ...
    if not workspace_name:
        ...
        workspace_name = get_default_property(DEFAULT_WORKSPACE_CONFIG_KEY)
        if throw_error and not workspace_name:
...
def get_resource_group_or_default_name(resource_group_name, throw_error=False, auth=None, project_path=None):
    """
    Order is
    1) Get workspace name from the specified parameter,
    2) From project context,
...
def get_workspace_or_default(workspace_name=None, resource_group=None, auth=None, project_path=None):
    """
    Order is
    ...
    if resource_group and workspace_name:
        # Simple case where both are specified.
        return Workspace(default_subscription_id, resource_group, workspace_name, auth=auth)
    else:
        if not workspace_name:
...
def _get_project_object(auth=None, project_path=None):
    if not auth:
        auth = get_cli_specific_auth()
    ...
    if project_object:
        if project_object.workspace.subscription_id != default_subscription_id:
            raise UserErrorException("Default CLI subscription id = {}, which doesn't match "
                                     "with the project subscription id = {} "
                                     "at location = {}".format(default_subscription_id,
