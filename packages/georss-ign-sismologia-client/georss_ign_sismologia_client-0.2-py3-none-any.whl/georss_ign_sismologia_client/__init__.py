...
IMAGE_URL_PATTERN = "http://www.ign.es/web/resources/sismologia/www/" \
                    "dir_images_terremotos/detalle/{}.gif"

REGEXP_ATTR_MAGNITUDE = r'magnitud (?P<{}>[^ ]+) '\
    .format(CUSTOM_ATTRIBUTE)
REGEXP_ATTR_REGION = r'magnitud [^ ]+ en (?P<{}>[A-ZÁÉÓÜÑ0-9 \-\.]+) en'\
    .format(CUSTOM_ATTRIBUTE)
REGEXP_ATTR_PUBLISHED_DATE = r'-Info.terremoto: (?P<{}>.+)$'\
    .format(CUSTOM_ATTRIBUTE)
REGEXP_ATTR_SHORT_ID = r'http:\/\/www\.ign\.es\/web\/ign\/portal\/' \
                       r'sis-catalogo-terremotos\/-\/catalogo-terremotos\/' \
                       r'detailTerremoto\?evid=(?P<{}>\w+)$'\
    .format(CUSTOM_ATTRIBUTE)
