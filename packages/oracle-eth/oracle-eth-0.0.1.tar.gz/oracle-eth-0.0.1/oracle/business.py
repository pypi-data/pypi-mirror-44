...
class ArmyEvent(BaseEvent):
    ...
    def did_attribute_change(self, cport, army_private_key, operator_private_key, delegate_private_key, entry):
        ...
        if event.status == status.EVENT_ETHEREUM_SUBMIT_SUCESS:
            return
        id = event.id
        args = entry['args']
        ...
        if not Did.verify_did(did_json_bytes.decode(), sign_text_bytes.decode()):
            ...
            return
        # 数据签名验证成功
        Army.update(status=status.EVENT_SIGN_VERI_SUCESS).where(Army.id == id).execute()
        ...
        if len(str(original)) == 0:
            ...
            Army.update(status=status.EVENT_DECRYPTION_SUCESS).where(Army.id == id).execute()   # 解密成功
        ...
        if check is True:
            ...
            if tf is True:
                Army.update(status=status.EVENT_ETHEREUM_SUBMIT_SUCESS, txhash=txhash).where(Army.id == id).execute()
            if tf is False:
...
class OrganizationEvent(BaseEvent):
    ...
    def did_attribute_confirmed(self, cport, entry, *args, **kwargs):
        ...
        """
        # XXX: 需要确认是否会抛异常
        army_private_key = kwargs['army_private_key']
        operator_private_key = kwargs['operator_private_key']
        delegate_private_key = kwargs['delegate_private_key']
        ...
        did_json_bytes = b''.join(value.split(b'\n')[:-2])  # did 信息
        did_json_obj = json.loads(did_json_bytes.decode())  # did json对象
        # 在 dbs.save_organization(entry) 已经保存了这些信息，可以不用再次写入
        #################################################################
        # event.first_sign = first_sign_text_bytes.decode()
        # event.second_sign = second_sign_text_bytes.decode()
        # event.did = did_json_bytes.decode()
        # event.did_id = did_json_obj['id']
        # event.created = int(did_json_obj['created'])
        #################################################################
        ...
        # 数据签名验证成功
        event.status = status.EVENT_SIGN_VERI_SUCESS
        event.save()
    ...
    def monitor_event(self, event_name):
        ...
        while True:
            ...
            for entry in entries:
                ...
                args = entry['args']                    # event 的函数参数对应的数据
                name = args['name'].strip(b'\x00')      # 去除没有用的数据
                name = name.decode('utf8')              #
                event_name = entry['event']
                # 只处理军人身份的数据
                if event_name == 'DIDAttributeChange' and name == identity:
