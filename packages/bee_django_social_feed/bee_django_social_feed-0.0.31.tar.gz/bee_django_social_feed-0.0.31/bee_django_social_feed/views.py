...
def feeds(request):
    ...
    if tp == '0':
        ...
        feeds_data = Feed.objects.filter(publisher=user).order_by('-created_at')
    elif tp == '2':
        user_id = request.GET.get('user_id')
        classmates = get_classmates(user_id)
        feeds_data = Feed.objects.filter(publisher__in=classmates).order_by('-created_at')
    elif tp == '3':
        feed_id = request.GET.get('display_id')
        feeds_data = Feed.objects.filter(id=feed_id)
    elif tp == '4':
        feeds_data = Feed.objects.filter(is_star=True).order_by('-created_at')
    else:
        feeds_data = Feed.objects.order_by('-created_at')
    ...
    for e in data:
        ...
        for j in e.comments:
            ...
            if j.to_user:
                ...
                j.to_user_name = user_model.objects.get(pk=j.user_id).get_user_name()
...
def create_feed(request):
    if request.method == "POST":
        ...
        for i in feeds:
            ...
            i.publisher_name = user_model.objects.get(pk=i.publisher_id).get_user_name()
            i.publisher_avatar = unicode(user_model.objects.get(pk=i.publisher_id).get_user_profile_image())
