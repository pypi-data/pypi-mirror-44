...
class Version:
    ...
    def __init__(self, version):
        ...
        # a non digit (e.g. 'SNAPSHOT') specifier.

        # Handle when there is no specifier.
        if '-' not in self._str:
            specifier = None
            numeric_version = self._str
        else:
            numeric_version, specifier = self._str.split('-', maxsplit=1)

        # Parse the numeric part of versions.
        numeric_version_list = list(map(int, numeric_version.split('.')))
        ...
        # Attach specifier to adjusted version list.
        version_list = numeric_version_list
        if specifier is not None:
            version_list.append(specifier)
