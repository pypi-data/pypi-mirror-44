...
class OverflowBtn(VuetifyWidget):

    _model_name = Unicode('OverflowBtnModel').tag(sync=True)
    ...
    rules = List(None, allow_none=True).tag(sync=True)

    search_input = Any(Undefined).tag(sync=True)

    segmented = Bool(None, allow_none=True).tag(sync=True)
    ...
    transition = Any(Undefined).tag(sync=True)
...
OverflowBtn.items.default_value=None
OverflowBtn.rules.default_value=None
