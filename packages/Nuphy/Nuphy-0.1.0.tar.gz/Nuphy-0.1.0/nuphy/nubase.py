...
######################## MASSTABLE LOADED ######## READ Z A
            ...
            for li in masslist:
                if ( re.search( rese , li) ):
                    ...
                    if (len(self.massline)>0):
                        ...
                        print("     {:5s} (Z={:3d}) mex={:7.1f} {:s}{:s} amu={:7.4f} g/mol".format(self.name ,self.Z, self.mex, str(self.spin*2)+"/2" if self.spin!=int(self.spin) else str(int(self.spin)),'+' if self.parity==1 else '-', self.amu) )
                        break
        ...
        if not silent:
            print('        rho: elm/iso = {:.3f}/{:.3f} g/cm3 Mm_elm={:6.3f}'.format( densities[Z],self.isodensity,molarweights[Z]) )
...
print("i... module  nubase  is being loaded")
if __name__=="__main__":
    print("i... in nubase ")
