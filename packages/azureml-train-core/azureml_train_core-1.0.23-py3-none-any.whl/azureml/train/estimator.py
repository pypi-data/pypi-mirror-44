...
class MMLBaseEstimator(ABC):
    ...
    def _init_run_config(compute_target, vm_size=None, vm_priority=None, entry_script=None, script_params=None,
                         node_count=None, process_count_per_node=None, distributed_backend=None, use_gpu=None,
                         use_docker=None, custom_docker_image=None, image_registry_details=None, user_managed=False,
                         conda_packages=None, pip_packages=None, environment_definition=None, inputs=None,
...
class FrameworkBaseEstimator(MMLBaseEstimator):
    ...
    :param source_directory_data_store: The backing data store for the project share.
    :type source_directory_data_store: Datastore
    :param framework_name: Name of the framework to be used for the estimator. Supported values:
    ...
    SCENARIO_FILE_TEMPLATE = '{}-{}-{}.yml'
    ACR_IMAGE_TEMPLATE = 'viennaprivate.azurecr.io/{}:{}-{}'

    UNSUPPORTED_FRAMEWORK_VERSION_ERROR = '{name} {version} is not currently supported by the {name} estimator. ' \
        ...
        'experiment.'
    SCENARIO_FILE_PARSE_ERROR = 'There was an issue parsing the scenario file {}.'
    EMPTY_FRAMEWORK_VERSION_WARNING = 'framework_version is not specified, defaulting to version {}.'
    OPTIMIZED_MODE_PREVIEW_NOTICE = 'Note: Optimized Mode has been enabled which will use pre-built ' \
        ...
        if not MMLBaseEstimator._is_user_managed_environment(environment_definition):
            ...
            if framework_version is None:
                if len(self.__class__.get_supported_versions()) > 1:
                    module_logger.warning(self.EMPTY_FRAMEWORK_VERSION_WARNING.format(self.DEFAULT_VERSION))
            ...
            self._setup_environment(custom_docker_image, compute_target)

            MMLBaseEstimator._update_conda_dependencies(self._estimator_config.environment.
    ...
    def _load_from_scenario_file(self, framework_name, framework_version, framework_processor):
        scenario_filename = self.SCENARIO_FILE_TEMPLATE.format(framework_name,
                                                               framework_version,
                                                               framework_processor).lower()
        scenario_path = path.join(path.dirname(__file__), "scenarios", scenario_filename)

        if not path.isfile(scenario_path):
            raise TrainingException((self.UNSUPPORTED_FRAMEWORK_VERSION_ERROR).format(name=framework_name,
        ...
        try:
            ...
            raise TrainingException((self.SCENARIO_FILE_PARSE_ERROR).format(scenario_filename))

        return base_image, dependencies

    def _setup_environment(self, custom_docker_image, compute_target):
        if self._optimized_mode:
            if not (isinstance(compute_target, _BatchAITarget) or
                ...
                module_logger.warning("Optimized mode is not supported for non-AMLCompute targets."
                                      "Legacy mode will be used unless a different target is chosen.")
                self._optimized_mode = False

        if self._optimized_mode:
            base_image = self.ACR_IMAGE_TEMPLATE.format(self._framework_name.lower(), self._framework_version,
                                                        self._framework_processor)
            module_logger.warning(self.OPTIMIZED_MODE_PREVIEW_NOTICE)
            self._estimator_config.environment.python._base_conda_environment = 'base'
        else:
            base_image, dependencies = self._load_from_scenario_file(self._framework_name,
                                                                     self._framework_version,
                                                                     self._framework_processor)
            self._estimator_config.environment.python. \
