...
b = 256
q = 2 ** 255 - 19
large = 2 ** 252 + 27742317777372353535851937790883648493
...
def encodeint(y: int) -> bytes:
    ...
    return bytes([sum([bits[i * 8 + j] << j
                  for j in range(8)]) for i in range(b >> 3)])
...
def signature(m: bytes, sk: bytes, pk: bytes) -> bytes:
    ...
    R = scalarmult(B, r)
    S = (r + Hint(encodepoint(R) + pk + m) * a) % large
