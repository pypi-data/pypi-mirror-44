...
def dump_uvarint(writer, n):
    if n < 0:
        raise ValueError("Cannot dump signed value, convert it "
                         "to unsigned first.")
    buffer = _UVARINT_BUFFER
    shifted = True
    while shifted:
...
# so counting up, you go: 0, -1, 1, -2, 2, ... (as the first bit changes,
# sign flips)
#
...
#    This keeps positive number the same, and converts negative from
#    twos-complement to the appropriate value, while setting the sign bit.
#
...
# Then you can take "number XOR all-sign-bits", which is XOR 0 (identity) for
# positive and XOR 1 (bitwise negation) for negative. Cute and efficient.
#
# But this is harder in Python because we don't natively know the bit size of
# the number. So we have to branch on whether the number is negative.
...
class MessageType:
    ...
    def _additem(self, attr):
        ...
        for v in self.FIELDS.values():
            ...
            try:
                lst = getattr(self, v[0])
            except AttributeError:
                lst = []
                setattr(self, v[0], lst)
...
def format_message(pb: MessageType,
    ...
    def pformat_value(value: Any, indent: int) -> str:
        ...
        if isinstance(value, list):
            ...
            lines = ['[', level + ']']
            lines[1:1] = [leadin + pformat_value(x, indent + 1) + ','
                          for x in value]
            return '\n'.join(lines)
        if isinstance(value, dict):
            ...
            for key, val in sorted(value.items()):
                if val is None or val == []:
                    continue
                lines.append((leadin + key + ': ' +
                              pformat_value(val, indent + 1) + ','))
            lines.append(level + '}')
            return '\n'.join(lines)
