...
class Transport(object):
    ...
    def find_by_path(cls, path, prefix_search=False):
        ...
        raise TransportException(('{} device not found: '
                                  '{}').format(cls.PATH_PREFIX, path))


def all_transports() -> Iterable[Type[Transport]]:
    transports = set()  # type: Set[Type[Transport]]
    for modname in ('bridge', ):
        try:
            # Import the module and find the Transport class.
            # To avoid iterating over every item, the module
            # should assign its Transport classto a constant
            # named TRANSPORT.
...
def enumerate_devices() -> Iterable[Transport]:
    ...
    for transport in all_transports():
        try:
            ...
            LOG.info(('Enumerating {}: found {} '
                      'devices').format(transport.__name__, len(found)))
            devices.extend(found)
        except NotImplementedError:
            LOG.error(('{} does not implement '
                       'device enumeration').format(transport.__name__))
        except Exception as e:
            LOG.error(('Failed to enumerate {}. {}: '
                       '{}').format(transport.__name__,
...
def get_transport(path: str = None, prefix_search: bool = False) -> Transport:
    ...
    LOG.info(('looking for device by {}: '
              '{}').format('prefix' if prefix_search else 'full path', path))
    transports = [t for t in all_transports()
