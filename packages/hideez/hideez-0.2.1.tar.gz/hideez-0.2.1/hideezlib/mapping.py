...
def build_map():
    for msg_name in dir(messages.MessageType):
        ...
        try:
            ...
            raise ValueError("Implementation of protobuf message "
                             "'%s' is missing" % msg_name)

        if msg_class.MESSAGE_WIRE_TYPE != getattr(messages.MessageType,
                                                  msg_name):
            raise ValueError("Inconsistent wire type and MessageType "
                             "record for '%s'" % msg_class)
...
def register_message(msg_class):
    if msg_class.MESSAGE_WIRE_TYPE in map_type_to_class:
        raise Exception("Message for wire type "
                        "%s is already registered by %s" %
                        (msg_class.MESSAGE_WIRE_TYPE,
                         get_class(msg_class.MESSAGE_WIRE_TYPE)))
