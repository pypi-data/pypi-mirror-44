...
def startServer(models, allowclose=True, showbrowser=True, debug=False):
    ...
    pid = str(os.getpid())
    pidfile = os.path.join(os.path.dirname(__file__), "geodynserver.pid")
    ...
    # Add file handler for each model that link to the directory of raw files for their export
    for model_name, model in models.items():
        handlers.append(url('/'+model_name+'/(.*)', StaticFileHandler, {'path': model.dataDirectory}))
