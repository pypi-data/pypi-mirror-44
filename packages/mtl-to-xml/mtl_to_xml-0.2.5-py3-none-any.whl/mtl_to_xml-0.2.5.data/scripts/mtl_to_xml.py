...
class MTLConverter:
    ...
    def parse_args(self):
        parser = argparse.ArgumentParser(
            description='Choose target mode \'-t\','
                        ' directory mode \'-d\','
                        ' or scan mode \'-s\' mode to convert Landsat MTL to XML.')
        ...
        if len(sys.argv) == 1:
            print('Type mtl_to_xml -h for help options')
        # Target mode. Converts target file from MTL to XML.
        elif args.target:
            self.target_mode(args.target)
    ...
    # FIXME: XML is properly structured, but the first Element in the tree doesnt pretty print correctly.
