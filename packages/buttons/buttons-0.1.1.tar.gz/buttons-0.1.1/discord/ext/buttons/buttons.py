...
class Session:
    ...
    async def _session_loop(self, ctx):
        while True:
            _add = asyncio.ensure_future(ctx.bot.wait_for('raw_reaction_add',
                                                          timeout=self.timeout,
                                                          check=lambda _: self.check(_)(ctx)))
            _remove = asyncio.ensure_future(ctx.bot.wait_for('raw_reaction_remove',
                                                             timeout=self.timeout,
                                                             check=lambda _: self.check(_)(ctx)))
            try:
                done, pending = await asyncio.wait((_add, _remove), return_when=asyncio.FIRST_COMPLETED)
            ...
            if button in self._defaults.values():
                await button._callback(ctx)
                continue

            if action and button._callback:
