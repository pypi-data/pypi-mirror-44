...
from .prioritydict import ButtonsDict
...
class Paginator(Session):
    ...
    async def _session(self, ctx):
        if self.use_defaults:
            self.buttons = {**self._defaults, **self._buttons.mapping()}
        else:
            self.buttons = self._buttons
...
def button(emoji: str, *, position: int=0):
    ...
    emoji: str
        ...
        for custom emojis.
    position: int
        The position to inject the button into. This i
