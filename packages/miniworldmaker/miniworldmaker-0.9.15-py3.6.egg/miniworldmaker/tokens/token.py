...
class Token(pygame.sprite.DirtySprite):
    ...
    def image_action(self, attribute: str, value: bool):
        try:
            if attribute not in ImageRenderer.actions:
                raise ValueError("Error in image_action. Action '{0}' not in actions list: {1}".format(attribute, ImageRenderer.actions))
    ...
    def rect(self):
        try:
            return self.board.deprecated_rect_to_position(self.position, self.image.get_rect())
