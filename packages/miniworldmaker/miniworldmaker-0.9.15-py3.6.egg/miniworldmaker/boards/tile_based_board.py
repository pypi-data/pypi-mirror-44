...
from miniworldmaker.actors.token import Token
...
class TileBasedBoard(Board):
    ...
    def get_tokens_in_area(self, value: Union[pygame.Rect, tuple]) -> list:
        self._dynamic_actors_dict.clear()
        self._update_actors_positions()
        if type(value) == tuple:
            x, y = value[0], value[1]
    ...
    def remove_from_board(self, actor: Token) -> None:
    ...
    def add_to_board(self, token: Token, board_position: tuple = None) -> Token:
        if token.is_static:
            self._static_actors_dict[(board_position[0], board_position[1])].append(token)
        else:
