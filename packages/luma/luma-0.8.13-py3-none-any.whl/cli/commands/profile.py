...
def refresh_token(profile):
    ...
    profile_env = config_data.get('envs', {}).get(config_data.get('profiles', {}).get(profile).get('env'))
    profile_env['profileName'] = profile

    util = pyke.Util(context=profile_env, profile=profile)
    util.login()
