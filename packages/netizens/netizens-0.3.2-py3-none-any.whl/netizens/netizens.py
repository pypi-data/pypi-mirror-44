...
from netizensapi.netizensnoderpc import NetizensNodeRPC
from netizensbase import operations
from netizensbase.account import PublicKey
...
class Netizens(AbstractGrapheneChain):
    """ Connect to the Netizens network.
        ...
        * **Wallet Database**: Here, the netizenslibs load the keys from the
          locally stored wallet SQLite database (see ``storage.py``).
          To use this mode, simply call ``Netizens()`` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to ``Netizens()``.
    ...
    def define_classes(self):
        ...
        self.rpc_class = NetizensNodeRPC
        self.default_key_store_app_name = "netizens"
    ...
    def create_account(
        ...
        **kwargs
        ...
        """ Create new account on Netizens

            The brainkey/password can be used to recover all generated keys
            (see `netizensbase.account` for more details.
        ...
        from netizensbase.account import PasswordKey, PublicKey
    ...
    def set_proxy(self, proxy_account, account=None, **kwargs):
        """ Set a specific proxy for account
            ...
            :param netizens.account.Account proxy_account: Account to be
                    proxied
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
    ...
    def vesting_balance_withdraw(self, vesting_id, amount=None, account=None, **kwargs):
        """ Withdraw vesting balance
            ...
            :param netizens.amount.Amount Amount: to withdraw ("all" if not
                provided")
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
    ...
    def publish_price_feed(
        ...
        """ Publish a price feed for a market-pegged asset
            ...
            :param netizens.price.Price settlement_price: Price for settlement
            :param netizens.price.Price cer: Core exchange Rate (default
                ``settlement_price + 5%``)
        ...
        ), "settlement_price needs to be instance of `netizens.price.Price`!"
        assert cer is None or isinstance(
            cer, Price
        ), "cer needs to be instance of `netizens.price.Price`!"
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
    ...
    def update_cer(self, symbol, cer, account=None):
        """ Update the Core Exchange Rate (CER) of an asset
            ...
            :param str symbol: Symbol of the asset to publish feed for
            :param netizens.price.Price cer: Core exchange Rate
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
    ...
    def reserve(self, amount, account=None, **kwargs):
        """ Reserve/Burn an amount of this shares
            ...
            :param netizens.amount.Amount amount: The amount to be burned.
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        ...
        **kwargs
        ...
        """ Create a worker
            ...
            **Required**
            ...
            :param netizens.amount.Amount daily_pay: The amount to be paid
                daily
            ...
            **Optional**
            ...
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        from netizensbase.transactions import timeformat
    ...
    def invoice_create(self, customer,  amount_init=0, memo="", merchant_order_id = "", account=None, **kwargs):
        from .memo import Memo
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        amount = Amount(amount_init, "1.3.0", blockchain_instance=self)
        print(amount)
        merchant = Account(account, blockchain_instance=self)
        customer = Account(customer, blockchain_instance=self)

        memoObj = Memo(from_account=account, to_account=customer, blockchain_instance=self)

        op = operations.Invoice_create(
            **{
                "fee": {"amount": 0, "asset_id": "1.3.0"},
                "merchant": merchant["id"],
                "customer": customer["id"],
                "amount": {"amount": int(amount), "asset_id": "1.3.0"},
                "merchant_order_id": merchant_order_id,
                "memo": memoObj.encrypt(memo),
            }
        )
        print(op)
        return self.finalizeOp(op, merchant, "active", **kwargs)