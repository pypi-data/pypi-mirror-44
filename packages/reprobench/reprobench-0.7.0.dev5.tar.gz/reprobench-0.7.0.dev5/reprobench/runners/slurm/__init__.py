...
class SlurmRunner(Runner):
    ...
    def spawn_server(self, time_limit):
        ...
        server_cmd = f"{sys.exec_prefix}/bin/reprobench server --database={self.db_path} --port={self.port}"
        server_submit_cmd = [
            "sbatch",
            "--parsable",
            f"--time={time_limit}",
            f"--job-name=\"{self.config['title']}-benchmark-server\"",
            f"--output={self.output_dir}/slurm-server.out" f'--wrap="{server_cmd}"',
        ]
        ...
        server_host = get_nodelist(f"{server_job}.batch")
        logger.info(f"Server spawned at {server_host}")

        return server_host

    def spawn_workers(self, server_host, time_limit, mem_limit):
        logger.info("Spawning workers...")
        worker_cmd = f"{sys.exec_prefix}/bin/reprobench worker --host={server_host} --port={self.port}"
        worker_submit_cmd = [
            "sbatch",
            "--parsable",
    ...
    def run(self):
        ...
        mem_limit = 128 + limits["memory"]

        server_host = self.spawn_server(time_limit)
        worker_job = self.spawn_workers(server_host, time_limit, mem_limit)
