...
def local_runner(output_dir, resume, config, server):
    config_text = config.read()
    config = strictyaml.load(config_text, schema=schema).data
    runner = LocalRunner(
        config, output_dir=output_dir, resume=resume, server_address=server
    )
    runner.run()
...
@click.argument("config", type=click.File("r"))
def slurm_runner(output_dir, python_path, server, **kwargs):
