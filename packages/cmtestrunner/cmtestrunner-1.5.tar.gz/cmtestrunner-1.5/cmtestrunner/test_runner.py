...
from .middleware import (request_response_formatter, get_all_locales,
                                   ...
                                   unit_test_formatter, generate_failed_test_report, fail_log)
...
class TestRunner(TestCase):
    ...
    def verify_test_result(self, exp_response, test_id, accept_lang):
        ...
        for key, value in exp_response.items():
            ...
            # try:
            if re.match(r'snapshot\((.*\.json)\)', value):
                jsonFile = re.match(r'snapshot\((.*\.json)\)', value)[1]
                if not os.path.isfile(
                    settings.TEST_DATA_PATH + 'snapshots/' + jsonFile):
                    f = open(settings.TEST_DATA_PATH + 'snapshots/' +
                                      jsonFile, 'w')
                    json.dump(self.response.get(key), f, indent=4)
