...
def get_job_id_from_name(name):
    cmd = "%s/bin/hadoop job -list 2>/dev/null | grep %s | awk '{print $1}'" % (service_config.GET_CONF("hadoop", "hadoop_home"), name)
...
def get_job_progress_from_name(name):
    cmd = "%s/bin/hadoop job -list 2>/dev/null | grep %s | awk '{print $1}' | xargs %s/bin/hadoop job -status 2>/dev/null | grep complet" % (
        service_config.GET_CONF("hadoop", "hadoop_home"),
        name,
        service_config.GET_CONF("hadoop", "hadoop_home")
    )
    res = os.popen(cmd).read()
...
def hrm(dfs_path):
    cmd = "%s/bin/hadoop fs -rm -r -f %s" % (
        service_config.GET_CONF("hadoop", "hadoop_home"),
        service_config.GET_CONF("hadoop", "dfs_path_prefix") + dfs_path
    )
...
def hput(local_path, dfs_path):
    cmd = "%s/bin/hadoop fs -put -f %s %s" % (service_config.GET_CONF("hadoop", "hadoop_home"), local_path, dfs_path)
...
def hmkdir(dfs_path, recursive=True):
    ...
    if recursive:
        cmd_fmt += '-p'
    cmd_fmt += ' %s'
    cmd = cmd_fmt % (service_config.GET_CONF("hadoop", "hadoop_home"), dfs_path)
...
def hcat(dfs_path_file):
    cmd = "%s/bin/hadoop fs -cat %s" % (service_config.GET_CONF("hadoop", "hadoop_home"), dfs_path_file)
...
def getTrackPaths(taskId):
    ...
    track_path_list = []
    cmd = "%s/bin/hadoop fs -ls %s/track | awk '{print $8}' | cat -A" % (service_config.GET_CONF("hadoop", "hadoop_home"), task_path)
