...
from bot.base import Cfg, Dotted, OutputCache, cdir, cfg, get_type
...
def daemon():
    ...
    if not os.path.exists(cfg.logdir):
        cdir(bot.cfg.logdir)
    path = os.path.join(cfg.logdir, "pidfile")
    f = open(path, 'w')
...
def level(loglevel, logfile="bot.log", nostream=False):
    if not os.path.exists(cfg.logdir):
        cdir(cfg.logdir)
    logfile = os.path.join(cfg.logdir, logfile)
    datefmt = '%H:%M:%S'
...
def parse_cli(name="bot", options=None, usage="", version="", wd=""):
    ...
    cfg.update(vars(opt))
    cfg.logdir = os.path.abspath(os.path.join(os.path.expanduser("~"), ".logs"))
    cfg.name = name
    cfg.version = version
    cfg.args = arguments
    if not cfg.workdir and wd:
        cfg.workdir = wd
    if not cfg.workdir:
        cfg.workdir = os.path.abspath(os.path.join(os.path.expanduser("~"), ".%s" % cfg.name))
