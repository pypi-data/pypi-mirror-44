...
from bot import cfg
from bot.base import get_type, sliced, __txt__
from bot.kernel import kernel
from bot.tasks import launch
from bot.utils import locked
...
class IRC(bot.Bot):
    ...
    def connect(self):
        ...
        if self.cfg.ssl:
            ...
            self._sock = oldsock
        self.resume.fd = self._sock.fileno()
    ...
    def start(self):
        last = store.last(get_type(self.cfg))
        if last:
            self.cfg.update(last)
        prev = bot.Cfg(self.cfg)
