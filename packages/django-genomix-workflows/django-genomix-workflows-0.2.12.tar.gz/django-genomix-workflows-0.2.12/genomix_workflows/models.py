...
class WorkflowInstance(TimeStampedModel):
    ...
    def current_status(self):
        ...
        r = self.task_instances.filter(status__in=[pending, running])

        # sometimes there is no running task due to other related workflows's status
...
class Reference(TimeStampedModel):
    ...
    """
    workflow_category = models.CharField(max_length=100)
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    object_id = models.PositiveIntegerField(db_index=True)
    content_object = GenericForeignKey('content_type', 'object_id')
