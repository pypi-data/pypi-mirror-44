...
def task_instance_pre_save(sender, instance, **kwargs):
    ...
    pending = getattr(choices.STATUS, 'PENDING')
    running = getattr(choices.STATUS, 'RUNNING')
    ...
    if instance.pk is not None:
        ...
        # Only allow change to complete/running when all previous tasks are complete
        if old_instance.status is None and instance.status in (complete, pending, running, skip,):
        ...
        if old_instance.status in (complete, skip) and \
            ...
            for task in related_next_instances:
                if task.status in (complete, running,):
                    raise Exception('Cannot roll back a Task if any of the next tasks have already been initiated')
            for task in related_next_instances:
