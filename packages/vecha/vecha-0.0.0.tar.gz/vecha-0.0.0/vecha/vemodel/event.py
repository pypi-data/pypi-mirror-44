...
class EventDecoder(object):
    ...
    def __init__(self, *event_abi_list):
        super(EventDecoder, self).__init__()

        self.event_data = {}

        for abi in event_abi_list:
    ...
    def decode_event(self, log):
        ...
        if not len(log_topics) or log_topics[0] not in self.event_data:
            print(ValueError('Unknown log type'))
            return None
        ...
        result = {}
        for arg, type_, indexed in zip(event['args'], event['types'], event['indexed']):
            if indexed:
        ...
        meta = log.get('meta', None)
        return Event(event['name'], result, 0 if not meta else meat.get('blockNumber', 0))
