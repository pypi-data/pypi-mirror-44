...
class IpWebDB:
    ...
    def get_location(self, ip_address):
        ...
        try:
            ...
            self.__db_data = DbIpWeb.get(ip_address)
            if self.__db_data.latitude is None or self.__db_data.longitude is None:
                raise InvalidResponseError
            if self.__db_data.latitude == 0 or self.__db_data.longitude == 0:
                raise InvalidResponseError
            ...
            logger.error("Database DbIpWeb returned %s ", str(exception.__class__))
