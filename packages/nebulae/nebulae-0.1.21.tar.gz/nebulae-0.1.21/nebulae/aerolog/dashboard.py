...
class DashBoard(object):
    ...
    def _log(self, guage=True, tachograph=True):
        if guage:
            boards = {}
            # clustering
            ...
            for k in boards.keys():
                for i, b in enumerate(boards[k]):
                    ...
                    plt.savefig(path.join(self.param['log_path'], '%s_%s_%.3g_mile_%d.jpg'
                                          % (k, stage, self.gauge_mile[b][-1], self.trail_mile[b][-1])))
                    plt.close()
        ...
        if tachograph:
            for k in self.gauge_mile.keys():
                ...
                df.to_csv(path.join(self.param['log_path'], '%s_%s_mile.csv'%(metric, stage)),
                          header=None, index=None)
                df = pd.DataFrame(data={'A': self.trail_epoch[k], 'B': self.gauge_epoch[k][:-1]})
                df.to_csv(path.join(self.param['log_path'], '%s_%s_epoch.csv' % (metric, stage)),
                          header=None, index=None)