...
class FuelDepot(object):
    ...
    def loadFuel(self, config=None, name=None, batch_size=None, if_shuffle=True,
                ...
                channel=3, frame=0, resol_ratio=1, complete_last_batch=True,
    ...
    def unloadFuel(self, name=''):
        name_err = Exception('NEBULAE ERROR ⨷ %s is not an mounted fuel tank.' % name)
        if name != '' and (name not in self.tanks.keys()):
            raise name_err
        elif name == '':
