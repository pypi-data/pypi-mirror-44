...
def convert_entsoe_to_numpy(year, rdf_base_dir):
    ...
    nrows = len(COUNTRIES) * len(set(FLOW_MAPPING))
    ncols = len(COUNTRIES) * len(set(ACTIVITY_MAPPING))

    supply = np.zeros((nrows, ncols), dtype=np.float32)
    use = np.zeros((nrows, ncols), dtype=np.float32)

    sorted_flow_values = sorted(set(FLOW_MAPPING))
    sorted_activity_values = sorted(set(ACTIVITY_MAPPING))
    ...
    for technology, country, amount in iterate_generation(year):
        supply[
            row_mapping[(country, technology)],
            col_mapping[(country, technology)],
        ] += amount

    for technology, country, amount in iterate_generation(year):
        use[
            row_mapping[(country, technology)],
            col_mapping[(country, 'Grid')],
        ] += amount

    for from_, to_, amount in iterate_trade(year):
        use[
            row_mapping[(from_, 'Grid')],
            col_mapping[(to_, 'Grid')],
        ] += amount
...
def convert_entsoe_to_numpy_exiobase(year, rdf_base_dir):
    metadata = get_metadata(rdf_base_dir)

    # Use mapping to exiobase
