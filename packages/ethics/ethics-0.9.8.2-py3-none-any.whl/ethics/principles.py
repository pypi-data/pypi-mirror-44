...
class Principle(object):
    ...
    def explain(self):
        pd = self.permissible()
        f = self.buildConjunction()
        reasons = generateReasons(f, pd, self.model, self)
        ...
        return {"permissible": pd, "sufficient": suff, "necessary": nec, "inus": inus}
...
class DoNoHarmPrinciple(Principle):
    ...
    def _check(self):
        ...
        f = None        
        for c in self.model.consequenceLiterals:
            if f == None:
                f = self.getSymbol(Impl(Causes(self.model.action, c), GEq(U(c), 0)))
            else:
                f = And(self.getSymbol(Impl(Causes(self.model.action, c), GEq(U(c), 0))), f)
        self.formulae = [f]
        self.result = [self.model.models(self.mapSymbolToFormula(f))]
        return self.result
...
class DoNoInstrumentalHarmPrinciple(Principle):
    ...
    def _check(self):
        f = None
        for c1 in self.model.consequenceLiterals:
            for c2 in self.model.consequenceLiterals:
                if f == None:
                    f = self.getSymbol(Impl(And(Causes(self.model.action, c1), And(Causes(c1, c2), I(c2))), GEq(U(c1), 0)))
                else:
                    f = And(self.getSymbol(Impl(And(Causes(self.model.action, c1), And(Causes(c1, c2), I(c2))), GEq(U(c1), 0))), f)
        self.formulae = [f]
        self.result = [self.model.models(self.mapSymbolToFormula(f))]
        return self.result
...
class DeontologicalPrinciple(Principle):
    ...
    def _check(self):
        f = self.getSymbol(GEq(U(self.model.action), 0))
        self.formulae = [f]
        self.result = [self.model.models(self.mapSymbolToFormula(f))]
        return self.result
