...
def test_est97_to_wgs84(l_est97, expected_wgs97):
    tolerance = 0.0000000001  # 10 ** (-10)
    actual_wgs97 = CoordinateConverter.l_est97_to_wgs84(l_est97)
    assert actual_wgs97.long == pytest.approx(expected_wgs97.long, tolerance)
    assert actual_wgs97.lat == pytest.approx(expected_wgs97.lat, tolerance)
...
def test_wgs84_to_est97(wgs97, expected_l_est97):
    ...
    assert actual_est97.x == pytest.approx(expected_l_est97.x, tolerance)
    assert actual_est97.y == pytest.approx(expected_l_est97.y, tolerance)
