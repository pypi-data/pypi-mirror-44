...
class Podcast:
    ...
    def iframes(self):
        ...
        def gen_title(num):
            log.info('Title not found. Generating title...')
        ...
        if self.htmlsoup.find('iframe') != None:
            ...
            for item in self.htmlsoup.findAll('iframe'):
                ...
                if item.has_attr('title'):
                    log.info('Inline title found...')
                    title = item['title']

                elif item.find_previous_sibling():
                    parent = item.find_previous_sibling()
                    if parent.find('title'):
                        log.info('Title found in parent...')
                        title = item['title']
                    else:
                        title = gen_title(i)
                        i += 1
        ...
        for podcast in self.mp3list:
            ...
            for file in filelist:
                if podcast['title'].replace(' ', '_').replace('/', '_') == file:
                    exists = True
                    log.info('%s already exists. Skipping...' % file)
                if exists == True:
                    break
