...
def generate_python_structure(process_name, input_f):
    ...
    for line in input_f.readlines():
        ...
        # Comprovem si el camp anterior era una classe
        if prev_field and (number == 1 or not number) and last_number > 0:
            parents.append(prev_field)
            partents_number.append(last_number)
            parent_added_check = True
            if not python_structure.get(prev_field):
                python_structure[prev_field] = []
        ...
        if last_number != number-1 and not parent_added_check and len(parents) != 1:
            parents.pop(-1)
            partents_number.pop(-1)
...
def get_field_data(field, line):
    ...
    if len(data) > 1 and data[1] not in ["S", "N"]:
        desc = data[1]
    else:
        desc = "Descripción no encontrada. " + field
...
data = generate_input_python_code("A1_46", "info", inherit_proces="A1_48")
generate_input_test_files_main("A1_46", data)
generate_erp_models_main("A1_46", data)
