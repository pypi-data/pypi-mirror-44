...
# Defining words
keywords = ['string', 'float', 'int', 'hash']
                ...
                if content[ien] == '];':
                            ...
                            for identifer, definer in enumerate(content[i : ]):
                                ...
                                if definer[-1:] == '}':
                                    ...
                                    if define[0].lower() in keywords:
                                        ...
                                        # If so, then checks if the second (after dot) is in string words
                                    elif define[1].lower() in hashwords:
                                            # Checks if the second word is equal to eve
                                            if define[1].lower() == 'eve':
                                                # If so, then use a custom hash method for the string
                                                value = value
                                            # Checks if the second word is equal to med5
                                            elif define[1].lower() == 'md5':
                                                # If so, then hash it with md5
                                                value = str(md5(value.encode()).hexdigest()))
                    ...
                    for i, line in enumerate(insides):
                        ...
                        # Check if the line starts with a '
                            ...
                            # Remove the '' around the parent/name and value
...
# The add method :: Adds a default variable to the given file
def add (path, parent, value):
    # Opens the given file
    file = open(path, 'r')
    # Assigns the content variable to the lines
    content = file.readlines()
    # Close the file
    file.close()

    # Sets a text variable to what it should write
    toWrite = "\n  '{0}' :: {1}\n\n".format(parent, value)
    # Sets the last line before the close tags to that variable
    content[(len(content) - 1) - 1] = toWrite

    # Opens the given file again
    file = open(path, 'w')
    # Now writes the modified content to the file
