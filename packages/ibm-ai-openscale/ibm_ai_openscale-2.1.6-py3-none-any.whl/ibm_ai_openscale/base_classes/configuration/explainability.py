...
class Explainability(TableFromRestApiViewer):
    ...
    def enable(self, training_data_statistics=None, training_data=None, **kwargs):
        ...
        validate_asset_properties(asset_properties,
                                  ['input_data_type'])
        input_data_type = asset_properties['input_data_type']
        params = {}

        if training_data_statistics is not None:
            validate_meta_prop(training_data_statistics,
                               'explainability_configuration', dict, True)
            validate_asset_properties(asset_properties, ['output_data_schema'])
            ...
            subscription_details = self._subscription.get_details()
            asset_properties = subscription_details['entity']['asset_properties']
            validate_asset_properties(asset_properties, ['output_data_schema', 'feature_fields',
            ...
            training_data_statistics_4_explainability = explain_config.get(
            ...
            if input_data_type == InputDataType.STRUCTURED:
                if 'training_data_reference' not in asset_properties.keys():
                    raise MissingMetaProp(
                        'training_data_reference or training_data_statistics')

                validate_asset_properties(
                    asset_properties, ['output_data_schema', 'feature_fields', 'label_column'])
