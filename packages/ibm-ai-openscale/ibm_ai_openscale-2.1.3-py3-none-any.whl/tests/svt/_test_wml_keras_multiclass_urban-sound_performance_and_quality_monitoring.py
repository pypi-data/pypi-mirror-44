...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    model_path = os.path.join(os.getcwd(), 'artifacts', 'urban-sound', 'urban-sound-model.h5.tgz')
    ...
    def test_05_prepare_deployment(self):
        model_name = "AIOS Keras Urban Sound model"
        deployment_name = "AIOS Keras Urban Sound deployment"
    ...
    def test_06_subscribe(self):
        subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.add(
            WatsonMachineLearningAsset(
                ...
                input_data_type=InputDataType.UNSTRUCTURED_AUDIO,
                label_column='label'
            )
        )
    ...
    def test_08_select_asset_and_get_details(self):
        TestAIOpenScaleClient.subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.get(self.model_uid)
    ...
    def test_13_score(self):
        ...
        def load_audio_from_file(path):
            audio_data, sr = librosa.core.load(path=path, sr=16000)
            return audio_data

        test_filename = "datasets/urban-sound/5.wav"
        audio_data = load_audio_from_file(test_filename)
        mfccs = np.mean(librosa.feature.mfcc(y=audio_data, sr=16000, n_mfcc=40).T, axis=0).tolist()
        df2 = pd.DataFrame([[mfccs]], columns=['feature'])

        scoring_payload = {'values': [df2.feature.tolist()[0], df2.feature.tolist()[0]]}

        TestAIOpenScaleClient.scoring_records = 16
    ...
    def test_16_feedback_logging(self):
        ...
        test_filename = "datasets/urban-sound/7.wav"
        audio_data = load_audio_from_file(test_filename)
        mfccs = np.mean(librosa.feature.mfcc(y=audio_data, sr=16000, n_mfcc=40).T, axis=0).tolist()
        df2 = pd.DataFrame([[mfccs]], columns=['feature'])

        scoring_payload = {'values': df2.feature.tolist()}

        feedback_payload = df2.feature.tolist()
        feedback_payload = str(feedback_payload[0])
        feedback_payload += ";[0]"
