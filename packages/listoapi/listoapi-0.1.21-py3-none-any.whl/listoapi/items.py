...
class Items(ListoAPI):
    ...
    def update_fields(self, item_type, iid, new_data):
        """Update field of item

        kwargs:
        - item_type: name of Item
        - id: id of item to modify
        - new_data: json with key, value pairs of new item data
        """
        return self.make_request(
            method="POST", path="/items/%s/%s/update_fields/" % (item_type, iid),
            json=new_data).json()
