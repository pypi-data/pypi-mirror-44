...
std::shuffle_order_engine<std::discard_block_engine<std::mt19937_64, 12, 10>, 256> hurricane { hardware_seed() };

void set_seed(unsigned long long seed) {
    if (seed == 0) {
        std::shuffle_order_engine<std::discard_block_engine<std::mt19937_64, 12, 10>, 256> dynamic_engine { hardware_seed() };
        hurricane = dynamic_engine;
    }
    else {
        std::shuffle_order_engine<std::discard_block_engine<std::mt19937_64, 12, 10>, 256> static_engine { seed };
        hurricane = static_engine;
    }
}
