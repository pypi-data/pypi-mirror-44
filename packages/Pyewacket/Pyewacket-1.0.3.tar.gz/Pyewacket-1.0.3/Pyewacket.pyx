...
__all__ = (
    ...
    "choices", "cumulative_weighted_choice", "distribution_timer", "timer", "quick_test",
    "knuth", "fisher_yates", "set_seed"
)
...
cdef extern from "Pyewacket.hpp":
    ...
    long long _randbelow                "Pyewacket::randbelow"(long long)
    long long _randint                  "Pyewacket::randint"(long long, long long)
    long long _randrange                "Pyewacket::randrange"(long long, long long, long long)
    double _uniform                     "Pyewacket::uniform"(double, double)
...
def quick_test():
    ...
    seed_test_size = 100
    software_seed = 2**63 - 1
    arr1 = [i for i in range(seed_test_size)]
    set_seed(software_seed)
    shuffle(arr1)
    arr2 = [i for i in range(seed_test_size)]
    set_seed(software_seed)
    shuffle(arr2)
    print("Software seed test passed:", arr1 == arr2)
    hardware_seed = 0
    arr3 = [i for i in range(seed_test_size)]
    set_seed(hardware_seed)
    shuffle(arr3)
    arr4 = [i for i in range(seed_test_size)]
    set_seed(hardware_seed)
    shuffle(arr4)
    print("Hardware seed test passed:", arr3 != arr4)
    print()
