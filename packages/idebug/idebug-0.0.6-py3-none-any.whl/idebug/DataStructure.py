...
def eachcolumn_unqvalue_count(df, grouping_impossible_cols=['_id'], shown_glen=10):
    ...
    df = df.fillna('_None')
    loop = performance.LoopReporter(title=inspect.stack()[0][3], len=len(cols))
    for col in cols:
...
def obj(object, title='_None', selective=True):
    ...
    if selective:
        ...
        attrs = dir(object)
        for attr in attrs:
            value = object.__getattribute__(attr)
            """
            if isinstance(value, object):
                subattrs = dir(attr)
                for subattr in subattrs:
                    subvalue = attr.__getattribute__(subattr)
                    print(f"\n {attr}.{subattr} : {subvalue}")
            else:
            """
            print(f"\n {attr} : {value}")
    ...
    df = df.reindex(columns=['key','type','bytes','value'])
    dbg.dframe(df, "object.__dict__ 인자의 속성검사.")
