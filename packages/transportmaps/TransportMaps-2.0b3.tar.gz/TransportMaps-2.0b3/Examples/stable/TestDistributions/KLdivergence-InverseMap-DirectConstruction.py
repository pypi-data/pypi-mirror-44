...
def usage():
    print('python KLdivergence-InverseMap-DirectConstruction.py -t <test_number> -s <span_type> -q <quad_type> -n <quad_n_points> [--btype=<basis_type> --tol=<tolerance> --max-it=<max_iterations> --with-reg=<alpha> --ord-list=<comma_sep_order_list> --no-plotting --no-l2 --nprocs=<nprocs>]')
...
def print_avail_btype():
    print('Available <basis_type>: %s' % AVAIL_BTYPE)
...
def print_example():
    print('python KLdivergence-InverseMap-DirectConstruction.py -t 2 -s total -q 3 -n 30 --tol=1e-4 --with-reg=1e-3 --ord-list=1,3,5,7,9')
...
try:
    ...
    for i_ord,order in enumerate(ORDERS):
        ...
        tm_approx = TM.Default_IsotropicIntegratedExponentialTriangularTransportMap(
            setup['dim'], order, SPAN, btype=BTYPE)
