...
def nob_del(obj_, *keys, verbose=False):
    ...
    def rec_delete_items(nob_in, curr_addr=None):
        ...
        if curr_addr is not None:
            pass
            #print(">", '/'.join([str(i) for i in curr_addr]))
        else:
            curr_addr = list()
...
def nob_find_unique(obj_, *keys):
    ...
    if not matchlist:
        raise NobReferenceError("No match for keys -" + keys_str + "-")
    elif len(matchlist) > 1:
        match_err = [str_address(match) for match in matchlist]
        raise NobReferenceError("Multiple match for key -" + keys_str + "-\n"
                                + "\n".join(match_err))
    else:
        return matchlist[0]
...
def nob_filter_boolean(nob):
    """Return a nested object without booleans."""

    print('Using this function nob_filter_boolean is UGLY, period!')
    def rec_filter_boolean(nob_in):
        nob_out = None
        if isinstance(nob_in, dict):
            nob_out = dict()
            for key in nob_in:
                nob_out[key] = rec_filter_boolean(nob_in[key])
        elif isinstance(nob_in, list):
            nob_out = list()
            for list_item in nob_in:
                nob_out.append(rec_filter_boolean(list_item))
        elif isinstance(nob_in, bool):
            if nob_in is True:
                nob_out = "yes"
            else:
                nob_out = "no"
             ...
             nob_out = nob_in
        return nob_out
    nob_out = rec_filter_boolean(nob)
    return nob_out
