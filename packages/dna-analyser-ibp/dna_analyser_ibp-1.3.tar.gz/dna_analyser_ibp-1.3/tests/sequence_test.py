...
def test_sequence_file_uploading_creation_and_deleting(user):
    ...
    factory = FileSequenceFactory(
        user=user,
        circular=True,
        file_path="/home/sephyx/Git/DNA_analyser_IBP/tests/sequence.txt",
...
@pytest.mark.parametrize(["_len", "pos"], [(-10, 10), (-20, -200), (20, -211)])
@vcr_instance.use_cassette
def test_loading_sequence_data(user, _len, pos):
    sq_lst = [se for se in seq_load_all(user, filter_tag=["Escherichia coli"])]

    with pytest.raises(ValueError):
        _ = seq_load_data(user=user, id=sq_lst[0].id, data_len=_len, pos=pos)
        time.sleep(1)
