...
def getfasta(
    ...
    command = [
        'bedtools',
        'getfasta',
        *short_arg(fi), *short_arg(bed), *short_arg(fo),
        flag_arg(name, short=True), flag_arg(tab, short=True),
        flag_arg(bedOut, short=True), flag_arg(s, short=True),
        flag_arg(split, short=True)
    ]
    return command
