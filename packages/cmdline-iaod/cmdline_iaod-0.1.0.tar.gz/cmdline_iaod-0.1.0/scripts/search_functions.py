...
parser.add_argument('-q', '--U12_search_query', 
...
parser.add_argument('-c', '--intron_class',
    help='U12-type or U2-type, as annotated by intronIC')
parser.add_argument('-p', '--phase', type='int', 
    help='0 if intron is between two codons, 1 if between first and second nt \
    of one codon, 2 if between second and third')
...
parser.add_argument('-chr', '--chromosome', help='chromosome containing intron')
parser.add_argument('-s', '--strand', choices=['+', '-'], 
    help="strand containing intron (+ or - usually, but maybe 1 or -1 \
...
large minority of U12-types are AT-AC, and others are generally quite rare. If\
 specifying multiple sets of terminal dinucleotides, separate them with commas.
Do not neglect the hyphen in the middle.')
