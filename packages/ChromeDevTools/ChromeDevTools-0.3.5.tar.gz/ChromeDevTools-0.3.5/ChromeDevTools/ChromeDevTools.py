...
from .utils import camelize, normalize_dict
...
class ChromeDevTools:
  ...
  def _create_tab(self, d: Mapping[str, Any]) -> 'Tab':
    d = normalize_dict(d)
    logger.debug(d)
    return Tab(self, **d)
...
class Tab:
  ...
  _id = 0
  _skipped = []
  # Только аннотации будут аргументами конструктора
  ...
  async def activate(self) -> str:
    r = await self.devtools.fetch(f'/json/activate/{self.id}')
    return await r.text()

  async def close(self) -> str:
    r = await self.devtools.fetch(f'/json/close/{self.id}')
    # disconnect?
    return await r.text()

  async def connect(self):
    logger.debug(f'connect to {self.web_socket_debugger_url}')
    self.ws = await self.devtools.connect_ws(self.web_socket_debugger_url)
