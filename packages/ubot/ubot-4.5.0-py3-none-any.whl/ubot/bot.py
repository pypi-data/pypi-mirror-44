...
class Bot:
    ...
    async def __handle_update(self, update):
        ...
        # find all the matching triggers
        matching_triggers = []
        ...
        # if any matching triggers is found execute it (or them, if a choose_trigger function has been provided)
        if matching_triggers:
            choose_trigger = self.choose_trigger
            if choose_trigger is not None:
                chosen_triggers = await await_or_call(choose_trigger, matching_triggers)
                for trigger in chosen_triggers:
                    self.loop.create_task(self.__execute_trigger(trigger, update, update_type, update_flavor))
            else:
                await self.__execute_trigger(matching_triggers[0], update, update_type, update_flavor)
