...
def exception_handler(module_name=''):
    def handler(fn):
        def wrapped(*args, **kwargs):
            ...
            except (ParameterTypeError, ParameterValueError) as ex:
                raise ex
...
def is_in_range(value, value_range: Range = Range(0, 0)):
    """
    检查值是否在某个范围内
    :param value:
...
def is_length_in_range(value, len_range: Range = Range(0, 0)):
    """
    检查字符串的长度是否在某个范围内
    :param value: 字符串
...
def is_correct_file_format(filename, format_list: list = []):
    """
    检查文件类型是否在所允许的类型列表内
...
def params_check(param_check_entries: list = []):
    """
    检查方法参数类型和值是否合法的修饰器
    ...
    def check(fn):
        ...
        def value_check(*params):
            ...
            :return:
            """
            def update_error_fields_if_is_invalid_value(param_value, entry: ParamCheckEntry):
