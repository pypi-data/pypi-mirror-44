...
def create_ttrace(station, virtualgates, vgates, scanrange, sweepgates, param={}):
    ...
    ttrace = ttrace_t({'markerperiod': 80e-6, 'fillperiod': fillperiod,
                       'period': param.get('period', 500e-6), 'alpha': .1})
    ttrace['period0'] = 250e-6
    ttrace['fpga_delay'] = 2e-6
    ttrace['traces'] = []
    ttrace['traces_volt_gate'] = []
    try:
...
def read_trace_m4i(station, ttrace_elements, read_ch=[1], Naverage=20, verbose=0,
    ...
    if verbose:
        print('measuresegment_m4i: processing data: data shape %s, memsize %s' %
              (data.shape, digitizer.data_memory_size()))
...
def parse_data(data_raw, ttraces, ttrace, clockbias=1, verbose=1):
