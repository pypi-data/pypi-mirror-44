...
class Control(object):
    ...
    def simu(
        ...
        batch_calc = kwargs.pop('batch_calc', False)
        ...
        # batch calculation
            ...
            self.lcoe[inval] = np.nan

        # print results
        if print_prog:
            t1 = time.time()
            print('Simulation completed in {:.4f} s.'.format(t1-t0))
        if print_out and not batch_calc:
            self.res_print()
    ...
    def _lcoe(dis, yr_proj, infl, proj_capex, proj_opex):
        ...
        cost_c = (
            ...
            dis.su.cost_c+proj_capex
        )
        cost_of = (
            dis.ld.cost_of+dis.im.cost_of+dis.st.cost_of +
            dis.dp.cost_of+dis.sk.cost_of+dis.gd.cost_of +
            dis.su.cost_of +
            proj_opex*np.sum(1/(1+infl)**np.arange(1, 1+yr_proj))
        )
        cost_ov = (
            dis.ld.cost_ov+dis.im.cost_ov+dis.st.cost_ov +
            dis.dp.cost_ov+dis.sk.cost_ov+dis.gd.cost_ov +
            dis.su.cost_ov
        )
        cost_ou = (
            dis.ld.cost_ou+dis.im.cost_ou+dis.st.cost_ou +
            dis.dp.cost_ou+dis.sk.cost_ou+dis.gd.cost_ou +
            dis.su.cost_ou
        )
        cost_r = (
            dis.ld.cost_r+dis.im.cost_r+dis.st.cost_r +
            dis.dp.cost_r+dis.sk.cost_r+dis.gd.cost_r +
            dis.su.cost_r
        )
        cost_f = (
            dis.ld.cost_f+dis.im.cost_f+dis.st.cost_f +
            dis.dp.cost_f+dis.sk.cost_f+dis.gd.cost_f +
            dis.su.cost_f
        )
        cost = (
