        ...
        def colored(self, record):
            ...
            for line in lines_:
                if first_line is None:  # single line
                    ...
                    lines.append("{color_on}{inverse_on}{level}{inverse_off}{color_on}{date}{color_off} {file_color_on}{filepart}:{color_off} {all_off}".format(
                        filepart=filepart, level=level, message=line, date=timestamp, **formatter))
                lines.append(
                    "{color_on}{inverse_on}{level_filler}{inverse_off}{color_off} {color_on}{message}{color_off}{background_off}{all_off}".format(
                        level_filler=level_filler, message=line, date=timestamp, date_filler=timestamp_filler, **formatter))
                first_line = False
            # end for
            return "\n".join(lines)
            # end def
