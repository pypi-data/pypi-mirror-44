...
import warnings
message_collections = "Using or importing the ABCs from 'collections' instead of from 'collections.abc' is deprecated, and in 3.8 it will stop working"
warnings.filterwarnings ("ignore", message = message_collections)
...
class Serial (Executor):
    ...
    def bootup (self, peers):
        ...
        if hasattr (self.task, 'executor'):
            self.taskport = self.task.executor.init (peers)
        else:
            self.taskport = None
    ...
    def resample (self, indices):
        ...
        self.worker_timing.start ('replicate')
        ...
        # 3. replicate particles according to 'keep_counters':
        ...
        for index, reindices in keep_counters.items ():
            if len (reindices) > 1:
                ...
                for reindex in reindices [1:]:
                    ...
                    self.ensemble.prepare (reindex)
                    self.ensemble.particles [reindex] .load (state)
                    copys += 1
        traffic ["copy"] = copys / len (indices)

        # 4. prepare all particles
        for index in self.ensemble.particles.keys ():
            self.ensemble.prepare (index)

        self.worker_timing.time ('replicate')
