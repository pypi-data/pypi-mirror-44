...
class Direct (Likelihood):
    ...
    def __call__ (self, parameters):
        ...
        for snapshot in self.data.index:
            ...
            predictions [snapshot] = self.executor.call ('run', args = [snapshot], flatten=1) [0]

            # compute estimate
            if self.verbosity >= 2:
                print("Computing estimate...")
            estimates [snapshot] = self.executor.call ('errors', args = [self.data.loc [snapshot]], flatten=1) [0]
