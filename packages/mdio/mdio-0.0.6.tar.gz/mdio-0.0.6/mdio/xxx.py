...
class Frame(object):
    ...
    """
    def __init__(self, crds, box=None, time=0.0, precision=1000, timestep=0):
        crds = np.array(crds)
        if len(crds.shape) != 2:
    ...
    if maxatoms <= 9:
        ...
        frame = Frame(crds[:natoms], box=box, time=time, timestep=timestep)
        return frame
        ...
        if run > 0:
            for k in range(0, run, 3):
                ...
                if (k == 0):
                    ...
                    thiscoord = prevcoord
                    prevcoord = tmp
                    crds[i] = prevcoord * inv_precision
                    i += 1
                else:
                    prevcoord = thiscoord
                crds[i] = thiscoord * inv_precision
                i += 1
        else:
            crds[i] = thiscoord * inv_precision
            i += 1
    ...
    frame = Frame(crds[:natoms], box=box, time=time, timestep=timestep, precision=precision)
    return frame
...
def xtc_open(filename, mode='r'):
    if not mode in ['r', 'w']:
        raise ValueError('Error: mode must be "r" or "w"')
    if mode == 'r':
        filehandler = XtcfileReader(filename)
    else:
...
class XtcfileReader(object):
    ...
    def read_frame(self, natoms=None):
        """
        Returns the next frame from the xtc file.

        """
        return read_frame(self.up, natoms=natoms)
