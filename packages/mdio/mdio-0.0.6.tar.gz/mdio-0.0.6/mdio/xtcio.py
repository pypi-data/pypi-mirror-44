...
import xdrlib
import numpy as np
from mdio.xtcutils import BitByteBuffer
import mdio.base 
...
def read_frame(up, selection=None):
    """
    ...
    if maxatoms <= 9:
        ...
        if selection is not None:
            frame = mdio.base.Frame(crds[selection], box=box, time=time, timestep=timestep)
        else: 
    ...
    while i < natoms:
        if bitsize == 0:
            thiscoord = np.zeros(3, dtype=np.int)
            for j in range(3):
                thiscoord[j]= bs.receiveint(bitsizeint[j])
        else:
            nums = bs.receiveints(bitsize, sizes)
            thiscoord = np.array(nums, dtype=np.int)
        thiscoord += minint
        prevcoord = thiscoord
        ...
        if run > 0:
            for k in range(0, run, 3):
                nums = bs.receiveints(ms.smallidx, ms.sizesmall)
                thiscoord = np.array(nums, dtype=np.int)
                thiscoord += prevcoord - ms.smallnum
                if (k == 0):
                    tmp = thiscoord
                    thiscoord = prevcoord
                    prevcoord = tmp
    ...
    if selection is None:
        ...
        frame = mdio.base.Frame(crds[selection] * inv_precision, box=box, time=time, timestep=timestep, precision=precision)
    return frame
...
def write_frame(p, frame):
    ...
    p.pack_int(MAGIC)
    p.pack_int(frame.natoms)
    p.pack_int(frame.timestep)
    p.pack_float(frame.time)
    if frame.box is None:
        p.pack_farray(9, np.zeros((9)), p.pack_float)
    else:
        p.pack_farray(9, frame.box.flatten(), p.pack_float)  
...
def xtc_open(filename, mode='r', selection=None):
    if not mode in ['r', 'w']:
        raise ValueError('Error: mode must be "r" or "w"')
    if mode == 'r':
        filehandler = XtcfileReader(filename, selection=selection)
    else:
...
class XtcfileReader(object):
    ...
    def read_frame(self, selection=None):
        ...
        return read_frame(self.up, selection=selection)

    def read(self, selection=None):
        """
        Returns all emaining frames fromthe xtc file as a Trajectory.
        """
        frames = []
...
class XtcfileWriter(object):
    ...
    def write_frame(self, frame):
        ...
        """
        self.p.reset()
        if frame.crds is None:
            raise ValueError('Error - no coordinates in frame')
        frame.natoms = len(frame.crds)
