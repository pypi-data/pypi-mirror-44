...
class DCDFileReader(object):
    def __init__(self, f, selection=None):
        self.f  = f
    ...
    def read_frame(self, selection=None):
        ...
        frame = mdio.base.Frame(crds, box, time=time, timestep=timestep)
        return frame
...
def dcd_open(filename, mode='r', selection=None):
    if not mode in ['r', 'w']:
        raise ValueError('Error - mode must be "r" or "w".')
    if mode == 'r':
        f = open(filename, 'rb')
        filehandler = DCDFileReader(f, selection=selection)
    elif mode == 'w':
        f = open(filename, 'wb')
