...
class Frame(object):
    ...
    def select(self, selection):
        ...
        """
        return Frame(self.crds[selection], self.box, self.time)
    ...
    def rmsd_from(self, frame):
        ...
        if frame.crds.shape != self.crds.shape:
            return ValueError("Error - reference structure has {} atoms but frame has {} atoms.".format(frame.crds.shape[0], self.crds.shape[0]))
        return rmsd.kabsch_rmsd(self.crds, frame.crds, translate=True)
    ...
    def fitted_to(self, frame, weights=None):
        ...
        crds = rmsd.kabsch_fit(self.crds, frame.crds, weights)
        return Frame(crds, None, self.time)
    ...
    def packed_around(self, centre_atom_index):
        ...
        dv = box_centre - self.crds[centre_atom_index]
        r = self.crds + dv
        f = np.matmul(B, r.T)
        g = f - np.floor(f)
        t = np.matmul(A, g)
        crds = t.T - dv
        return Frame(crds, self.box, self.time)
...
class Trajectory(object):
    ...
    def append(self, data):
        ...
        Data may be a single Frame, a single Trajectory, or a list of either.
.
        """
        if isinstance(data, Frame):
            frames = [data]
            ...
            if isinstance(data[0], Frame):
                frames = data
            elif isinstance(data[0], Trajectory):
                frames = data[0].frames()
                for t in data[1:]:
                    frames += t.framees()
            else:
                raise TypeError('Error - argument must be a frame, trajectory, or list of either.')
        else:
            raise TypeError('Error - argument must be a frame, trajectory, or list of either.')
        crds = []
        box = []
...
def load(filenames, selection=None):
    """
