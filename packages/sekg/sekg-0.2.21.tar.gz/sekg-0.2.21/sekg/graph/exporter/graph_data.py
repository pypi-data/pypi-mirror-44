...
class GraphData(gensim.utils.SaveLoad):
    ...
    def merge_two_nodes_by_id(self, main_node_id, merged_node_id, merge_properties=False):
        ...
        print(self.out_relation_map)
        print(self.in_relation_map)
        print(self.relation_type_to_relation_json_map)
        ...
        if self.DEFAULT_KEY_PROPERTY_ALIAS not in new_node_properties:
            qualified_name = new_node_properties.get(self.DEFAULT_KEY_PROPERTY_QUALIFIED_NAME, "")
            new_node_properties[self.DEFAULT_KEY_PROPERTY_ALIAS] = set()
            if qualified_name != "":
                new_node_properties[self.DEFAULT_KEY_PROPERTY_ALIAS].add(qualified_name)
        qualified_name = merged_node_json[self.DEFAULT_KEY_NODE_PROPERTIES].get(self.DEFAULT_KEY_PROPERTY_QUALIFIED_NAME, "")
        if qualified_name != "":
            new_node_properties[self.DEFAULT_KEY_PROPERTY_ALIAS].add(qualified_name)
        if merge_properties:
