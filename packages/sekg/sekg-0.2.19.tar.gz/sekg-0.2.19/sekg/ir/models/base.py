...
class DocumentSimModel(metaclass=abc.ABCMeta):
    ...
    def __init__(self, name, model_dir_path, **config):
        ...
        # use to cache the score vector, score vector is a vector v=[0.5,2.0,3.0], v[0] means that the document 'd' whose index is 0 in document collection, its score with query is 0.5.
    ...
    def search(self, query, top_num=10, valid_doc_id_set=None):
        ...
        :param valid_doc_id_set: the doc id set. the id of the returned document, its doc_id must in this id set. if None, any document could be returned.
    ...
    def train_from_doc_collection(self,
                                  doc_collection: MultiFieldDocumentCollection,
                                  preprocessor: Preprocessor,
                                  **config):
        """
        train the model with the document collection that not preprocessed
        :param doc_collection: doc_collection
        :param preprocessor: the preprocessor used to preprocess the document
        :param config: the config to train the model
        :return:
        """
        preprocess_doc_collection = PreprocessMultiFieldDocumentCollection.create_from_doc_collection(
            doc_collection=doc_collection, preprocessor=preprocessor)
        self.train_from_doc_collection_with_preprocessor(doc_collection=preprocess_doc_collection, **config)
    ...
    def filter_retrieve_document(self, sorted_index_scores, top_n, valid_doc_id_set=None):
        ...
        """
        if valid_doc_id_set is not None:
