...
class Page(object):
    ...
    def open(self, **kwargs):
        if not self.url:
            raise PromiumException(u"Can't open page without url")
        ...
        if isinstance(self._get_current_driver(), WebElement):
            if self.by == By.XPATH and not self.locator.startswith("."):
                raise LocatorException(
                    u'Xpath locator must start with "." (dot). '
                    u'Please check this locator "%s"' % self.locator
                )
...
class Collection(Base, Sequence):
    ...
    def wait_empty(self, timeout=10):
        msg = u'Collection with {}={} is not empty after {}'.format(
    ...
    def wait_not_empty(self, timeout=10):
        msg = u'Collection with {}={} is empty after {}'.format(
            self.by, self.locator, timeout
        )
    ...
    def filter_by(self, child_locator):
        ...
        if not results:
            ElementLocationException(
                u'Not fond elements with %s' % child_locator
            )
        ...
        if len(found_elements_by_test) == 0:
            if check_negative:
                ...
                raise PromiumException(
                    u'Element with text "%s" has not been found' % text
                )
        else:
...
class ElementBase(Base):
    ...
    def _get_element(self):
        if self.index >= len(self._cached_elements):
            raise PromiumException(
                u'Collection by={by} locator={locator} has been changed, '
                u'get {index} of {maximum} elements'.format(
                    by=self.by,
    ...
    def wait_to_display(self, timeout=10):
        ...
        # wait_for_page_loaded(self.driver) TODO to test if this is necessary
        params = (self.by, self.locator, timeout)
        wait_until(
            driver=obj,
            seconds=timeout,
            expression=lambda driver: self.is_displayed(),
            msg=(
                u"Element with %s=%s was found "
                u"but was not displayed after %s seconds" % params
            )
        )
        return self
    ...
    def wait_to_present(self, timeout=10):
        obj = self._get_current_driver()
        params = (self.by, self.locator, timeout)
        msg = u"Element with %s=%s not found after %s seconds" % params
        wait_until(obj, is_present(self), timeout, msg)
        return self

    def wait_to_disappear(self, timeout=10):

        def is_no_visible(element):
            if not element:
                return True
        ...
        return wait_until(
            driver=self.driver,
            expression=lambda driver: is_no_visible(self._find_elements()),
            seconds=timeout,
            msg=u"Element with %s=%s did not disappear after %s seconds" % (
                self.by, self.locator, timeout
            )
        )
...
class Element(ElementBase):
    ...
    def wait_for_text_present(self, text):
        ...
        return wait_until(
            ...
            msg=u"Text: %s, does not appear after 10 seconds" % text
        )

    def set_inner_html(self, value):
        self.driver.execute_script(
            u'arguments[0].innerHTML = "%s"' % value, self.lookup()
        )
    ...
    def wait_for_changes_text_attribute(self, name_attribute, text):
        return wait_until(
            ...
            seconds=10,
            msg=u'Text attribute has not changed'
        )
    ...
    def wait_for_text_change(self, text=None):
        ...
        return wait_until(
            ...
            seconds=5,
            msg=u'Text "{}" not changed'.format(element_text)
        )

    def wait_to_enabled(self, timeout=10):

        def is_disabled(element):
            return bool(element.get_attribute("disabled") == 'true')

        params = (self.by, self.locator, timeout)
        return wait_until(
            driver=self.driver,
            seconds=timeout,
            expression=lambda driver: is_disabled(self),
            msg=u"Element with %s=%s not enabled after %s seconds" % params
        )
