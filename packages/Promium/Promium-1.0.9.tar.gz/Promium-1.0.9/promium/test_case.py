...
def create_driver(device):
    ...
    if not driver:
        raise ValueError(
            'Need choice Selenium WebDriver: {}'.format(DRIVERS.keys())
        )
    ...
    if driver in DRIVERS and not grid_host:
        ...
        if capabilities is None:
            raise ValueError('Unknown client specified: {}'.format(driver))
        ...
        try:
            ...
            log.warning("[SETUP] Second try for remote driver connection.")
            driver = webdriver.Remote(
                command_executor=grid_host,
                desired_capabilities=capabilities,
                browser_profile=settings.get_preferences()
    ...
    raise ValueError(
        'Unknown driver specified: "{}", "{}"'.format(driver, grid_host)
    )
...
class TestCase(object):
    ...
    def get_path_to_test(self, method):
        return "{path}.py -k {test_name}".format(
            path='/'.join(str(self.__module__).split('.')[1:]),
    ...
    def get_failed_test_command(self, name):
        ...
        return (
            "{command} {path_to_test} --fail-debug-info --capturelog".format(
                command=command,
                ...
                if self.excluded_browser_console_errors:
                    try:
                        ...
                        raise PromiumException(
                            ...
                            "Original exception is: %s" % e.message
                        )
