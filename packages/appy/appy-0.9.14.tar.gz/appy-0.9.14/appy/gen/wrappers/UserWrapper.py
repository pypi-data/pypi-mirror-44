...
class UserWrapper(AbstractWrapper):
    ...
    def showResetPassword(self):
        ...
        if self.user.hasRole('Manager') and (self.source == 'zodb') and \
    ...
    def doResetPassword(self, noSecurity=False):
        ...
        if not noSecurity and not self.user.hasRole('Manager'):
    ...
    def hasRole(self, role, obj=None):
        ...
        # Try first with the user's global roles
        r = sutils.stringIsAmong(role, self.getRoles())
        if r or not obj: return r
    ...
    def hasPermission(self, permission, obj):
        ...
        # roles like Authenticated or Anonymous).
