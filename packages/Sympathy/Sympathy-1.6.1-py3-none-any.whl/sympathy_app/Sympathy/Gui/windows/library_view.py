...
class SeparateTagLibraryModel(LibraryModel):
    ...
    def _build_model(self):
        ...
        if 'Sympathy' in libraries and 'Internal' in libraries:
            libraries['Sympathy'].extend(libraries.pop('Internal'))

        self._root = LibraryGroupItem('Root', None, self._style)
        for library in libraries.keys():
            child = LibraryGroupItem(library, self._root, self._style)
            tag_mapping = {
