...
class SubflowView(BaseNodeView):
    ...
    def _init_context_menu(self):
        ...
        if self._model.class_name == 'Lambda':
            ...
            self._show_input_menu = self._input_menu.addMenu('Create')
            self._show_output_menu = self._output_menu.addMenu('Create')
        else:
            self._ports_menu = None
            self._input_menu = None
            self._output_menu = None
            self._show_input_menu = None
            self._show_output_menu = None
    ...
    def contextMenuEvent(self, event):
        ...
        if self._ports_menu:
            for ports, menu, ports_menu, basefunc in [
                    (input_ports, self._show_input_menu,
                     self._input_menu, self._create_input),
                    (output_ports, self._show_output_menu,
                     self._output_menu, self._create_output)]:
                if menu:
                    ports_menu.menuAction().setVisible(bool(ports))

                    for port_ in ports:
                        action = QtWidgets.QAction(
                            '{}[{}]'.format(port_.description, port_.index),
                            self)
        ...
        for menu in [self._show_input_menu, self._show_output_menu]:
            if menu:
