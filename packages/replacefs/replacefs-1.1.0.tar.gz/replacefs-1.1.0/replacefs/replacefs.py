...
def treat_input_parameters(input_parameters, filename_must_end_by, init_str,
                    ...
                    if arg in FILENAME_MUST_END_BY_INDICATORS_STRINGS:
                        for potential_end_filter_index, potential_end_filter in enumerate(
                            ...
                            if not potential_end_filter.startswith("-"):
                                ...
                                args_not_used_indexes.remove(arg_index + 2 + potential_end_filter_index)
                            else:
                                break
                        ...
                        init_str = input_parameters[arg_index + 2]
                        args_not_used_indexes.remove(arg_index + 2)
                    elif arg in DESTINATION_STRING_INDICATORS_STRINGS:
                        dest_str = input_parameters[arg_index + 2]
                        args_not_used_indexes.remove(arg_index + 2)
                    elif arg in DIRECTORY_PATH_TO_APPLY_INDICATORS_STRINGS:
                        dir_path_to_apply = input_parameters[arg_index + 2]
                        args_not_used_indexes.remove(arg_index + 2)
...
def get_final_pmts_local_recursive(dir_path_to_apply, dest_str, init_str, local, recursive,
                                   input_params, args_not_used_indexes):
    if dir_path_to_apply is None:
        ...
        dir_path_to_apply = input_params[args_not_used_indexes[-1]]
    ...
    if dest_str is None:
        ...
        dest_str = input_params[args_not_used_indexes[-1]]
        args_not_used_indexes.pop()
    if init_str is None:
        if not args_not_used_indexes:
            logger.error("arguments are missing ... please review the command syntax.")
        ...
        init_str = input_params[args_not_used_indexes[-1]]
        args_not_used_indexes.pop()
...
def get_final_pmts_specific(file_paths_to_apply, dest_str, init_str,
                            specific, input_params, args_not_used_indexes):
    if init_str is None:
        if not args_not_used_indexes:
            logger.error("arguments are missing ... please review the command syntax.")
            missing_args_specific_error(specific)

        init_str = input_params[args_not_used_indexes[0]]
        args_not_used_indexes.pop(0)
    ...
    if dest_str is None:
        ...
        dest_str = input_params[args_not_used_indexes[0]]
        args_not_used_indexes.pop(0)
    ...
    if not file_paths_to_apply:
        ...
        for parameter_not_already_used_index in args_not_used_indexes:
            file_paths_to_apply.append(input_params[parameter_not_already_used_index])
...
def print_prev_lines(previous_lines, line_nb):
    if len(previous_lines) == 2:
        ...
        print(CBYELLOW + "\t    %s: " % (line_nb - 1) + CTEXT_FILES + "%s" % previous_lines[0] + CNORMAL_WHITE, end='')
...
def display_line_highlighting_init_strs(line, line_nb, start_string_positions, init_str, previous_lines):
    if len(start_string_positions) > 1:
        print("\n\tthere are several occurrences of " + COCCURRENCES + "\"%s\"" % init_str +
              CNORMAL_WHITE + " in this line:\n")

    print_prev_lines(previous_lines, line_nb)

    print(COCCURRENCES + "\t    %s: " % line_nb + CTEXT_FILES + "%s" % line[0:start_string_positions[0]],
          end='')
    for string_list_index, string_index in enumerate(start_string_positions):
        print(COCCURRENCES + "%s" % init_str + CNORMAL_WHITE, end='')
        if string_list_index == len(start_string_positions) - 1:
            print(CTEXT_FILES + "%s" % line[string_index + len(init_str):] + CNORMAL_WHITE, end='')
        else:
...
def display_line_highlighting_defined_init_str(new_line, line, init_str,
    ...
    print(COCCURRENCES + "%s" % init_str + CNORMAL_WHITE, end='')
    print(CTEXT_FILES + "%s" %
          line[defined_init_str_start_position + len(init_str):] + CNORMAL_WHITE, end='')
...
def file_replace(file_path, temporary_file_path, init_str, dest_str, nb_occs_found,
    ...
    print("\nthere is " + COCCURRENCES + "\"%s\"" % init_str + CNORMAL_WHITE +
          " in the file " + CFILE_PATHS + "%s" % file_path + CNORMAL_WHITE)
    print("\treplacing " + COCCURRENCES + "\"%s\"" % init_str + CNORMAL_WHITE +
          " by " + COCCURRENCES + "\"%s\"" % dest_str + CNORMAL_WHITE +
...
def occs_summary(nb_occs_found, nb_occs_replaced, init_str):
    if nb_occs_found == 0:
        print(CFILE_PATHS + "\n\t0" + CNORMAL_WHITE + " occurrence of " + COCCURRENCES + "%s" % init_str + CNORMAL_WHITE + " found")
    elif nb_occs_found == 1:
        print(CFILE_PATHS + "\n\t1" + CNORMAL_WHITE + " occurrence of " + COCCURRENCES + "%s" % init_str +
              CNORMAL_WHITE + " found and " + CFILE_PATHS + "%s" % nb_occs_replaced + CNORMAL_WHITE + " replaced")
    else:
        print(CFILE_PATHS + "\n\t%s" % nb_occs_found + CNORMAL_WHITE + " occurrences of " + COCCURRENCES +
              "%s" % init_str + CNORMAL_WHITE + " found and " + CFILE_PATHS + "%s" % nb_occs_replaced +
...
def check_integrity_of_mode_request(local, recursive, specific, dir_path_to_apply, file_paths_to_apply):
    if local or recursive:
        if file_paths_to_apply:
            print_WARNING_in_red()
            print("the replace mode is not specific, file_paths_to_apply "
                  "should be empty and is: %s" % file_paths_to_apply)
            raise ValueError("file_paths_to_apply should be empty")
        if dir_path_to_apply is None:
            print_WARNING_in_red()
            print("the replace mode is not specific, dir_path_to_apply should not be None")
        ...
        if not file_paths_to_apply:
            print_WARNING_in_red()
            print("the replace mode is specific, file_paths_to_apply should not be empty and is: %s")
            raise ValueError("file_paths_to_apply should not be empty in specific mode replacement")
        if dir_path_to_apply is not None:
            print_WARNING_in_red()
            print("the replace mode is specific, dir_path_to_apply should be None")
...
def main():
    ...
    filename_must_end_by, init_str, dest_str, dir_path_to_apply, file_paths_to_apply, \
        local, recursive, specific, ask_replace, case_sensitive, black_list_extensions, binary_accepted, \
        symlink_accepted, excluded_strings, excluded_extensions, excluded_paths, args_not_used_indexes = \
        treat_input_parameters(sys.argv, filename_must_end_by, init_str, dest_str,
    ...
    if local or recursive:
        dir_path_to_apply, dest_str, init_str = \
            get_final_pmts_local_recursive(dir_path_to_apply, dest_str,
                                                              init_str, local, recursive, sys.argv,
                                                              args_not_used_indexes)

    elif specific:
        file_paths_to_apply, dest_str, init_str = \
            get_final_pmts_specific(file_paths_to_apply, dest_str,
                                                       init_str, specific, sys.argv,
                                                       args_not_used_indexes)
    else:
        logger.error("the replace mode can only be \"local\", \"recursive\" or \"specific\"")
        raise ValueError("please pick only one mode with the -l, -r or -s short options")

    dir_path_to_apply, file_paths_to_apply = get_full_paths(dir_path_to_apply,
                                                                        file_paths_to_apply)
