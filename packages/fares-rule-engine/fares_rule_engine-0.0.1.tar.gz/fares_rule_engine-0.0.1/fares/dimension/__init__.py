...
def resolve_user_dimensions(user):
    return {
        "user.pass_purchase_count": user[UserDimensions.PASS_PURCHASE_COUNT.value],
        "user.id": user[UserDimensions.ID.value],
        "user.gender": user[UserDimensions.GENDER.value],
    }
