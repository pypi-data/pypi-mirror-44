...
def _split_module(module: str, char='\n', comments=False):
    ...
    '''
    returned = []
    # incomplete lines accululation, stored as [<accululated>, <contin type>]
    incomplete = ['', None]
    for part in module.split('\n'):
        contin_type = _contin_type(incomplete[0], part)
        if contin_type:
            if contin_type == 'continu':
                incomplete[0] += '\n' + part
            else:
                if incomplete[0]:
                    returned.append(_identify_part(incomplete[0], comments))
                incomplete = [part, contin_type]
        else:
            if incomplete[0]:
                returned.append(_identify_part(incomplete[0], comments))
