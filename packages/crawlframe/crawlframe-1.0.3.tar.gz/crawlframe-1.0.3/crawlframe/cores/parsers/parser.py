...
from crawlframe.cores.parsers import BaseParse


# 所有的解析类, 继承此类.
class HtmlParser(BaseParse):
    @staticmethod
    def get_new_data(
        ...
        for expr in expr_list:
            try:
                # 正常情况, 返回结果集.
                result = callable_obj(parse_obj, expr)
...
class GetData:
    ...
    def get_data_use_xpath(xpath_obj: etree._Element, xpath_expr: str) -> list or None:
        if not hasattr(xpath_obj, 'xpath'):
            ...
            raise TypeError('param "xpath_expr" type must be string!')
        return xpath_obj.xpath(xpath_expr)

    @staticmethod
    def get_data_use_re_findall(pattern_str: str, pattern_expr: object) -> list or None:
        if not (isinstance(pattern_str, str) or isinstance(pattern_str, bytes)):
            raise TypeError('param "pattern_str" type must be string or bytes!')
        ...
        if isinstance(pattern_str, bytes):
            pattern_str = pattern_str.decode('utf-8')
