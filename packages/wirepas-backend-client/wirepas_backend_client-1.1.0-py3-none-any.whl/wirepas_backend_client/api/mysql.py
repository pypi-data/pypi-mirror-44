...
class MySQLObserver(StreamObserver):
    ...
    def __init__(self,
                 ...
                 logger=None) -> 'MySQLObserver':
        super(MySQLObserver, self).__init__(start_signal=start_signal,
        ...
        self.mysql = MySQL(username=mysql_settings.username,
    ...
    def on_data_received(self):
        ...
        while not self.exit_signal.is_set():
            ...
            try:
                message = self.rx_queue.get(timeout=self.timeout,
                                            block=True)
            except queue.Empty:
                message = None
                continue
    ...
    def pool_on_data_received(self, n_workers=4):
        ...
        def work(storage_q, exit_signal, settings, logger):
            ...
            mysql = MySQL(username=settings.username,
        ...
        for n in range(0, n_workers):
            executor.submit(work, self.rx_queue,
                            self.exit_signal, self.settings, self.logger)
        self._wait_for_exit()
...
class MySQL(object):
    ...
    def connect(self, table_creation=True)->None:
        ...
        self.logger.info('MySQL connection to {user}:{password}'
                         '@{host}:{port}'.format(user=self.username,
    ...
    def create_tables(self):
        ...
        if 'hop_count' not in column_names:
            ...
            query = 'ALTER TABLE received_packets\n' \
        ...
        if 'current_seq' not in column_names:
            ...
            query = 'ALTER TABLE diagnostic_boot\n' \
        ...
        # Create advertiser
        createtable = 'CREATE TABLE IF NOT EXISTS advertiser (   '\
            'id BIGINT NOT NULL AUTO_INCREMENT UNIQUE,   '\
                ...
                for value in fields['value']:
                    values.append((
                        ...
                        datetime.datetime.utcfromtimestamp(
                            message.timestamp / 1e3).isoformat(),
    ...
    def put_to_received_packets(self, message):
        ...
        query = 'INSERT INTO received_packets (logged_time, launch_time, path_delay_ms, network_address, sink_address, source_address, '\
            ...
            .format(message.rx_time_ms_epoch / 1000,
    ...
    def put_traffic_diagnostics(self, message):
        ...
        query = "INSERT INTO diagnostic_traffic "\
            ...
            format(message.apdu['access_cycles'],
    ...
    def put_neighbor_diagnostics(self, message):
        ...
        for i in range(0, 14):
            ...
            values.append("(LAST_INSERT_ID(),{},{},{},{},{})".format(
    ...
    def put_boot_diagnostics(self, message):
        ...
        firmware_version = firmware_version * 256 + \
            message.apdu['sw_minor_version']
        firmware_version = firmware_version * 256 + \
            message.apdu['sw_maint_version']
        firmware_version = firmware_version * 256 + \
            message.apdu['sw_dev_version']
        ...
        query = "INSERT INTO diagnostic_boot " \
            ...
            "stack_trace_2, current_seq) " \
                "VALUES (LAST_INSERT_ID(), {}, {}, {}, {}, {}, {}, {}, {}, "\
                "{}, {}, {}, {}, {});". \
            format(message.apdu['boot_count'],
    ...
    def put_node_diagnostics(self, message):
        ...
        query = "INSERT INTO diagnostic_node "\
            ...
            "VALUES (LAST_INSERT_ID(),{},{},{},{},{},{},{},{},{},{},{},{},{},"\
            "{},{},{},{},{},{},{},{},{},{},{},{},{});".\
            format(message.apdu['access_cycle'],
                   message.apdu['role'],
                   voltage,
        ...
        for row in range(message.row_count):
            ...
            query = 'INSERT INTO ' + table_name + ' ' +\
                ...
                'field_count,' +\
                    ...
                    '{0:.32f}'.format((message.rx_time_ms_epoch - message.travel_time_ms) / 1000, '.32f') + ',' +\
