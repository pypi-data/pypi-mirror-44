...
{	// Changes here will require changes to get_filter_rate();
	...
	if ( ! cSamples) {	// Initialize all filters
		for (i = 0; i < MAX_RX_CHANNELS; i++) {
			...
			quisk_filt_cInit(&Storage[i].filtDecim5B, quiskFilt240D5Coefs, sizeof(quiskFilt240D5Coefs)/sizeof(double));
	...
	case 100:	// Red Pitaya
		nSamples = quisk_cDecim2HB45(cSamples, nSamples, &Storage[bank].HalfBand1);
		nSamples = quisk_cInterpDecim(cSamples, nSamples, &Storage[bank].filt300D5, 6, 5);
		nSamples = quisk_cInterpDecim(cSamples, nSamples, &Storage[bank].filtDecim5S, 4, 5);
		break;
	case 111:	// SDR-IQ
