...
static int dc_remove_bw=100;			// bandwidth of DC removal filter for soundcard samples
...
static void DCremove(struct sound_dev * dev, complex double * cSamples, int nSamples)
{
	int i;
	...
	if (dev->sample_rate != old_sample_rate || dc_remove_bw != old_bandwidth) {
		old_sample_rate = dev->sample_rate;	// calculate a new alpha
	...
	if (old_bandwidth > 0) {
		for (i = 0; i < nSamples; i++) {	// DC removal; R.G. Lyons page 553; 3rd Ed. p 762
			c = cSamples[i] + dev->dc_remove * alpha;
...
{  // called in an infinite loop by the main program
	...
	if (pt_sample_read) {			// read samples from SDR-IQ or UDP or SoapySDR
		...
		DCremove(&Capture, cSamples, nSamples);
		if (nSamples <= 0)
			QuiskSleepMicrosec(2000);
	}
	else {
