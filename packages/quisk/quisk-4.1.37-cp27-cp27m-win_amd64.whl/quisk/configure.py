    ...
    for i in range(len(controls)):
      ...
      if gain_mode == "automatic":
        ...
        if i == 0:
          ...
          ctrl.Enable(True)
  def MakeSampleRates(self):
    rx_sample_rates = (48000, 50000, 96000, 100000, 192000, 240000, 250000, 288000, 384000, 480000,
               500000, 768000, 960000, 1152000, 1250000, 1536000, 1920000, 2304000)
    radio_dict = local_conf.GetRadioDict(self.radio_name)
    rx_rates = []
    for fmin, fmax, delta in radio_dict['soapy_getSampleRateRange_rx']:
      for rate in rx_sample_rates:
        if fmin - 2 < rate < fmax + 2:
          if rate not in rx_rates:
            rx_rates.append(rate)
    #print ('RxRates', rx_rates)
    tx_rates = []
    for fmin, fmax, delta in radio_dict['soapy_getSampleRateRange_tx']:
      for rate in (48000, 50000, 96000, 192000):
        if fmin - 2 < rate < fmax + 2:
          if rate not in tx_rates:
            tx_rates.append(rate)
    #print ('TxRates', tx_rates)
    if not rx_rates:
      rx_rates.append(48000)
      txt = "The hardware does not offer an Rx sample rate that Quisk supports."
      msg = wx.MessageDialog(None, txt, 'SoapySDR Error', wx.OK|wx.ICON_ERROR)
      msg.ShowModal()
      msg.Destroy()
    if not tx_rates:
      tx_rates.append(48000)
      txt = "The hardware does not offer a Tx sample rate that Quisk supports."
      msg = wx.MessageDialog(None, txt, 'SoapySDR Error', wx.OK|wx.ICON_ERROR)
      msg.ShowModal()
      msg.Destroy()
    tx_rates.sort()	# choose the best tx_rate
    if len(tx_rates) > 1 and 50000 in tx_rates:
      del tx_rates[50000]
    radio_dict['soapy_sample_rates_rx'] = rx_rates
    radio_dict['soapy_sample_rate_tx'] = tx_rates[0]
    #print ('Final Tx sample rate', tx_rates[0])
  ...
  def OnChange(self, ctrl):
    ...
    if soapy and self.radio_name == Settings[1]:	# changed for current radio
      if name in ('soapy_gain_mode_rx', 'soapy_gain_mode_tx'):
        ...
        soapy.set_parameter(name, value, 0.0)	# string values
