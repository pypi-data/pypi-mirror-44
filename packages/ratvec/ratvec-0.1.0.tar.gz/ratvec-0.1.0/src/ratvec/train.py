...
def main(full_vocab_file: str,
    ...
    if sim == 'global-alignment':
        ...
        secho(f'Done with repr vocab with global alignment ({sim_alignment_matrix})')
        ...
        ngram_to_index[OOV_TOKEN] = len(ngram_to_index)
...
def _get_ngram_elements_helper(strings, *, ngram_to_index, n: int, desc=None):
    ...
    for string in tqdm(strings, desc=desc):
        string_ngrams = []
        for t in ngrams(string, n) :
            if t in ngram_to_index:
                new_index = ngram_to_index[t]
            else:
                new_index = ngram_to_index[OOV_TOKEN]
            string_ngrams.append(new_index)
        final_res.append(string_ngrams)
    return final_res
...
def calculate_global_alignment_similarity_matrix(
    ...
    from ratvec.similarity_slow import global_alignment_similarity, global_alignment_similarity_tup

    func = partial(global_alignment_similarity_tup, matrix=matrix)
