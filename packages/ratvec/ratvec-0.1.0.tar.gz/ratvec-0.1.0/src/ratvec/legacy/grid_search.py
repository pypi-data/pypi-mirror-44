...
with codecs.open(labelsPath, "r") as f_in:
    # with codecs.open(DATA_DIRECTORY +"Y_test.txt", "r") as f_in:
    Y = np.array([l[:-1] for l in f_in])

with codecs.open(vocabPath, "r") as fIn:
    vocab = [normalize_word(w[:-1]) for w in fIn if len(w[:-1].split()) == 1]

eval_file = open("evaluation_{}_{}_{}_{}.csv".format(args.sim, args.n_ngram, len(vocab), args.size), "w")
eval_file.write("|".join(["kernel", "hyperparam", "sensitivity", "specificity", "accuracy"]) + '\n')

kernels = ["poly", "rbf"]
hyperparams = {"poly": range(2, 20), "rbf": [10 ** i for i in range(-9, 9)]}

termcolor.cprint("Computing similarity matrix for representative vocabulary\n", "blue")
simMatrix_repr = computeSimMatrix(reprVocab, reprVocab)
if simMatrixMode != "infer":
    termcolor.cprint("Computing similarity matrix for full vocabulary\n", "blue")
    simMatrix_full = computeSimMatrix(reprVocab, vocab)
    np.save(outputPath + "/simMatrix_{}_{}_{}.npy".format(args.sim, args.n_ngram, len(reprVocab)), simMatrix_full,
            allow_pickle=False)
...
for kernel in kernels:
    for hyperparam in hyperparams[kernel]:
        ...
        if simMatrixMode == "infer":
            ...
            # alphas = pickle.load( open(outputPath+"/alphas_{}_{}_{}_{}_{}_{}.p".format(args.sim,args.n_ngram, len(reprVocab),kernel, hyperparam, n_components), "rb" ) )
            # lambdas = pickle.load( open(outputPath+"/lambdas_{}_{}_{}_{}_{}_{}.p".format(args.sim,args.n_ngram, len(reprVocab),kernel, hyperparam, n_components), "rb" ) )
        else:
            # termcolor.cprint("Computing similarity matrix for full vocabulary\n", "blue")
            # simMatrix = compute_similarity_matrix(reprVocab, vocab)
            simMatrix = simMatrix_full
        ...
        if useGPU:
            X_train = projectWordsGPU(vocab, reprVocab, hyperparam, projmatrix, kernel, simMatrix)
            cm.shutdown()
        else:
            X_train = pool.map(projectWordTup, [(word, reprVocab, hyperparam, projmatrix, kernel) for word in vocab])
