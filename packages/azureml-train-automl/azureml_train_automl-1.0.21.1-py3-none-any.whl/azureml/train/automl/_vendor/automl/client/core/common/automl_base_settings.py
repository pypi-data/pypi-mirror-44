...
class AutoMLBaseSettings:
    ...
    def rule_based_validation(
        ...
        if self._is_rule_based_cv(X):
            self.n_cross_validations = constants.RuleBasedValidation.DEFAULT_N_CROSS_VALIDATIONS
            logger.info(
    ...
    @staticmethod
    def _is_rule_based_cv(X: Any) -> bool:
        """Return whether the cross validation is needed."""
        return not scipy.sparse.issparse(X) and X.shape[0] < constants.RuleBasedValidation.CV_VALIDATION_LIMIT
