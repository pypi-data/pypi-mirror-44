...
class AutoMLConfig(object):
    ...
    def _validate_config_settings(self):
        ...
        if settings['compute_target'] != constants.ComputeTargets.LOCAL:
            ...
            def are_dataflows_valid():
                for key in ['X', 'y', 'sample_weight', 'X_valid', 'y_valid', 'sample_weight_valid']:
                    if key not in settings.keys():
                        continue
                    value = settings[key]
                    if value is not None and not dataprep_utilities.is_dataflow(value):
                        return False
                if 'cv_splits_indices' in settings.keys():
                    ...
                    for split in cv_splits_indices:
                        if not dataprep_utilities.is_dataflow(split):
                            return False
                return True
