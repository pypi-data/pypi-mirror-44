...
class DataTransformer(AutoMLTransformer):
    ...
    def _learn_transformations(self, df, y=None):
        ...
        self.mapper = DataFrameMapper(self._get_transforms(df, y), input_df=True, sparse=True)
    ...
    def _get_transforms(self, df: pd.DataFrame, y: DataSingleColumnInputType = None) -> List[TransformerType]:
        """
        ...
        self._raw_feature_names, all_new_column_names = self._generate_new_column_names(all_columns)
        ...
        transforms.extend(self._sweep_column_purpose(transforms, all_columns, dtypes,
                                                     stats_and_column_purposes, all_new_column_names))

        sweeped_transforms = self._perform_feature_sweeping(df, y, stats_and_column_purposes)
        self._logger_wrapper("debug", "Sweeping added the following transforms: {trs}".format(
        ...
        self._logger_wrapper('info', self._get_transformations_str(all_columns, transforms))
    ...
    def _perform_feature_sweeping(self, df: pd.DataFrame, y: DataSingleColumnInputType,
        ...
        transforms = []
        try:
            if self._is_onnx_compatible is False:
                from automl.client.core.common.sweeping.meta_sweeper import MetaSweeper
                feature_sweeper = MetaSweeper()

                sweeped_transforms = feature_sweeper.sweep(df, y, stats_and_column_purposes)
                if sweeped_transforms:
                    ...
                    for col_name, tfs in sweeped_transforms:
                        text_wordembedding_transformer = _Transformer(
                            parent_feature_list=[str(col_name)],
                            transformation_fnc=_TransformationFunctionNames.WordEmbedding,
                            operator=None, feature_type=FeatureTypeRecognizer.Text,
                            should_output=True
                        )

                        feature_transformers = _FeatureTransformers([text_wordembedding_transformer])
                        json_obj = feature_transformers.encode_transformations_from_list()
                        wordembedding_column_name = self._engineered_feature_names_class. \
                            get_raw_feature_alias_name(json_obj)

                        transforms.append((col_name,
                                           make_pipeline(TextFeaturizers.string_cast(), tfs),
                                           {"alias": str(wordembedding_column_name)}))
        except Exception as e:
            # Never fail the main run even if sweeping fails.
            self._logger_wrapper("debug", "Sweeping failed with error: {e}".format(e=e))
        finally:
            return transforms
    ...
    def _get_transforms_per_column_purpose(
            ...
            stats_and_column_purposes: List[Tuple[RawFeatureStats, str, str]]) -> List[TransformerType]:
        ...
        for column in columnspurpose:
            ...
            if detected_column_purpose == FeatureTypeRecognizer.DateTime:
                tr.extend(self._get_datetime_transforms(column, new_column_name))
            if detected_column_purpose == FeatureTypeRecognizer.CategoricalHash:
                tr.extend(self._get_categorical_hash_transforms(column, new_column_name, raw_stats.num_unique_vals))

            if detected_column_purpose == FeatureTypeRecognizer.Categorical:
                tr.extend(self._get_categorical_transforms(column, new_column_name, raw_stats.num_unique_vals))
    ...
    def _get_transformations_str(
        ...
        for column in columns:
            ...
            column_matches_transforms = [i for i in range(num_transforms) if transforms[i][0] == column]
