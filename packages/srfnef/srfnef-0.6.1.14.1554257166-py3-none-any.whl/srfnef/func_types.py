...
from .correction.psf.deconvolutor import DeconvolutorXy
...
class MlemPsf:
    ...
    projector: Projector
    back_projector: BackProjector
    deconvolutor_xy: DeconvolutorXy
    emap_mlem: EmapMlem
    saver: Saver = attr.ib(default = Saver())
    is_tqdm: bool = attr.ib(default = True)
    name: str = attr.ib(default = 'mlem')
    ...
    def __call__(self, listmode: Listmode, *, x = None, labels = []):
        ...
        for ind in range_:
            ...
            bp = self.back_projector(listmode / proj)
            x = x * bp / self.emap_mlem
            x = self.deconvolutor_xy(x)
            x = self.saver(ind + 1, x, labels = labels)
        return x

    @classmethod
    def from_mlem(cls, mlem: Mlem, deconvolutor_xy: DeconvolutorXy):
        kwargs = mlem.to_dict(recurse = False)
        kwargs.update({'deconvolutor_xy': deconvolutor_xy})
        return cls(**kwargs)
