...
from ..preprocess import pre_all_scatter_position,pre_atten,pre_lors_atten,pre_sumup_of_emission,get_all_crystal_position
from numba import jit,cuda
import srfnef as nef

@jit
def scatter_fraction(emission_image,u_map,index,lors,scanner,low_energy,high_energy,resolution):
    ...
    atten_lors = pre_lors_atten(u_map,crystal_position[lors[index,0]],crystal_position[lors[index,1]]).reshape(-1,1)  
    ...
    loop_all_lors[(512,512),(16,16)](scanner.nb_blocks_per_ring*scanner.blocks.shape[1],scanner.nb_blocks_per_ring,np.array(scanner.blocks.shape,dtype=np.int32),
                    scatter_position,crystal_position,low_energy,high_energy,math.sqrt(511)*resolution,
    ...
    efficiency_without_scatter = eff_without_scatter(low_energy,high_energy,math.sqrt(511)*resolution)*5/(1022*math.pi)**0.5/resolution
    scatter = scatter*efficiency_without_scatter*(scanner.blocks.size[1]*scanner.blocks.size[2]/scanner.blocks.shape[1]/scanner.blocks.shape[2])**2*5/(1022*math.pi)**0.5/resolution/4/math.pi
