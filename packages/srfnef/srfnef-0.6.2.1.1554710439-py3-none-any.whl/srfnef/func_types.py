    ...
    def __call__(self, listmode: Listmode, *, x = None, labels = []):
        if x is None:
            x = Image(np.ones(self.emap_mlem.shape, dtype = np.float32), self.emap_mlem.center,
                      self.emap_mlem.size)
        range_ = range(self.n_iter) if not self.is_tqdm else tqdm(range(self.n_iter))
        sub_length = listmode.length//self.n_sub
        for ind in range_:
            num_sub = ind%self.n_sub
            index = np.random.randint(0,high=listmode.length,size=sub_length)
            proj = self.projector(x, nef.Lors(listmode.lors.data[index,:]))
            bp = self.back_projector(nef.Listmode(listmode.data[index],
                    nef.Lors(listmode.lors.data[index,:])) / proj)
            x = x * bp / self.emap_mlem
            x = self.saver(ind + 1, x, labels = labels)
        return x
