					...
					for gene in range(len(j)):
						if j[gene] in base_chain and contigs[-1] == []:
							...
							if current_depth >= depth:
								contigs[-1], right_add = contigs[-1][:min(-depth+1, 0)], contigs[-1][-depth+1:min(-depth+tails,-1)] 
								
								for cg in range(len(right_add)):
									if right_add[cg] in self.list_graph[reference][c]:
										right_add = right_add[:cg]
										break
								contigs[-1] += right_add
								current_depth = 0
								contigs.append([])
							...
							if j[gene] == j[-1]:
								contigs[-1], right_add = contigs[-1][:min(-current_depth+1, 0)], contigs[-1][-current_depth+1:min(-current_depth+tails,-1)] 
