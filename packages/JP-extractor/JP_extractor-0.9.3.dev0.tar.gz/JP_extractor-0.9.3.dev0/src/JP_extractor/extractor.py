...
class BodyExtractor():
    ...
    def _preprocess(self):
        ...
        timesd=re.sub(r'(?:<head[\S\s]*?>[\S\s]*?</head>)','',self.html)
        # reqTime=re.search(r'(\d{4}\S+\d{1,2}\S+\d{1,2})',timesd)
        ...
        if reqImg is not None:
            ...
            for x in reqImg:
                ...
                try:
                    ...
                    filename,type=imgs
                    if 'weixin' not in b[0] and 'xhtml' not in b[0] and 'foot' not in b[0] and type !='.png' and type !='.gif' :                    
                        f.append(b[0])
                except:
                    pass
            self.img=f
