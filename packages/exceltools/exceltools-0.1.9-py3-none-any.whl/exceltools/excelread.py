...
class excelread(object):
    """ Simple OS Independent Class for Extracting Data from Excel Files 
        ...
        for sheet in sheetnames:
            ...
            for row in xl.getiter(sheet):
                # Do Something here
        """ 
    def __init__(self, filename, rowskip = 0):
    ...
    def get_config(self, sheetname):
        sheet_names = self.getiter(sheetname)
        conf_dict = OrderedDict()
        conf_fields =  u'id name value'.split()
        for rawdata in sheet_names:
            rowdata = map(rawdata.get, conf_fields)
            rowdata = [str(v).strip() for v in rowdata]
            if rowdata[0]:
                conf_dict[rowdata[1].strip()] = rowdata[2].strip()
        return conf_dict
