...
@singledispatch
def normalform(formula, recursive=True):
...
def normalform_Not(formula, recursive=True):
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    return Not(phi)

@normalform.register(And)
def normalform_And(formula, recursive=True):
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    psi = normalform(formula.psi, recursive=recursive) if recursive else formula.psi
    return And(phi, psi)

@normalform.register(EU)
def normalform_EU(formula, recursive=True):
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    psi = normalform(formula.psi, recursive=recursive) if recursive else formula.psi
    return EU(phi, psi, interval=formula.interval)

@normalform.register(AU)
def normalform_AU(formula, recursive=True):
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    psi = normalform(formula.psi, recursive=recursive) if recursive else formula.psi
...
def normalform_bool(formula, recursive=True):
    """ 4.1 """
    if formula is False:
        formula = Not(True)
    return formula
...
def normalform_Or(formula, recursive=True):
    """ 4.2 """
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    psi = normalform(formula.psi, recursive=recursive) if recursive else formula.psi
    return Not(And(Not(phi), Not(psi)))

@normalform.register(Implies)
def normalform_Implies(formula, recursive=True):
    """ 4.3 """
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    psi = normalform(formula.psi, recursive=recursive) if recursive else formula.psi
...
def normalform_Equality(formula, recursive=True):
    """ 4.4 """
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    psi = normalform(formula.psi, recursive=recursive) if recursive else formula.psi
    return And(Implies(phi, psi), Implies(formula.psi, formula.phi))

@normalform.register(EF)
def normalform_EF(formula, recursive=True):
    """ 4.5 """
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    return EU(True, phi, interval=formula.interval)

@normalform.register(AF)
def normalform_AF(formula, recursive=True):
    """ 4.6 """
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    return AU(True, phi, interval=formula.interval)

@normalform.register(EG)
def normalform_EG(formula, recursive=True):
    ...
    if intvl.start_operator == operator.ge and a == 0 and \
        ...
        return EU(phi, True, interval=(Interval() > intvl.end) )
    ...
    if not intvl.compare(Interval()):  # compare to [0,inf)
        return Not(AF(Not(phi), interval=formula.interval))
...
def normalform_AG(formula, recursive=False):
    """ 4.10 rewrite AG_I """
    phi = normalform(formula.phi, recursive=recursive) if recursive else formula.phi
    return Not(EF(Not(phi), interval=formula.interval))
...
def normalform_AU(formula, recursive=False):
    ...
    if intvl.start_operator == operator.ge and a != 0 and \
        ...
        return AG(And(phi, p2), interval=Interval() < a)
