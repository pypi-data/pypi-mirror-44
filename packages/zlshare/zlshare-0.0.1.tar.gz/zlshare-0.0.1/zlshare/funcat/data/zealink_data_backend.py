...
import zlshare as zl
from cached_property import cached_property

from zlshare.funcat.data.backend import DataBackend
from zlshare.funcat.utils import lru_cache, get_str_date_from_int, get_int_date

class ZealinkDataBackend(DataBackend):

    @cached_property
    def stock_basics(self):
        return zl.get_stock_basics()
    ...
    def convert_code(self, order_book_id):
        code = order_book_id.replace("XSHG", "sh")
    ...
    def get_price(self, order_book_id, start, end, freq):
        """
        :param order_book_id: e.g. 000002.sh
        ...
        #code = order_book_id
        is_index = False
        if ((code.startswith("0") and code.endswith(".sh")) or
                (code.startswith("3") and code.endswith(".sh"))
            ):
            is_index = True
        ktype = freq
        if freq[-1] == "m":
        ...
        df = zl.get_k_data(code, start=start, end=end, index=is_index, ktype=ktype)
    ...
    def get_trading_dates(self, start, end):
        ...
        df = zl.get_k_data("000001.sh", index=True, start=start, end=end)
    ...
    def symbol(self, order_book_id):
        ...
        """
        code = self.convert_code(order_book_id)
        return u"{}[{}]".format(order_book_id, self.code_name_map.get(code))