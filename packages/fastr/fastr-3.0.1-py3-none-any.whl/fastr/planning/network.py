...
class Network(Serializable):
    ...
    def draw(self, name=None, draw_dimensions=True, hide_unconnected=True, context=None, graph=None, expand_macro=False):
        ...
        if graph is None:
            graph = Digraph(
                ...
                node_attr={'shape': 'record', 'fontsize': '14', 'margin': '0.55,0.055'}
            )
    ...
    def draw_network(self, name="network_layout", img_format='svg', draw_dimension=True, hide_unconnected=True, expand_macro=False):
        """
        ...
        graph = self.draw(draw_dimensions=draw_dimension, hide_unconnected=hide_unconnected, expand_macro=expand_macro)
