...
module rebo2sich_default_tables
  use supplib
...
contains
  ...
  !<
  subroutine rebo2sich_default_Fcc_table(F, dFdi, dFdj, dFdk)
    implicit none
    ...
    ! Schall, Harrison, 2012 - changes energies of a-C slightly
    F(1, 1, 2:8) = -0.30_DP

    !
    ! Values from Table 4
    ! Stuart et al. Table IX
    !

    F(0, 0,  2:) =  0.0099172158_DP
    F(0, 1,   0) =  0.04338699_DP
    F(0, 1,  1:) =  0.0099172158_DP
    F(1, 1,   0) =  0.105000_DP
    F(1, 1,   1) = -0.0041775_DP
    F(1, 1,  2:) = -0.0160856_DP
    F(0, 2,   0) =  0.0493976637_DP
    F(0, 2,   1) = -0.011942669_DP
    F(0, 2,  2:) =  0.0099172158_DP
    ...
    F(0, 3,  2:) =  0.0099172158_DP
    F(1, 3,  1:) = -0.124836752_DP
    F(2, 3,  0:) = -0.044709383_DP

    dFdj(3, 1,   1) = 2*0.0375447764_DP ! This is from Stuarts paper, half in DWB's code
    dFdi(2, 1,   0) = -0.052500_DP
    dFdi(2, 1,  4:) = -0.054376_DP
    dFdi(2, 3,  1:) =  2*0.062418_DP ! This is from Stuarts paper, half in DWB's code

    !dFdk(1, 1,   1)  = -0.060543_DP ! This is not in Stuarts paper
  ...
  endsubroutine rebo2sich_default_Fcc_table


  !>
  ! Construct the Fch-table
  !<
  subroutine rebo2sich_default_Fch_table(F, dFdi, dFdj, dFdk)
    implicit none
    ...
    !
    ! Values from Table 9 of Brenner 2002
    !             Table IX of Stuart 2000
    !
    ...
    F(1, 1,   2)  = -0.3_DP*2
    F(1, 1,  3:)  = -0.05_DP*2
    F(0, 2,  4:)  = -0.0090477875161288110_DP
    F(1, 2, 1:2)  = -0.25_DP*2
    F(1, 3,   0)  = -0.1_DP*2
    F(1, 3, 1:2)  = -0.125_DP*2
    F(1, 3,  3:)  = -0.1_DP*2
  ...
  endsubroutine rebo2sich_default_Fch_table


  !>
  ! Construct the Fhh-table
  !<
  subroutine rebo2sich_default_Fhh_table(F, dFdi, dFdj, dFdk)
    implicit none
...
! Derivatives from dFijkHH.dat (already symmetric) Schall-2013
  ...
  endsubroutine rebo2sich_default_Fhh_table


  !>
  ! Construct the Fsic-table
  !<
  subroutine rebo2sich_default_Fsic_table(F, dFdi, dFdj, dFdk)
    implicit none
  ...
  endsubroutine rebo2sich_default_Fsic_table


  !>
  ! Construct the Fsih-table
  !<
  subroutine rebo2sich_default_Fsih_table(F, dFdi, dFdj, dFdk)
    implicit none
...
dFdk(3,1,0) = -0.086859_DP
  ...
  endsubroutine rebo2sich_default_Fsih_table


  !>
  ! Construct the Fsisi-table
  !<
  subroutine rebo2sich_default_Fsisi_table(F, dFdi, dFdj, dFdk)
    implicit none
...
!    F(3, 2, 3)    = -0.0208718_DP
    ...
    do k = 0, 8
       do i = 0, 2
          do j = i+1, 3
             ...
             x = (dFdi(i, j, k) + dFdj(j, i, k))/2
             if (abs(dFdi(i, j, k)-x) > 1d-6 .or. &
                 abs(dFdj(j, i, k)-x) > 1d-6) then
                call prlog("Warning! Table not symmetric: dFdi("//i//","//j//","//k//") = "//dFdi(i,j,k)//" != dFdj("//j//","//i//","//k//") = "//dFdj(j,i,k))
             endif
             dFdi(i, j, k) = x
             dFdj(j, i, k) = x

             x = (dFdi(j, i, k) + dFdj(i, j, k))/2
             if (abs(dFdi(j, i, k)-x) > 1d-6 .or. &
                 abs(dFdj(i, j, k)-x) > 1d-6) then
                call prlog("Warning! Table not symmetric: dFdi("//j//","//i//","//k//") = "//dFdi(j,i,k)//" != dFdj("//i//","//j//","//k//") = "//dFdj(i,j,k))
             endif
             dFdi(j, i, k) = x
             dFdj(i, j, k) = x

             x = (dFdk(i, j, k) + dFdk(j, i, k))/2
             if (abs(dFdk(i, j, k)-x) > 1d-6 .or. &
                 abs(dFdk(j, i, k)-x) > 1d-6) then
                call prlog("Warning! Table not symmetric: dFdk("//i//","//j//","//k//") = "//dFdk(i,j,k)//" != dFdk("//j//","//i//","//k//") = "//dFdk(j,i,k))
             endif
             dFdk(i, j, k) = x
             dFdk(j, i, k) = x
          enddo
  ...
  endsubroutine rebo2sich_default_Fsisi_table
  ...
  !<
  subroutine rebo2sich_default_Pcc_table(P, dPdi, dPdj, dPdk)
    implicit none
    ...
    !P(2, 0, 0) = 0.007860700254745_DP ! Brenner 2002
    P(2, 0, 0) = -0.000500_DP ! Stuart 2000
    P(3, 0, 0) = 0.016125364564267_DP
    !P(1, 1, 0) = 0.003026697473481_DP ! Brenner 2002
    P(1, 1, 0) = -0.010960_DP ! Stuart 2000
    P(2, 1, 0) = 0.006326248241119_DP
    P(0, 2, 0) = -0.027603_DP ! Stuart 2000 - zero in Brenner 2002
    P(1, 2, 0) = 0.003179530830731_DP ! Brenner 2002
    ...
    P(0, 1, 1) = 0.160178_DP
    P(1, 0, 1) = 0.161117_DP
    P(0, 0, 1) = 0.1_DP ! - this needs to be disabled to give energies from paper
...
! there are none.
  ...
  endsubroutine rebo2sich_default_Pcc_table


  !>
  ! Construct the Pch-table
  !<
  subroutine rebo2sich_default_Pch_table(P, dPdi, dPdj, dPdk)
    implicit none
    ...
    !
    ! Values from Table 8 of Brenner 2002
    !
...
! there are none
  ...
  endsubroutine rebo2sich_default_Pch_table


  !>
  ! Construct the Psic-table
  !<
  subroutine rebo2sich_default_Psic_table(P, dPdi, dPdj, dPdk)
    implicit none
...
dPdk(3,0,1)=0.01059475_DP
  ...
  endsubroutine rebo2sich_default_Psic_table


  !>
  ! Construct the Pcsi-table
  !<
  subroutine rebo2sich_default_Pcsi_table(P, dPdi, dPdj, dPdk)
    implicit none
    ...
    P(0, 2, 0) =  0.320357_DP
    P(1, 1, 0) =  0.161117_DP
    P(0, 1, 0) =  0.04_DP ! - this needs to be disabled to give energies from paper
    P(0, 0, 1) = -0.16218_DP
...
dPdk(3,0,1)=0.01059475_DP
  ...
  endsubroutine rebo2sich_default_Pcsi_table


  !>
  ! Construct the Psih-table
  !<
  subroutine rebo2sich_default_Psih_table(P, dPdi, dPdj, dPdk)
    implicit none
...
dPdk(3,0,1)=-0.000335339_DP
  ...
  endsubroutine rebo2sich_default_Psih_table


  !>
  ! Construct the Psisi-table
  !<
  subroutine rebo2sich_default_Psisi_table(P, dPdi, dPdj, dPdk)
    implicit none
...
dPdk(3,0,1)=-0.0379305_DP
  ...
  endsubroutine rebo2sich_default_Psisi_table


  !**********************************************************************
  ! Construct the Tcc-table
  !********************************************************************** 
  subroutine rebo2sich_default_Tcc_table(T)
    implicit none
...
!    T(2, 2, 8)  = -0.00809675_DP
  ...
  endsubroutine rebo2sich_default_Tcc_table
...
endmodule rebo2sich_default_tables
