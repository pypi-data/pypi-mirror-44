  ...
  if (ktyp == rebo2_C_) then
     ...
     if (this%c_g > -0.5_DP) then
        ...
        if (n < 3.2_DP) then
           ...
           call g_from_spline(this%cc_g_theta, this%cc_g1_coeff, costh, val, &
                              dval_dcosth)
           dval_dN = 0.0_DP

        else if (n > 3.7_DP) then
           
           call g_from_spline(this%cc_g_theta, this%cc_g2_coeff, costh, val, &
                              dval_dcosth)
           dval_dN = 0.0_DP
     ...
     if (this%Si_g > -0.5_DP) then
        ...
        call g_from_spline(this%sisi_g_theta, this%sisi_g_coeff, costh, val, &
                           dval_dcosth)
        dval_dN = 0.0_DP
     ...
     if (this%h_g > -0.5_DP) then
        ...
        call g_from_spline(this%hh_g_theta, this%hh_g_coeff, costh, val, &
                           dval_dcosth)
        dval_dN = 0.0_DP
...
pure subroutine g_from_spline(theta, coeff, costh, val, dval)
  ...
  real(DP), intent(in)  :: coeff(6, size(theta)-1)
  real(DP), intent(in)  :: costh
  real(DP), intent(out) :: val
  real(DP), intent(out) :: dval
  ...
  j = size(theta)-1
  do i = size(theta)-1, 2, -1
     if (costh < theta(i)) then
        j = i-1
     endif
  enddo
  ...
  dval = 5*coeff(6, j)
  do i = 5, 2, -1
     val  =  val*costh +       coeff(i, j)
     dval = dval*costh + (i-1)*coeff(i, j)
  enddo
  val = val*costh + coeff(1, j)
...
subroutine h(this, elj, eli, elk, ijpot, ikpot, dr, val, dval)
  ...
  real(DP)  :: fac
#ifdef SEPARATE_H_ARGUMENTS
  real(DP)  :: VAij, dVAij, VAik, dVAik, r1, r2, fC, dfC
#endif
  integer   :: m
  ...
  !
  ! exp ( ( arg * ( (rij - rij^eq) - (rik - rik^eq) ) )**m )
  !
  ...
  if (this%lambda(eli,elj,elk) >= 0.0_DP) then
     ...
     r1 = 0.5_DP*(this%cut_bo_l(ijpot) + this%cut_bo_l(ikpot))
     if (drik-drij > r1) then
        r2 = 0.5_DP*(this%cut_bo_h(ijpot) + this%cut_bo_h(ikpot))
        call cutoff_f_and_df(drik-drij, r1, r2, fC, dfC)
        ...
        if (VAik /= 0.0_DP) then
           val = VAik/VAij
           val = fC + (1.0_DP-fC)*val*val

           dval_drij = -dfC*(1.0_DP+2*val/VAij)
           dval_drik = dfC*(1.0_DP-2*val/VAik)
        endif
     endif
  endif
#else
