...
#define TOO_SMALL(what, i, ierror)  RAISE_DELAYED_ERROR("Internal neighbor list exhausted, *" // what // "* too small: " // "nebtot = " // nebtot // "/" // nebsize // ", nebmax = " // nebmax // ", nebavg = " // nebavg // ", this%neb_last(i)-this%neb_seed(i)+1 = " // (this%neb_last(i)-this%neb_seed(i)+1) // ", i = " // i // "/" // natloc // " (" // nat // ")", ierror) ; nebtot = 1 ; this%neb_last(i) = this%neb_seed(i)
  ...
  recursive subroutine BOP_KERNEL( &
       ...
       epot, f_inout, wpot_inout, &
       epot_per_at, epot_per_bond, f_per_bond, wpot_per_at, wpot_per_bond, &
       VA_per_bond, VR_per_bond, bij_per_bond, bji_per_bond, baveij_per_bond, & 
       piRC_per_bond, Pij_per_bond, Pji_per_bond, &
       ierror)
    ...
    !
    ! copyright: Todd Knippenberg, David Schall, Lars Pastewka 2012-2013
    ! - added Silicon parameter set. 10/2012
    !     J. D. Schall, J. A. Harrison, J. Phys. Chem. C 117, 1324 (2013)
    ! - changed definition of coordination numbers N. 01/2013
    ! - per element screening parameters Cmin, Cmax. 03/2013
    ! - generalized h(...) function. 12/2013
    !
    !
    ...
    real(DP),  optional, intent(inout) :: VA_per_bond(ptrmax)
    real(DP),  optional, intent(inout) :: VR_per_bond(ptrmax)
    real(DP),  optional, intent(inout) :: bij_per_bond(ptrmax)
    real(DP),  optional, intent(inout) :: bji_per_bond(ptrmax)
    real(DP),  optional, intent(inout) :: baveij_per_bond(ptrmax)
    real(DP),  optional, intent(inout) :: piRC_per_bond(ptrmax)
    real(DP),  optional, intent(inout) :: Pij_per_bond(ptrmax)
    real(DP),  optional, intent(inout) :: Pji_per_bond(ptrmax)
#endif
    ...
    real(DP)  :: bdh, bdhij, dbdhij, tij, tij_faij_fcij, dtdni, dtdnj
#ifdef SEP_NCONJ
    real(DP)  :: dtdncni, dtdncnj
#else
    real(DP)  :: dtdncn
#endif
#ifndef SCREENING
    ...
    real(DP)  :: fcsik, fcsjl, dfcsik, dfcsjl
#endif
#endif
    ...
    real(DP)  :: dnidik(3, nebmax, typemax)
    real(DP)  :: dnjdjl(3, nebmax, typemax)
    real(DP)  :: nti, ntj

#ifndef SEP_NCONJ
    real(DP)  :: nconj,dfdncn
#endif
    real(DP)  :: nconji,nconjit,nconjj,nconjdx,nconjdr
    ...
    real(DP)  :: pij,pji,dpdnci,dpdnhi,dpdnsii,dpdncj,dpdnhj,dpdnsij
    ...
    if (.not. this%neighbor_list_allocated) then
...
#endif
#ifdef SEPARATE_H_ARGUMENTS
       call prlog("The " // BOP_NAME_STR // " potential has been compiled with SEPARATE_H_ARGUMENTS.")
#endif
#ifdef SEP_NCONJ
       call prlog("The " // BOP_NAME_STR // " potential has been compiled with SEP_NCONJ.")
#endif
#ifdef EXP_CUT
       call prlog("The " // BOP_NAME_STR // " potential has been compiled with exponential cutoff functions.")
#endif
#ifdef SCREENING
       ...
       allocate(this%cutfcn(nebsize))
       allocate(this%cutdrv(nebsize))
       ...
       allocate(this%cutdrik(snebsize))
       allocate(this%cutdrjk(snebsize))
...
#ifndef LAMMPS
    !$omp& shared(VA_per_bond, VR_per_bond, bij_per_bond, bji_per_bond) &
    !$omp& shared(baveij_per_bond, piRC_per_bond, Pij_per_bond, Pji_per_bond) &
#endif
#ifdef SCREENING
    ...
    !$omp& private(bdh, bdhij, dbdhij, tij, tij_faij_fcij, dtdni, dtdnj) &
#ifdef SEP_NCONJ
    !$omp& private(dtdncni, dtdncnj) &
#else
    !$omp& private(dtdncn) &
#endif
#ifdef DIHEDRAL_SIN_CUTOFF
    !$omp& private(fcsik, fcsjl, dfcsik, dfcsjl) &
#endif
#ifndef SCREENING
    ...
    !$omp& private(bdh, bdhij, dbdhij, tij, tij_faij_fcij, dtdni, dtdnj, dtdncn) &
#endif
#ifdef NUM_NEIGHBORS
    !$omp& private(ni, nj) &
    !$omp& private(dnidik) &
...
#endif
    !$omp& private(nconji,nconjit,nconjj,nconjdx,nconjdr) &
    ...
    !$omp& private(pij,pji,dpdnci,dpdnhi,dpdncj,dpdnhj,dpdnsii,dpdnsij) &
#endif
#if defined(SCREENING) || defined(NUM_NEIGHBORS)
    !$omp& private(xik) &
#endif
          ...
          jn_loop1: do jn = jbeg, jend
             ...
             j_known_el1: if (ktypj > 0) then
                ...
                if (rlij < this%cut_in_l(ijpot)**2) then
                   ...
                   ! Screening is turned off if Cmax < Cmin
                   if (this%Cmax(ijpot) < this%Cmin(ijpot)) then
                      screened = .true.
                      kn = jend
                   endif
...
#endif
#endif
                      if (dot_product(rik, rik) < this%C_dr_cut(ijpot)*rlij) then

#ifdef LAMMPS
                         k_neq_j: if (k /= j) then
#else
                         k_neq_j: if (k /= j .or. any(VEC3(dc, kn) /= VEC3(dc, jn))) then
#endif
                            ...
                            if (dot_ij_ik > this%dot_threshold .and. dot_ij_jk < -this%dot_threshold) then
                               ...
                               if (C <= this%Cmin(ijpot)) then
                                  screened = .true.
                               else if (C < this%Cmax(ijpot)) then
                                  need_derivative = .true.

                                  Cmax_C    = this%Cmax(ijpot)-C
                                  C_Cmin    = C-this%Cmin(ijpot)

#ifdef SIN_S
                                  csij      = (1 - cos(PI*(C-Cmin)/this%dC(ijpot)))/2
                                  sij       = sij * csij
#else
...
#ifdef SIN_S
                                  fac       = PI/(2*this%dC(ijpot)) * sin(PI*(C-Cmin)/this%dC(ijpot)) / csij
#else
                                  fac       = 2*Cmax_C*this%dC(ijpot)/(C_Cmin**3)
#endif
                   ...
                   if ((screened .or. sij < this%screening_threshold) .and. rlij > this%cut_in_h2(ijpot)) then
                      ...
                      if ( screened ) then
                         ...
                         this%cutfcn(nebtot)  = (1.0_DP-fcinij)*sij*fcij + fcinij
                         this%cutdrv(nebtot)  = (1.0_DP-fcinij)*sij*(dfcijr + fcij*dsijdrij/rlij) - dfcinijr*sij*fcij + dfcinijr
...
#ifdef NUM_NEIGHBORS
                         this%cutdrncik(ineb:snebtot-1) = this%cutdrik(ineb:snebtot-1)*sij*fcncij * (1.0_DP-fcinij)
                         this%cutdrncjk(ineb:snebtot-1) = this%cutdrjk(ineb:snebtot-1)*sij*fcncij * (1.0_DP-fcinij)
#endif

                         this%cutdrik(ineb:snebtot-1) = this%cutdrik(ineb:snebtot-1)*sij*fcij * (1.0_DP-fcinij)
                         this%cutdrjk(ineb:snebtot-1) = this%cutdrjk(ineb:snebtot-1)*sij*fcij * (1.0_DP-fcinij)
                         ...
                         if (rlij < this%cut_in_h(ijpot)) then
                            ...
                            this%cutfcn(nebtot)  = (1.0_DP-fcinij)*fcij + fcinij
                            this%cutdrv(nebtot)  = (1.0_DP-fcinij)*dfcijr - dfcinijr*fcij + dfcinijr
                      ...
                      if (this%sneb_last(nebtot)-this%sneb_seed(nebtot)+1 > nebmax_sq) then
                         TOO_SMALL("nebmax", i, ierror_loc)
                      endif
    ...
    i_loop2: do i = 1, natloc
       ...
       i_known_el2: if (ktypi > 0) then
          ...
          ik_loop1: do ik = istart, ifinsh
                ...
                ! FIXME!!! Generalize
                ktypk_eq_C_or_Si: if (ktypk == rebo2_C_ .or. ktypk == rebo2_Si_) then
                   ...
                   forall(km = this%neb_seed(k):this%neb_last(k))
                      ...
                      drk(1:3, kmc + km-this%neb_seed(k)+1)  = this%bndlen(km)*this%bndnm(1:3, km)

                      xikdm(1:3, km-this%neb_seed(k)+1)      = this%cutdrvnc(km)*this%bndnm(1:3, km)

                   endforall

                   xik   = this%nn(rebo2_H_, k) + this%nn(rebo2_C_, k) + this%nn(rebo2_Si_, k) - fcik
                   xikdk = -sum(xikdm(1:3,1:this%neb_seed(k)-this%neb_last(k)+1), 2) + dnidik(1:3, ikc, ktypk)
                   ...
                   dncnidm(1:3, numnbk(ikc):numnbk(ikc+1)-1) = nconjdx*xikdm(1:3,1:numnbk(ikc+1)-numnbk(ikc))

                else
                   numnbk(ikc+1)     = kmc + 1
                   fxik(ikc)         = 0.0_DP
                   dncnidk(1:3, ikc) = 0.0_DP
                endif ktypk_eq_C_or_Si
          ...
          ij_loop: do ij = istart, ifinsh
             ...
             j_gt_i: if ((jdc == 0 .and. j > i) .or. jdc > 0) then
                ...
                ar_within_cutoff: if (rlij < this%cut_out_h(ijpot)) then
                   ...
                   if (ni(rebo2_C_) > 5.0_DP)  ni(rebo2_C_) = 5.0_DP
                   if (ni(rebo2_H_) > 5.0_DP)  ni(rebo2_H_) = 5.0_DP
!                   if (ni(rebo2_Si_) > 7.0_DP) ni(rebo2_Si_) = 7.0_DP
                   if (ni(rebo2_Si_) > 5.0_DP) ni(rebo2_Si_) = 5.0_DP

                   nti = ni(rebo2_C_) + ni(rebo2_H_) + ni(rebo2_Si_)

                   if (nj(rebo2_C_) > 5.0_DP)  nj(rebo2_C_) = 5.0_DP
                   if (nj(rebo2_H_) > 5.0_DP)  nj(rebo2_H_) = 5.0_DP
!                   if (nj(rebo2_Si_) > 7.0_DP) nj(rebo2_Si_) = 7.0_DP
                   if (nj(rebo2_Si_) > 5.0_DP) nj(rebo2_Si_) = 5.0_DP

                   ntj = nj(rebo2_C_) + nj(rebo2_H_) + nj(rebo2_Si_)
...
#ifndef LAMMPS
                   if (present(VA_per_bond)) then
                      VA_per_bond(this%nbb(ij)) = faij
                   endif
#endif
                   call VR(this, ijpot, rlij, frij, dfrijr)
#ifndef LAMMPS
                   if (present(VR_per_bond)) then
                      VR_per_bond(this%nbb(ij)) = frij
                   endif
#endif
                   ...
                   ik_loop2: do ik = istart, ifinsh
                      ...
                      ik_neq_ij: if (ik /= ij) then
                         ...
                         bo_within_cutoff1: if (rlik < this%cut_bo_h(ikpot)) then
...
#ifdef SEPARATE_H_ARGUMENTS
                            call h(this, ktypj, ktypi, ktypk, ijpot, ikpot, rlij, rlik, h_Dr, dh_dDr, dh_dr2)
#else
                            call h(this, ktypj, ktypi, ktypk, ijpot, ikpot, rlij - rlik, h_Dr, dh_dDr)
#endif
...
#ifdef NUM_NEIGHBORS
                            call g(this, ktypi, costh, nti, g_costh, dg_dcosth, dg_dn)
#else
                            call g(this, ktypj, ktypi, ktypk, ijpot, ikpot, costh, g_costh, dg_dcosth)
#endif
                            ...
                            wijb  = wijb - outer_product(rij, df) - outer_product(rik, dbidk(1:3, ikc))
...
!                  Adding in Pij values for atom types C, H, Si - MTK
                   if (ktypi == rebo2_C_) then
                      if (ktypj == rebo2_C_) then
                         call eval(this%Pcc, ni(rebo2_H_), ni(rebo2_C_), &
                              ni(rebo2_Si_), pij, dpdnhi, dpdnci, dpdnsii)
                      endif
                   endif

                   if (ktypi == rebo2_C_) then
                      if (ktypj == rebo2_H_) then
                         call eval(this%Pch, ni(rebo2_H_), ni(rebo2_C_), &
                              ni(rebo2_Si_), pij, dpdnhi, dpdnci, dpdnsii)
                      endif
                   endif

                   if (ktypi == rebo2_C_) then
                      if (ktypj == rebo2_Si_) then
                         call eval(this%Pcsi, ni(rebo2_H_), ni(rebo2_C_), &
                              ni(rebo2_Si_), pij, dpdnhi, dpdnci, dpdnsii)
                      endif
                   endif

                   if (ktypi == rebo2_Si_) then
                      if (ktypj == rebo2_C_) then
                         call eval(this%Psic, ni(rebo2_H_), ni(rebo2_C_), &
                              ni(rebo2_Si_), pij, dpdnhi, dpdnci, dpdnsii)
                      endif
                   endif
                   ...
                   if (ktypi == rebo2_Si_) then
                      if (ktypj == rebo2_H_) then
                         call eval(this%Psih, ni(rebo2_H_), ni(rebo2_C_), &
                              ni(rebo2_Si_), pij, dpdnhi, dpdnci, dpdnsii)
                      endif
                   endif
                   ...
                   if (ktypi == rebo2_Si_) then
                      if (ktypj == rebo2_Si_) then
                         call eval(this%Psisi, ni(rebo2_H_), ni(rebo2_C_), &
                              ni(rebo2_Si_), pij, dpdnhi, dpdnci, dpdnsii)
                      endif
                   endif

#ifdef DEBUG_PRINT
!                   write (*, '(A,2I3,4F15.7)')  "(ij) P "//rebo2_el_strs(ktypi)//" "//rebo2_el_strs(ktypj)//":", i, j, ni(rebo2_H_), ni(rebo2_C_), ni(rebo2_Si_), pij
                   write (*, '(A,2I3,2X,3I3,F15.7)')  "(ij) P "//rebo2_el_strs(ktypi)//" "//rebo2_el_strs(ktypj)//":", i, j, nint(ni(rebo2_H_))+1, nint(ni(rebo2_C_))+1, nint(ni(rebo2_Si_))+1, pij
#endif

#ifndef LAMMPS
                   if (present(Pij_per_bond)) then
                      Pij_per_bond(this%nbb(ij)) = pij
                   endif
#endif
...
#ifndef LAMMPS
                   if (present(bij_per_bond)) then
                      bij_per_bond(this%nbb(ij)) = bij
                   endif
#endif
                   ...
                   jl_loop: do jl = this%neb_seed(j), this%neb_last(j)
                      ...
                      l_neq_i: if (l /= i .or. ldc /= 0) then
                         ...
                         ! FIXME!!! Generalize
                         ktypl_eq_C_or_Si: if (ktypl == rebo2_C_ .or. ktypl == rebo2_Si_) then
                            ...
                            xjl   = this%nn(rebo2_H_, l) + this%nn(rebo2_C_, l) + &
                                 this%nn(rebo2_Si_, l) - fcjl
                            xjldl = &
...
#ifdef SEPARATE_H_ARGUMENTS
                            call h(this, ktypi, ktypj, ktypl, ijpot, jlpot, rlij, rljl, h_Dr, dh_dDr, dh_dr2)
#else
                            call h(this, ktypi, ktypj, ktypl, ijpot, jlpot, rlij - rljl, h_Dr, dh_dDr)
#endif
...
#ifdef NUM_NEIGHBORS
                            call g(this, ktypj, costh, ntj, g_costh, dg_dcosth, dg_dn)
#else
                            call g(this, ktypi, ktypj, ktypl, ijpot, jlpot, costh, g_costh, dg_dcosth)
#endif
...
!                  Adding in Pji values for C, H, Si - MTK
                   if (ktypj == rebo2_C_) then
                      if (ktypi == rebo2_C_) then
                         call eval(this%Pcc, nj(rebo2_H_), nj(rebo2_C_), &
                              nj(rebo2_Si_), pji, dpdnhj, dpdncj, dpdnsij)
                      endif
                   endif

                   if (ktypj == rebo2_C_) then
                      if (ktypi == rebo2_H_) then
                         call eval(this%Pch, nj(rebo2_H_), nj(rebo2_C_), &
                              nj(rebo2_Si_), pji, dpdnhj, dpdncj, dpdnsij)
                      endif
                   endif
                   ...
                   if (ktypj == rebo2_C_) then
                      if (ktypi == rebo2_Si_) then
                         call eval(this%Pcsi, nj(rebo2_H_), nj(rebo2_C_), &
                              nj(rebo2_Si_), pji, dpdnhj, dpdncj, dpdnsij)
                      endif
                   endif
                   ...
                   if (ktypj == rebo2_Si_) then
                      if (ktypi == rebo2_C_) then
                         call eval(this%Psic, nj(rebo2_H_), nj(rebo2_C_), &
                              nj(rebo2_Si_), pji, dpdnhj, dpdncj, dpdnsij)
                      endif
                   endif
                   ...
                   if (ktypj == rebo2_Si_) then
                      if (ktypi == rebo2_H_) then
                         call eval(this%Psih, nj(rebo2_H_), nj(rebo2_C_), &
                              nj(rebo2_Si_), pji, dpdnhj, dpdncj, dpdnsij)
                      endif
                   endif
                   ...
                   if (ktypj == rebo2_Si_) then
                      if (ktypi == rebo2_Si_) then
                         call eval(this%Psisi, nj(rebo2_H_), nj(rebo2_C_), &
                              nj(rebo2_Si_), pji, dpdnhj, dpdncj, dpdnsij)
                      endif
                   endif

#ifdef DEBUG_PRINT
!                   write (*, '(A,2I3,4F15.7)')  "(ji) P "//rebo2_el_strs(ktypj)//" "//rebo2_el_strs(ktypi)//":", j, i, nj(rebo2_H_), nj(rebo2_C_), nj(rebo2_Si_), pji
                   write (*, '(A,2I3,2X,3I3,F15.7)')  "(ji) P "//rebo2_el_strs(ktypj)//" "//rebo2_el_strs(ktypi)//":", j, i, nint(nj(rebo2_H_))+1, nint(nj(rebo2_C_))+1, nint(nj(rebo2_Si_))+1, pji
#endif

#ifndef LAMMPS
                   if (present(Pji_per_bond)) then
                      Pji_per_bond(this%nbb(ij)) = pji
                   endif
#endif
...
#ifndef LAMMPS
                   if (present(bji_per_bond)) then
                      bji_per_bond(this%nbb(ij)) = bji
                   endif
#endif
                   ...
                   if (nti > 4.0_DP)     nti = 4.0_DP
                   if (ntj > 4.0_DP)     ntj = 4.0_DP
#endif
...
#ifdef SEP_NCONJ
                         call eval(this%Tcc, nti, ntj, nconji, nconjj, tij, dtdni, dtdnj, dtdncni, dtdncnj)
#else
                         call eval(this%Tcc, nti, ntj, nconj, tij, dtdni, dtdnj, dtdncn)
#endif
                         tij_faij_fcij = tij*faij*fcij
                         ...
                         if (tij /= 0) then
                            ...
                            ik_loop3: do ik = istart, ifinsh
                               ...
                               if (ik /= ij) then
                                  ...
                                  ! dcik is sin(theta_jik)**2
                                  dcik       = 1.0_DP - dot_ij_ik**2

#ifdef DIHEDRAL_SIN_CUTOFF
                                  if (dcik > this%dh_sin1) then
                                     call trig_on(this%dh_sin1, this%dh_sin2, dcik, fcsik, dfcsik)
#endif
                                     ...
                                     do jl = this%neb_seed(j), this%neb_last(j)
                                        ...
                                        if ((l /= i .or. ldc /= 0) .and. &
                                           ...
                                           ! dcjl is sin(theta_ijl)**2
                                           dcjl       = 1.0_DP - dot_ij_jl**2
                                           abs_dc     = sqrt( dcik*dcjl )

#ifdef DIHEDRAL_SIN_CUTOFF
                                           if (dcjl > this%dh_sin1) then
                                              call trig_on(this%dh_sin1, this%dh_sin2, dcjl, fcsjl, dfcsjl)
#endif
                                              ...
                                              bdhij    = 1-costijkl**2
#ifdef DIHEDRAL_SIN_CUTOFF
                                              bdh      = bdh + bdhij*fcik*fcjl*fcsik*fcsjl
#else
                                              bdh      = bdh + bdhij*fcik*fcjl
#endif
                                              bdhij    = bdhij*tij_faij_fcij/2

                                              dbdhij   = -2*costijkl*tij_faij_fcij*fcik*fcjl/2
#ifdef DIHEDRAL_SIN_CUTOFF
                                              dbdhij   = dbdhij*fcsik*fcsjl
#endif
...
#ifdef DIHEDRAL_SIN_CUTOFF
                                              df = df - &
                                                   bdhij*fcik*fcjl*2*(dfcsik*fcsjl*dot_ij_ik*(rnik-dot_ij_ik*rnij) + &
                                                                      fcsik*dfcsjl*dot_ij_jl*(rnjl-dot_ij_jl*rnij)) / rlij
#endif
...
#ifndef DIHEDRAL_SIN_CUTOFF
                                                   + bdhij*dfcikr*fcjl * rnik
#else
                                                   + bdhij*dfcikr*fcjl*fcsik*fcsjl * rnik &
                                                   - bdhij*fcik*fcjl*dfcsik*fcsjl * 2*dot_ij_ik*(rnij-dot_ij_ik*rnik)/rlik
#endif
...
#else
                                                   + bdhij*fcik*dfcjlr*fcsik*fcsjl * rnjl &
                                                   - bdhij*fcik*fcjl*fcsik*dfcsjl * 2*dot_ij_jl*(rnij-dot_ij_jl*rnjl)/rljl
#endif
                                                       ...
                                                       jl2_loop: do jl2 = jl1+1, this%neb_last(j)
                                                          ...
                                                          jl2_neq_i_k: if ( &
                                                             ...
                                                             jl2_within_cutoff: if (rljl2 < this%cut_bo_h(jlpot)) then
                                                                ...
                                                                dbdhij         = -2*costijkl*tij_faij_fcij*fcik1*fcik2*fcjl1*fcjl2/2
                   ...
                   if (ijpot == C_C) then
                      ...
                      call eval(this%Fsisi, nti, ntj, nconji, nconjj, &
                           fij, dfdni, dfdnj, dfdncni, dfdncnj)

                   else if (ijpot == Si_C) then

                      call eval(this%Fsic, nti, ntj, nconji, nconjj, &
                           fij, dfdni, dfdnj, dfdncni, dfdncnj)

                   else if (ijpot == Si_H) then

                      call eval(this%Fsih, nti, ntj, nconji, nconjj, &
                           fij, dfdni, dfdnj, dfdncni, dfdncnj)

                   else if (ijpot == C_Si) then
                   ...
                   if (ijpot == C_C) then
                      ...
                      call eval(this%Fsisi, nti, ntj, nconj, &
                           fij, dfdni, dfdnj, dfdncn)

                   else if (ijpot == Si_C) then

                      call eval(this%Fsic, nti, ntj, nconj, fij, &
                           dfdni, dfdnj, dfdncn)

                   else if (ijpot == Si_H) then

                      call eval(this%Fsih, nti, ntj, nconj, fij, &
                           dfdni, dfdnj, dfdncn)
...
#ifndef LAMMPS
                   if (present(piRC_per_bond)) then
                      piRC_per_bond(this%nbb(ij)) = fij
                   endif
#endif
...
!                   write (*, '(A,2I3,4F15.7)')  "     F "//rebo2_el_strs(ktypi)//" "//rebo2_el_strs(ktypj)//":", i, j, nti, ntj, nconj, fij
...
#ifdef SEP_NCONJ
                   dfdncni = dfdncni + dtdncni * bdh
                   dfdncnj = dfdncnj + dtdncnj * bdh
#else
                   ...
                   do ikc = 1, numnbi
                      k    = nebofi(ikc)
! FIXME: Why is below commented out?
#ifdef LAMMPS
                      if (k /= j) then
#else
                      kdc  = dcofi(ikc)
                      if (k /= j .or. kdc /= jdc) then
                         ...
                         df         = &
                              ...
                              dpdnsii * dnidik(1:3, ikc, rebo2_Si_) + &
               ...
               do jlc = 1, numnbj
                  ...
                  df         = &
                           ...
                           dpdnsij * dnjdjl(1:3, jlc, rebo2_Si_) &
                           )
                      VEC3(f, l) = VEC3(f, l) + df
                      fj         = fj         - df
...
#ifndef LAMMPS
                   if (present(baveij_per_bond)) then
                      baveij_per_bond(this%nbb(ij)) = baveij
                   endif
#endif
                   ...
                   do ikc = 1, numnbi
                      ...
                      if (k /= j .or. kdc /= jdc) then
                         ...
                         !
                            ...
                            else if (ktyp(k) == rebo2_Si_) then
                               dffac = dpdnsii * dfbij + dfdni + dfdncni*fxik(ikc)
                            else
                      ...
                      if (i1 <= i2) then
                         ...
                         if (ktyp(l) == rebo2_C_) then
                            dffac = dpdncj * dfbji + dfdnj + &
                                 dfdncnj*fxjl(jlc)
                         else if (ktyp(l) == rebo2_Si_) then
                            dffac = dpdnsij * dfbji + dfdnj + &
                                 dfdncnj*fxjl(jlc)
                         else
