...
#define TOO_SMALL(what, i, ierror)  RAISE_DELAYED_ERROR("Internal neighbor list exhausted, *" // what // "* too small: " // "nebtot = " // nebtot // "/" // nebsize // ", nebmax = " // nebmax // ", nebavg = " // nebavg // ", this%neb_last(i)-this%neb_seed(i)+1 = " // (this%neb_last(i)-this%neb_seed(i)+1) // ", i = " // i // "/" // natloc // " (" // nat // ")", ierror) ; nebtot = 1 ; this%neb_last(i) = this%neb_seed(i)
  ...
  recursive subroutine BOP_KERNEL( &
    ...
    real(DP)  :: bdh, bdhij, dbdhij, tij, tije, dtdni, dtdnj
#ifdef SEP_NCONJ
    ...
    real(DP)  :: dnidik(3, nebmax, typemax), dnidfcik(nebmax, typemax)
    real(DP)  :: dnjdjl(3, nebmax, typemax), dnjdfcjl(nebmax, typemax)
    real(DP)  :: nti, ntj
    ...
    integer  :: numnbi,numnbj,numnbk(nebmax),numnbl(nebmax)
    ...
    !$omp& private(bdh, bdhij, dbdhij, tij, tije, dtdni, dtdnj) &
#ifdef SEP_NCONJ
...
#ifdef NUM_NEIGHBORS
    !$omp& private(ni, nj) &
    !$omp& private(dnidik, dnidfcik) &
    !$omp& private(dnjdjl, dnjdfcjl) &
    !$omp& private(nti, ntj) &
#ifndef SEP_NCONJ
...
#ifdef SIN_S
                                  fac       = PI/(2*this%dC(ijpot)) * &
                                       sin(PI*(C-Cmin)/this%dC(ijpot)) / csij
#else
                                  fac       = 2*Cmax_C*this%dC(ijpot)/ &
                                       (C_Cmin**3)
#endif
                   ...
                   if ((screened .or. sij < this%screening_threshold) .and. &
                      ...
                      if ( screened ) then
                         ...
                         this%cutdrboik(ineb:snebtot-1) = &
                              this%cutdrik(ineb:snebtot-1)*sij*fcboij * &
                              (1.0_DP-fcinij)
                         this%cutdrbojk(ineb:snebtot-1) = &
                              this%cutdrjk(ineb:snebtot-1)*sij*fcboij * &
                              (1.0_DP-fcinij)
...
!                   if (ni(rebo2_Si_) > 7.0_DP) ni(rebo2_Si_) = 7.0_DP
                   if (ni(rebo2_Si_) > 4.0_DP) ni(rebo2_Si_) = 4.0_DP
...
!                   if (nj(rebo2_Si_) > 7.0_DP) nj(rebo2_Si_) = 7.0_DP
                   if (nj(rebo2_Si_) > 4.0_DP) nj(rebo2_Si_) = 4.0_DP
                   ...
                   ik_loop: do ik = istart, ifinsh
                      ...
                      ik_neq_ij: if (ik /= ij) then
                         ...
                         ! FIXME!!! Generalize
                         ktypk_eq_C_or_Si: if (ktypk == rebo2_C_ .or. &
                              ktypk == rebo2_Si_) then
                            ...
                            xik   = this%nn(rebo2_H_, k) + this%nn(rebo2_C_, k) + &
                                 this%nn(rebo2_Si_, k) - fcik
                            xikdk = &
...
!                   write (*, '(A,2I3,4F15.7)')  "(ij) P "//rebo2_el_strs(ktypi)//" "//rebo2_el_strs(ktypj)//":", i, j, ni(rebo2_H_), ni(rebo2_C_), ni(rebo2_Si_), pij
                   ...
                   jl_loop: do jl = this%neb_seed(j), this%neb_last(j)
                      ...
                      l_neq_i: if (l /= i .or. ldc /= 0) then
                         ...
                         ! FIXME!!! Generalize
                         ktypl_eq_C_or_Si: if (ktypl == rebo2_C_ .or. &
                              ktypl == rebo2_Si_) then
...
!                   write (*, '(A,2I3,4F15.7)')  "(ji) P "//rebo2_el_strs(ktypj)//" "//rebo2_el_strs(ktypi)//":", j, i, nj(rebo2_H_), nj(rebo2_C_), nj(rebo2_Si_), pji
                   ...
                   if (ijpot == C_C) then
                      ...
                      !call eval(this%Fsisi, nti+eps, ntj, nconj, &
                      !     fipj, dfdni, dfdnj, dfdncn)
                      !call eval(this%Fsisi, nti-eps, ntj, nconj, &
                      !     fimj, dfdni, dfdnj, dfdncn)

                      !call eval(this%Fsisi, nti, ntj+eps, nconj, &
                      !     fijp, dfdni, dfdnj, dfdncn)
                      !call eval(this%Fsisi, nti, ntj-eps, nconj, &
                      !     fijm, dfdni, dfdnj, dfdncn)

                      call eval(this%Fsisi, nti, ntj, nconj, &
                           fij, dfdni, dfdnj, dfdncn)

                      !if (abs((fipj-fimj)/(2*eps)-dfdni) > eps) then
                      !   write (*, *)  (fipj-fimj)/(2*eps), dfdni, abs((fipj-fimj)/(2*eps)-dfdni)
                      !endif
                      !if (abs((fijp-fijm)/(2*eps)-dfdnj) > eps) then
                      !   write (*, *)  (fijp-fijm)/(2*eps), dfdnj, abs((fijp-fijm)/(2*eps)-dfdnj)
                      !endif
