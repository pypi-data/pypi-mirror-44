...
#define table4d_prlog table3d_prlog
  ...
  subroutine rebo2coh_db_init_with_parameters( &
       ...
       in_Pcc,   in_dPccdi,   in_dPccdj,   &
       in_Pcc_o, in_dPcc_odi, in_dPcc_odj, &
       in_Pch,   in_dPchdi,   in_dPchdj,   &
       in_Pch_o, in_dPch_odi, in_dPch_odj, &
       in_Phh,   in_dPhhdi,   in_dPhhdj,   &
       in_Poc,   in_dPocdi,   in_dPocdj,   &
       in_Pco,   in_dPcodi,   in_dPcodj,   &
       in_Poh,   in_dPohdi,   in_dPohdj,   &
       in_Pho,   in_dPhodi,   in_dPhodj,   &
       in_Poo,   in_dPoodi,   in_dPoodj,   &
    ...
    real(DP), intent(in)          :: in_Pcc(0:, 0:)
    real(DP), intent(in)          :: in_dPccdi(0:, 0:)
    real(DP), intent(in)          :: in_dPccdj(0:, 0:)
    real(DP), intent(in)          :: in_Pcc_o(0:, 0:)
    real(DP), intent(in)          :: in_dPcc_odi(0:, 0:)
    real(DP), intent(in)          :: in_dPcc_odj(0:, 0:)
    real(DP), intent(in)          :: in_Pch(0:, 0:)
    real(DP), intent(in)          :: in_dPchdi(0:, 0:)
    real(DP), intent(in)          :: in_dPchdj(0:, 0:)
    real(DP), intent(in)          :: in_Pch_o(0:, 0:)
    real(DP), intent(in)          :: in_dPch_odi(0:, 0:)
    real(DP), intent(in)          :: in_dPch_odj(0:, 0:)
    real(DP), intent(in)          :: in_Phh(0:, 0:)
    real(DP), intent(in)          :: in_dPhhdi(0:, 0:)
    real(DP), intent(in)          :: in_dPhhdj(0:, 0:)
    real(DP), intent(in)          :: in_Poc(0:, 0:)
    real(DP), intent(in)          :: in_dPocdi(0:, 0:)
    real(DP), intent(in)          :: in_dPocdj(0:, 0:)
    real(DP), intent(in)          :: in_Pco(0:, 0:)
    real(DP), intent(in)          :: in_dPcodi(0:, 0:)
    real(DP), intent(in)          :: in_dPcodj(0:, 0:)
    real(DP), intent(in)          :: in_Poh(0:, 0:)
    real(DP), intent(in)          :: in_dPohdi(0:, 0:)
    real(DP), intent(in)          :: in_dPohdj(0:, 0:)
    real(DP), intent(in)          :: in_Pho(0:, 0:)
    real(DP), intent(in)          :: in_dPhodi(0:, 0:)
    real(DP), intent(in)          :: in_dPhodj(0:, 0:)
    real(DP), intent(in)          :: in_Poo(0:, 0:)
    real(DP), intent(in)          :: in_dPoodi(0:, 0:)
    real(DP), intent(in)          :: in_dPoodj(0:, 0:)
    ...
    if (ilog /= -1) then
       ...
       call prlog("     Parameters for oxygen angular function:")
       write (ilog, '(5X,A,F20.10)')  "oo_g_a0     = ", this%oo_g_a0
       write (ilog, '(5X,A,F20.10)')  "oo_g_a1     = ", this%oo_g_a1
       write (ilog, '(5X,A,F20.10)')  "oo_g_a2     = ", this%oo_g_a2
...
!   Need to add in the Silicon penalty functions - MTK
    ...
    call init(this%Pcc, N_NHC_NO, in_Pcc, in_dPccdi, in_dPccdj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Pcc:")
    call table2d_prlog(this%Pcc, indent=5)
    call init(this%Pcc_o, N_NHC_NO, in_Pcc_o, in_dPcc_odi, in_dPcc_odj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Pcc_o:")
    call table2d_prlog(this%Pcc_o, indent=5)
    call init(this%Pch, N_NHC_NO, in_Pch, in_dPchdi, in_dPchdj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Pch:")
    call table2d_prlog(this%Pch, indent=5)
    call init(this%Pch_o, N_NHC_NO, in_Pch_o, in_dPch_odi, in_dPch_odj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Pch_o:")
    call table2d_prlog(this%Pch_o, indent=5)
    call init(this%Phh, N_NHC_NO, in_Phh, in_dPhhdi, in_dPhhdj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Phh:")
    call table2d_prlog(this%Phh, indent=5)
    call init(this%Poc, N_NHC_NO, in_Poc, in_dPocdi, in_dPocdj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Poc:")
    call table2d_prlog(this%Poc, indent=5)
    call init(this%Pco, N_NHC_NO, in_Pco, in_dPcodi, in_dPcodj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Pco:")
    call table2d_prlog(this%Pco, indent=5)
    call init(this%Poh, N_NHC_NO, in_Poh, in_dPohdi, in_dPohdj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Poh:")
    call table2d_prlog(this%Poh, indent=5)
    call init(this%Pho, N_NHC_NO, in_Pho, in_dPhodi, in_dPhodj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Pho:")
    call table2d_prlog(this%Pho, indent=5)
    call init(this%Poo, N_NHC_NO, in_Poo, in_dPoodi, in_dPoodj, &
         error=error)
    PASS_ERROR(error)
    call prlog("     Poo:")
    call table2d_prlog(this%Poo, indent=5)
