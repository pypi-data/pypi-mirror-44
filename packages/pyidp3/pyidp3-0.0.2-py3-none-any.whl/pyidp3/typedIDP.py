...
class IDP(object):
    ...
    def __str__(self):
        ...
        full IDP script. See the generateScript method for an example.
    ...
    def minimizeScript(self, termblock):
        """
    ...
    def refresh(self):
        ...
        if __debug__:
            print("SENT TO IDP:")
            print(self.generateScript())
            print("END OF IDP INPUT")
            f = open('IDPscript.txt', 'w')
            f.write(self.generateScript())
            f.close()
            print("SENT TO IDP:")
        idp = Popen(self.executable, stdin=PIPE, stdout=PIPE, stderr=PIPE)
        # input is the generated idp file
        out, err = idp.communicate(input=(self.generateScript()).encode())
        out = out.decode()
    ...
    def minimize(self, term):
        ...
        script = self.minimizeScript(termblock)
    ...
    # a constant string to define a mainblock to search for a model

    # a constant string to define a mainblock to search for minimization
