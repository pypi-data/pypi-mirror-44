...
class DataType(Enum):
    ...
    RawLineTracer               = 0x32      # 라인트레이서 데이터 RAW 값
    RawCard                     = 0x33      # 카드 데이터 RAW 값
    RawCardList                 = 0x34      # 카드 리스트 데이터 RAW 값
    RawCardFunctionList         = 0x35      # 카드 함수 리스트 데이터 RAW 값
...
class CommandType(Enum):
    ...
    # Navigation
    NavigationTargetClear   = 0xD0      # 네비게이션 목표점 초기화
    NavigationStart         = 0xD1      # 네비게이션 시작(처음부터)
    NavigationPause         = 0xD2      # 네비게이션 일시 정지
    NavigationRestart       = 0xD3      # 네비게이션 다시 시작(일시 정지 후 다시 시작할 때 사용)
    NavigationStop          = 0xD4      # 네비게이션 중단
    NavigationNext          = 0xD5      # 네비게이션 목표점을 다음으로 변경
    NavigationReturnToHome  = 0xD6      # 시작 위치로 귀환
