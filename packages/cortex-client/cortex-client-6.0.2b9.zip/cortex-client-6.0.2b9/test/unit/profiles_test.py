...
class TestAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        register_entry_from_path(
            Entry.GET,
            build_mock_url(ProfilesClient.build_get_profile_uri(self.profile_id, self.schema_id, historic=False),
                           version=3),
        ...
        register_entry_from_path(
            Entry.GET,
            build_mock_url(ProfilesClient.build_get_profile_uri(self.profile_id, self.schema_id, historic=True),
                           version=3),
    ...
    def test_get_historic_profile_with_builder_pattern(self):
        profile = self.cortex.profile(self.profile_id, self.schema_id).historic()
        self.assertIsNotNone(profile)
        self.assertEqual(profile.profileId, self.profile_id)
