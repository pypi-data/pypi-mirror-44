...
class Client(object):
    ...
    def profile(self, profileId: str, schemaId: str, version:int=3) -> Profile:
        profile_client = ProfilesClient(self._url, version, self._token.token)
        return Profile.get_profile(profileId, schemaId, profile_client)

    def schema(self, schemaId:str, version:int=3) -> ProfileSchema:
        profile_client = ProfilesClient(self._url, version, self._token.token)
