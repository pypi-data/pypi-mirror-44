...
from cortex.profile.utils import head, dicts_to_classes, dict_to_attr_class, construct_attr_class_from_dict, \
...
class ProfilesClient(_Client):
    ...
    def _get_profile(self, profileId:str, schemaId:Optional[str], historic:Optional[bool]=False, version:Optional[str]=None) -> List[dict]:
        ...
        url_with_args = "{}?{}".format(url, urllib.parse.urlencode(url_args))
        raw_profiles = self._get_json(url_with_args)
