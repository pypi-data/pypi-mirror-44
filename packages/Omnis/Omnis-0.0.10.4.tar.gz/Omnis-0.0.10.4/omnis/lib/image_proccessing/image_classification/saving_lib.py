...
def _serialize_model(model, h5dict, include_optimizer=True, class_indices = None):
    ...
    for key, values in class_indices.items():
        class_indices_list.append(key)
    h5dict['class_indices'] = class_indices_list
...
def _deserialize_model(h5dict, custom_objects=None, compile=True):
    ...
    class_list = h5dict['class_indices']
    class_indices = dict()
    for i in range(len(class_list)):
        class_name = class_list[i].decode('utf-8')
...
@allow_write_to_gcs
def save_model(model, filepath, overwrite=True, include_optimizer=True, class_indices = None):
    ...
    try:
        _serialize_model(model, h5dict, include_optimizer, class_indices=class_indices)
    finally:
        if opened_new_file:
            h5dict.close()
