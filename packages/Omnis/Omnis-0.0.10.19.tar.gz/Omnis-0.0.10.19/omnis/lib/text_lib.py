...
def parse_sentences_and_next_characters(whole_text, max_sentence_size, interval_between_each_sentence):
    """Extracts sentences from whole_text according to max sentence size and interval between each sentence.
    ...
    Arguments:
        ...
        interval_between_each_sentence {int} --
            ...
            then sentences will be ['hello', 'ello ', 'llo w', 'lo wo', 'o wor', ' worl', 'world'].
            or interval_between_each_sentence is 2,
            then sentences will be ['hello', 'llo w', 'o wor', 'world'].
    
    Returns:
        [list, list] -- A list of sentences and a list of next characters will be returned.
    """
    sentences = []
    next_characters = []
...
def get_words_from_text(text):
    word_list = re.sub("[^\w]", " ", text).split() # we will lower all words
    return word_list