...
class Style_Transfer(Application):
    ...
    def prepare_train_data(self, get_content_image_from = 'directory', content_image_path = None, get_style_image_as = 'filepath', style_image_path = None):
        assert get_content_image_from in ["directory", "argument"], "get_content_image_from should be either 'directory' or 'argument'."
        assert get_style_image_as in ["filepath", "ndarray"], "get_style_image_as should be either 'filepath' or 'ndarray'."
    ...
    def generate(self, iterations = 10, output_type = "file", output_path = "result", total_variation_weight=1.0, style_weight=1.0, content_weight=0.025):
