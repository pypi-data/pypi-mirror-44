...
class Style_Transfer_Worker(Process):
    def __init__(self, gpuid, queue, get_content_image_from, get_style_image_as, content_image_path = None, style_image_path = None, content_image_array = None, style_image_array = None, content_weight = 0.025, use_deepblock_log = False):
    ...
    def create_model(self, img_nrows, img_ncols, content_image_path, child):
        ...
        if self._get_content_image_from == 'directory':
            x = self.preprocess_image(os.path.join(content_image_path, child), img_nrows, img_ncols, model_type)
        elif self._get_content_image_from == 'argument':
