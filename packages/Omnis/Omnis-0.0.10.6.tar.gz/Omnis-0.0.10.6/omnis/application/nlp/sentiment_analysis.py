...
class Sentiment_Analysis(Application):
    ...
    def prepare_train_data(self, sentiment_texts, sentiment_values, skip_top, num_of_words_to_consider, max_num_of_words = 150):
        ...
        if hasattr(self, 'model') == False:
            ...
            self.model = self.create_model(input_shape, len(input_dictionary), num_classes )
            self.set_input_dictionary(input_dictionary)
        x = self.init_input_data(sentiment_texts)
        y = keras.utils.to_categorical(np.asarray(sentiment_values), num_classes = self.model.output_shape[1])
        return x, y

    def create_model(self, input_shape, input_dim, num_classes):
        input_layer = Input(shape = input_shape)
        embedding1 = Embedding(input_dim, 128, input_length = input_shape[0])(input_layer)
