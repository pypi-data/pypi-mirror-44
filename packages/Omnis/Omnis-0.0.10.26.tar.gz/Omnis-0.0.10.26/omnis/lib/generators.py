...
class Predict_Generator(keras.utils.Sequence):
    def __init__(self, image_filenames = None, image_classes = None, batch_size=None, image_shape=None, shuffle = False):
...
class Train_Generator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        batch_x = self.image_filenames[idx * self.batch_size:(idx + 1) * self.batch_size]
        class_x = self.image_classes[idx * self.batch_size:(idx + 1) * self.batch_size]
    ...
    def flow_from_csv(self, csv_file_path, openimage_directory_path, shuffle = False, target_size = None, batch_size = 1):
        ...
        for line in csv_reader:
            atoms = line
            file_full_path = os.path.join(openimage_directory_path, atoms[0]+'.jpg')
