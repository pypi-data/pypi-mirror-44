...
from ....lib.image_lib import merge_image, get_image_names_in_directory, split_one_image
...
from ....lib.config_lib import Config_Deblur
...
class Deblur_GAN(Application):
    ...
    def prepare_train_data(self, data_path, optimizer = 'adam', loss=[perceptual_loss, wasserstein_loss], metrics=['accuracy']):
    ...
    def train(self, batch_size=1, epochs = 1, critic_updates = 5):
        data_blur = self.load_images_data(self.blur_data_path, batch_size)
        data_sharp = self.load_images_data(self.sharp_data_path, batch_size)
        ...
        for epoch in range(0, epochs):
            ...
            for index in range(int(blur_image_array.shape[0] / batch_size)):
                ...
                d_on_g_loss = self.d_on_g.train_on_batch(image_blur_batch, [image_full_batch, output_true_batch])
    ...
    def predict(self, data_path, batch_size = 1, saved_path = 'results'):
        ...
        generated_images_array = np.array([self.deprocess_image(img) for img in generated_images])
    ...
    def compile_model(self, optimizer, loss=[perceptual_loss, wasserstein_loss], metrics=['accuracy']):
        if optimizer == 'adam':
        ...
        self.d_model.trainable = False
        loss_weights = [100, 1]
        self.d_on_g.compile(optimizer=d_on_g_opt, loss=loss, loss_weights=loss_weights)
        self.d_model.trainable = True
    ...
    def create_model(self):
        ...
        self.d_on_g = generator_containing_discriminator_multiple_outputs(self.g_model, self.d_model)
    ...
    def load_images_data(self, img_path):
        if n_images < 0:
            n_images = float("inf")
        ...
        if os.path.isfile(img_path):
            ...
            all_image_paths = get_image_names_in_directory(img_path)
        else:
    ...
    def load_predict_images_data(self, path):
        ...
        for path_image in all_image_paths:
            img = cv2.imread(path_image, cv2.IMREAD_COLOR)
            split_images, area_index = split_one_image(img, Config_Deblur.CELL_SHAPE[0], Config_Deblur.CELL_SHAPE[1])
    ...
    def preprocess_image(self, img):
        resized_img = cv2.resize(img, Config_Deblur.CELL_SHAPE[:2])
