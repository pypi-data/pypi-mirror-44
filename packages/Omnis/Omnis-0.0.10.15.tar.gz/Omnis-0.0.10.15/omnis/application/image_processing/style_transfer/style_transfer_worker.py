...
class Style_Transfer_Worker(Process):
    def __init__(self, gpuid, queue, get_content_image_from, get_style_image_as, content_image_path = None, style_image_path = None, content_image_array = None, style_image_array = None, content_weight = 0.025):
    ...
    def generate_one_image(self, directory_path, child):
        ...
        if self._get_content_image_from == 'directory':
            width, height = load_img(directory_path + "/" + child).size
        ...
        x, model = self.create_model(img_nrows, img_ncols, directory_path, child)
        ...
        for i in range(self.iterations):
            ...
            end_time = time.time()
            print(json.dumps(logJson, ensure_ascii=False, indent="\t"))
        ...
        if self.output_type == 'array':
            ...
            fname = os.getcwd() + "/" + self.output_path + "/" + child
    ...
    def create_model(self, img_nrows, img_ncols, content_image_path, child):
        ...
        if self._get_content_image_from == 'directory':
            x = self.preprocess_image(content_image_path + "/" + child, img_nrows, img_ncols, model_type)
        elif self._get_content_image_from == 'argument':
            x = self.preprocess_image(child, img_nrows, img_ncols, model_type)
        content_image = K.variable( x )
        style_image_tensor = K.variable( self.preprocess_image(self._style_image, img_nrows, img_ncols, model_type) )
        ...
        input_tensor = K.concatenate([content_image, style_image_tensor, self.combination_image], axis=0)
