...
class MyTestCase(unittest.TestCase):
    def test_something(self):
        cgroup = CGroup('test', 'cpuset', auto_delete=True, auto_delete_flag=DeleteFlag.RECURSIVE)
        self.assertIsNotNone(cgroup)
        for key, val in cgroup.get_all_from('cpuset'):
            print(key, val)

        cgroup.add_current_thread()
        cgroup.add_processes(4765, 4831, 31828)
        ...
        cgroup.remove_processes(4765, 4831, 31828)
