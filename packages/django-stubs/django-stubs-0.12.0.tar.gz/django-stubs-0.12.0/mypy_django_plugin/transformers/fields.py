...
def get_django_project() -> 'DjangoProject':
    import os

    if 'DJANGO_SETTINGS_MODULE' not in os.environ:
        raise KeyError("'DJANGO_SETTINGS_MODULE' not present in the environment.")
...
class DjangoProject:
    def __init__(self, installed_apps: Sequence[str]):
        self._apps = None  # type: Optional[Apps]
        self._installed_apps = installed_apps

    def get_apps(self) -> Apps:
        if not self._apps:
            self._apps = Apps(installed_apps=self._installed_apps)

        return self._apps


def evaluate_related_manager_names_for_outer_model_class(ctx: FunctionContext) -> None:
    related_name = helpers.get_argument_by_name(ctx, 'related_name')
    if not isinstance(related_name, StrExpr):
        return None

    outer_model = ctx.api.scope.active_class()  # type: Optional[TypeInfo]
    if not outer_model:
        return None
    ...
    # helpers.get_related_managers_metadata(referred_to_type.type)[related_name.value] = {
    #     'manager': helpers.RELATED_MANAGER_CLASS_FULLNAME,
    #     'of': [outer_model.fullname()]
    # }
    # referred_to_type.type.metadata
    # helpers.get_related_managers_metadata(outer_model)[related_name] = {
    #     ''
    # }
    # # related_name = helpers.evaluate_text_placeholders(related_name.value,
    #                                                   class_name=outer_model.name())
    # if 'DJANGO_SETTINGS_MODULE' in os.environ:
    #     proj = DjangoProject()
    #     print()
    # proj = DjangoProject()
    # print(proj.get_apps().app_configs)
    # print('proj.get_apps()', proj.get_apps().get_model('myapp', 'Publisher'))


def adjust_return_type_of_field_instantiation(ctx: FunctionContext) -> Type:
    record_field_properties_into_outer_model_class(ctx)

    # if helpers.is_foreign_key_like(ctx.default_return_type):
    #     evaluate_related_manager_names_for_outer_model_class(ctx)
