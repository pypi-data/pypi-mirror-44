...
def get_model_related_managers(model: TypeInfo) -> Dict[str, typing.Any]:
    raw_related_managers = get_related_managers_metadata(model)
    class_name = model.defn.name

    related_managers_metadata = {}
    for manager_name, metadata in raw_related_managers.items():
        manager_name = evaluate_text_placeholders(manager_name,
                                                  class_name=class_name)
        related_managers_metadata[manager_name] = metadata

    return related_managers_metadata
...
def get_related_manager_type_from_metadata(model_info: TypeInfo, related_manager_name: str,
    ...
    related_manager_metadata = get_model_related_managers(model_info)
...
def evaluate_text_placeholders(related_name: str, *, class_name: str) -> str:
    """ Replace %(class)s and %(app_label)s with corresponding values"""
    return related_name.replace('%(class)s', class_name.lower())
