...
from mypy_django_plugin.transformers.models import process_model_class, add_get_set_attr_fallback_to_any
...
def transform_model_class(ctx: ClassDefContext, ignore_missing_model_attributes: bool) -> None:
    ...
    if ignore_missing_model_attributes:
...
def get_class_fullname(model_cls: type) -> str:
    return model_cls.__module__ + '.' + model_cls.__qualname__


def process_model_attr_name(ctx: AttributeContext, attr_name: str) -> Type:
    from mypy_django_plugin.transformers.fields import DjangoProject

    model_type = ctx.type
    if not isinstance(model_type, Instance):
        return ctx.default_attr_type

    model_fullname = model_type.type.fullname()
    model_package, model_name = model_fullname.rsplit('.', maxsplit=1)
    app_name = model_package.split('.models', maxsplit=1)[0]

    proj = DjangoProject()
    model_cls = proj.get_apps().get_model(app_name, model_name)

    # check in reverse relations
    rel_object = model_cls._meta.fields_map.get(attr_name)
    if rel_object:
        if isinstance(rel_object, ManyToOneRel):
            related_model_fullname = get_class_fullname(rel_object.related_model)
            return ctx.api.named_generic_type(helpers.RELATED_MANAGER_CLASS_FULLNAME,
                                              [ctx.api.named_generic_type(related_model_fullname, [])])
...
class DjangoPlugin(Plugin):
    ...
    def get_attribute_hook(self, fullname: str
        ...
        if class_name in self._get_current_model_bases():
            # print(class_name, attr_name)
            return partial(process_model_attr_name,
                           attr_name=attr_name)
            ...
            model_info = self._get_typeinfo_or_none(class_name)
            if model_info:
                related_managers = helpers.get_model_related_managers(model_info)
