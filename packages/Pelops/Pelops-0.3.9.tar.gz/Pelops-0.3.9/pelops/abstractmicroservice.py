...
class AbstractMicroservice:
    ...
    def __init__(self, config, config_class_root_node_name,
        ...
        if manage_monitoring_agent:
            if "monitoring-agent" in config:
                log_filter = mylogger.MyMQTTFilter(mqtt_logger_name=self._mqtt_client._logger.name,
                                                   min_log_level="WARNING")
                monitoring_logger = pelops.mylogger.get_child(self._logger, MonitoringAgent.__class__.__name__)
                self._monitoring_agent = MonitoringAgent(config["monitoring-agent"], self, self._mqtt_client,
                                                         monitoring_logger, log_filter, self._logger)
    ...
    def _cmd_debug_threads(self, args):
        ...
        for t in threading.enumerate():
            text += " - {} (alive: {}, daemon: {})\n".format(t.name, t.is_alive(), t.daemon)
