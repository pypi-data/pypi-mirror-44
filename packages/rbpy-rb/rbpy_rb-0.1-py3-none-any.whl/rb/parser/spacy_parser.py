...
from rb.utils.downloader import download_spacy_model
...
class SpacyParser:
    ...
    def get_model(self, lang: Lang) -> Language:
        if lang not in self.loaded_models:
            folder = "resources/{}/spacy/{}".format(lang.value, models[lang])
            for model in [models[lang], folder]:
                try:
                    self.loaded_models[lang] = spacy.load(model)
                    break
                except:
                    pass
            else:
                if not download_spacy_model(lang, models[lang]):
                    raise FileNotFoundError("Spacy model not found")
