...
class publisher:
    ...
    def __init__(self, topic, node = "default", msg_type = "json", conf =  {'transport': "ZMQ", 'network': "broker", 'mode':"many2many"}, debug = False):
        ...
        if self.transport ==  "ROS":                                                    #Use ROS
            ...
            print ("PUB: New publisher using nanomsg, " + self.network)
            #many2many is the default value
            self.mode = "many2many" 
            if "mode" in self.conf:
                self.mode = self.conf['mode']
    ...
    def __network_selection(self):
        ...
        if self.network == "direct":
            ...
            if self.topic != "/nep_node":
                print("PUB: Waiting for NEP master ...")
            # Register the topic in the NEP Master and get the port and ip
            success, port, ip  = nep.masterRegister(self.node, self.topic, master_ip = '127.0.0.1', master_port = 7000, socket = "publisher", mode = self.mode)
            if self.topic != "/nep_node":
                print("PUB: Socket ready")
