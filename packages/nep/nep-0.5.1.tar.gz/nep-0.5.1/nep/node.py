...
class node:
        ...
        while not exit:
            ...
            if s == True:
                ...
                try:
                    ...
                    print ("NEP ERROR: when killing process")
                    pass
    ...
    def __init__(self, node_name, transport = "ZMQ", exit_thread = True):
        ...
        print ("New NEP node: " + self.node_name + ", pid: " + str(self.pid))
    ...
    def new_client(self, topic):
        ...
        print("CLIENT: Waiting for NEP master ...")
        s, port, ip  = nep.masterRegister(self.node_name, topic, master_ip = '127.0.0.1', master_port = 7000, socket = "client", self.pid)

        if s:
            client = nep.client(ip, port, debug = True)
            print("CLIENT: Socket ready")
            return client
        else:
            print ("NEP ERROR: client socket not connected")
    ...
    def new_server(self, topic):
        ...
        print("SERVER: Waiting for NEP master ...")
        s, port, ip  = nep.masterRegister(self.node_name, topic, master_ip = '127.0.0.1', master_port = 7000, socket = "server", self.pid)
        if s:
            server = nep.server(ip,port, debug = True) #Create a new server instance
            print("SERVER: Socket ready")
            return server
        else:
            print ("NEP ERROR: server socket not connected")
