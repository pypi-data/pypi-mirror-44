...
class subscriber:
    ...
    def __init__(self, topic, node = "default", msg_type = "dict", conf =  {'transport': "ZMQ", 'network': "broker", 'mode':"many2many"}, callback = "", debug = False):
        ...
        if self.transport ==  "NN" or self.transport ==  "nanomsg":          #Use nanomsg
            if self.topic != "/nep_node": 
                print ("SUB: New subscriber using nanomsg: " + self.network)
    ...
    def __network_selection(self):
        ...
        if self.network == "direct":
            ...
            if self.topic != "/nep_node":
                print("SUB: Waiting for NEP master ...")
            # Register the topic in the NEP Master and get the port and ip
            success, port, ip  = nep.masterRegister(self.node, self.topic, master_ip = '127.0.0.1', master_port = 7000, socket = "subscriber", mode = self.mode)
            if self.topic != "/nep_node":
                print("SUB: Socket ready")
