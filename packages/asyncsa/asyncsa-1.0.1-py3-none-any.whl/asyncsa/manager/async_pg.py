...
from async_sqlalchemy.connection.async_pg import AsyncPg
...
class PostgresManager(BaseManager):
    ...
    async def set(self, query=None, model=None):
        print(type(query))
        sql_string = self.format_query(query=query)
        print(sql_string)
    ...
    async def instance(self, instance=None, model=None, q_type=None):
        values = {k: v for k,
        ...
        if q_type == 'add':
            ...
            query = model.__table__.update().where(
                ...
                **values)
        return await self.set(query=query)
