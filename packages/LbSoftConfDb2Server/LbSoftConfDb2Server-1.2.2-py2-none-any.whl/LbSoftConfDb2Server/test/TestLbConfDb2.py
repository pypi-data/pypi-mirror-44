...
class TestLbConfDb2(unittest.TestCase):
    ...
    def test_getOrCreateProjectNode(self):
        res = self.db.getOrCreateProjectNode('My_First_project',
        ...
        res = self.db.getOrCreateProjectNode('My_First_project',
                                             sourceuri='gitlab:myfirstproject')
        self.assertEqual(len(self.db.listProjects()), 1)
        # Add the same node with different source_uri, but with same key
        res = self.db.getOrCreateProjectNode('My_First_project',
                                             sourceuri='gitlab:myfirstproject2')
        self.assertEqual(res.name, 'MY_FIRST_PROJECT')
        self.assertEqual(res.sourceuri, 'gitlab:myfirstproject2')
        self.assertEqual(len(self.db.listProjects()), 1)

        # Add a seconde node
        res = self.db.getOrCreateProjectNode('My_Second_project',
        ...
        self.assertEqual(sorted([
            ('MY_FIRST_PROJECT', 'gitlab:myfirstproject2'),
            ('MY_SECOND_PROJECT', 'gitlab:mysecondprj')]), sorted(res))

    def test_getProjectNode(self):
        self.db.getOrCreateProjectNode('My_First_project',
                                       sourceuri='gitlab:myfirstproject')
    ...
    def test_getOrCreatePV(self):
        self.db.getOrCreatePV('a', 'v1r0')
        self.db.getOrCreatePV('a', 'v1r1')
        self.db.getOrCreatePV('b', 'v1r0')
        self.db.getOrCreatePV('c', 'v1r0')
        ...
        res = self.db.getOrCreatePVSerializable('c', 'v1r0')
        self.assertEqual('C_v1r0', res)

    def test_getOrCreatePVPattern(self):
        d1 = self.db.getOrCreatePV("DAVINCI", "v50r0")
        f1 = self.db.getOrCreatePV("fifi", "v*")
        self.db.addRequires(d1, f1)
        tmp = self.db.getPV("DAVINCI", "v50r0")
        self.assertTrue(f1 in list(tmp.requiresPattern))

        t1 = self.db.getOrCreatePV("toto", "v1r0")
        tp1 = self.db.getOrCreatePV("toto", "v*")
        self.db.addRequires(d1, tp1)
        tmp = self.db.getPV("DAVINCI", "v50r0")
        self.assertTrue(t1 in list(tmp.requires))

        t2 = self.db.getOrCreatePV("toto", "v2r0")
        tmp = self.db.getPV("DAVINCI", "v50r0")
        self.assertTrue(t2 in list(tmp.requires))
        self.assertTrue(t1 not in list(tmp.requires))

        d2 = self.db.getOrCreatePV("DAVINCI", "v50r4")
        tp2 = self.db.getOrCreatePV("toto", "v2r*")
        ...
        t3 = self.db.getOrCreatePV("toto", "v2r1")
        tmp = self.db.getPV("DAVINCI", "v50r0")
    ...
    def test_show(self):
        ...
        a = self.db.getPV('a', 'v1r0')
        b = self.db.getPV('b', 'v1r0')
        project = self.db.getOrCreateProjectNode('b')
    ...
    def test_getSourceURI(self):
        self.db.addPVPlatform('a', 'v1r0', 'x64')
        self.db.getOrCreateProjectNode('a', sourceuri='gitlab:myfirstproject')
        ...
        self.db.getOrCreateProjectNode('Davinci')
    ...
    def test_ProjectProperty(self):
        pv = self.db.getOrCreateProjectNode('My_First_project')
    ...
    def test_listStackPlatformsToRelease(self):
        ...
        other = self.db.getOrCreatePV('e', 'v1r0')
    ...
    def test_tDatapkg(self):
        pk1 = self.db.getOrCreateDatapkgVersion('a', 'tck', 'datapacket1',
                                                'v1r0')
        pk2 = self.db.getOrCreateDatapkgVersion('a', 'tck', 'datapacket1',
                                                'v1r1')
        pk3 = self.db.getOrCreateDatapkgVersion('a', 'tck', 'datapacket2',
                                                'v1r1')
        pk4 = self.db.getOrCreateDatapkgVersion('a', None, 'datapacket3',
                                                'v1r1')
    ...
    def test_active(self):
        self.db.getOrCreatePV('a', 'v1r0')
        a = self.db.getOrCreatePV('a', 'v1r1')
        b = self.db.getOrCreatePV('b', 'v1r0')
        self.db.getOrCreatePV('c', 'v1r0')
        self.db.addRequires(a, b)
    ...
    def test_getOrCreateBulkPV(self):
        self.assertEqual(None, self.db.getOrCreateBulkPV(json.dumps([])))
        data = [{
            'project': 'A',
            'version': 'v1r0',
            'createNode': True,
        ...
        c = self.db.getOrCreatePV('c', 'v1r0')

        self.db.getOrCreateBulkPV(json.dumps(data))
        a = self.db.getPV('a', 'v1r0')
        b = self.db.getPV('b', 'v1r0')
        d = self.db.getPV('d', 'v1r0')
