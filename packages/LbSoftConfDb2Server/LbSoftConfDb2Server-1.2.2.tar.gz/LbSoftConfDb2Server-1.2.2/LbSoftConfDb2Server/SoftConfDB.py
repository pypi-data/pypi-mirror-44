...
class SoftConfDB():
    ...
    def listReleaseStacks(self):
        ...
        query = "MATCH p=(u:ToRelease)- [:REQUIRES*0..]->(v:ToRelease) WHERE" \
                ...
                "requested_platforms, m.buildTool as build_tool " \
    ...
    def delPVPlatform(self, project, version, platform, reltype="PLATFORM"):
        ...
        for node_platform in getattr(node_pv, reltype.lower()):
            if node_platform.name == platform:
                self.log.warning("Removing relationship: %s" %
                                 str(node_platform))
                remov_obj = node_platform
        if remov_obj is None:
            self.log.warning("No relation was found, exiting")
            return
    ...
    @writeAllowed
    def getOrCreateProjectNode(self, project, sourceuri=None):
        """
        ...
        elif sourceuri and project_db.sourceuri != sourceuri:
            project_db.sourceuri = sourceuri
            self.mNeoDB.push(project_db)
        return project_db

    @writeAllowed
    def getOrCreatePVSerializable(self, project, version):
        """
        Serialized verison of getOrCreatePV in order to be used in the XMLRPC
        server
        :param project: the project to use in the query
        :param version: the version to use in the query
        :return: a string representation to the project version
        """
        return str(self.getOrCreatePV(project, version))

    @writeAllowed
    def getOrCreateProjectSerializable(self, project, sourceuri):
        """
        Serialized verison of getOrCreateProject in order to be used in the XMLRPC
        server
        :param project: the project to use in the query
        :return: a string representation to the project version
        """
        return str(self.getOrCreateProjectNode(project, sourceuri=sourceuri))

    @writeAllowed
    def getOrCreateBulkPV(self, data):
        """
    ...
    def _insertPVFromBulk(self, proj, ver, saveURIinPV, deps, autorelease,
        ...
        :return: the neo4j node
        """
        tmp = self.findVersion(proj, ver)
        if len(tmp) == 0:
            node_parent = self.getOrCreatePV(proj, ver)
            if saveURIinPV:
                self.setPVProperty(proj, ver, "sourceuri", sourceuri)
            # If releasing is needed!
            if autorelease and proj.upper() not in ["LCG", "LCGCMT"]:
                self.log.warning("Requesting release of %s %s" % (proj, ver))
    ...
    def getOrCreatePV(self, project, version):
        ...
        parent_node = self.getOrCreateProjectNode(project)
    ...
    def _getLatestVersionForPattern(self, project_name, pattern):
        ...
        matching = self.matchAndSortVersions(pattern,
                                             versions)
        if len(matching) == 0:
            self.log.warning("No match found for %s_%s pattern" % (
    ...
    def _updateVersionPatternLinks(self, pv):
        ...
        for pattern in patterns.keys():
            self.log.warning("Processing pattern: %s_%s" % (pv.project_name,
                                                            pattern))
