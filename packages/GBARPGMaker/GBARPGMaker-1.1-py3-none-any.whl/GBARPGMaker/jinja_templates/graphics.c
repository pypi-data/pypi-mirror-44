...
void setHeroSpecialActions(void (*heroSpecialActions_[])(int x, int y), int heroSpecialActionCount_);
...
void loadMap{{ map_name }}(void (*heroSpecialActions_[])(int x, int y), int heroSpecialActionCount_) {
    ...
    if (heroSpecialActionCount_ > 0) {
        setHeroSpecialActions(heroSpecialActions_, heroSpecialActionCount_);
    }
}
{%- endfor %}
...
void initGraphics() {
    ...
    REG_BG1CNT = TILE_BASE(0) | MAP_BASE(30) | BG_256_COLOR | BG_PRIORITY(3);
