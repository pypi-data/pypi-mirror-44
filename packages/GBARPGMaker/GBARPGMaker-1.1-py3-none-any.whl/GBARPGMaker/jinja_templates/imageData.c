...
const unsigned short {{ sprite_graphic_name }}RealSize[2] = { {{sprite_graphic.real_size[0]}}, {{ sprite_graphic.real_size[1] }} };

const unsigned short {{ sprite_graphic_name }}Palette[{{ sprite_graphic.palette|length }}] __attribute__((aligned(4))) __attribute__((visibility("hidden")))=
{
    {% for o in range(sprite_graphic.palette|length) -%}
    {{ "{0:#06x}".format(sprite_graphic.palette[o]) }},
    {%- if loop.index is divisibleby 12 and not loop.last %}
    {% endif -%}
    {%- endfor %}
};

const int {{ sprite_graphic_name }}Tileset[{{ sprite_graphic.tileset|length }}] __attribute__((aligned(4))) __attribute__((visibility("hidden")))=
{
    {% for o in range(sprite_graphic.tileset|length) -%}
    {{ sprite_graphic.tileset[o] }},
    {%- if loop.index is divisibleby 64 and not loop.last %}
    {% endif -%}
    {%- endfor %}
};
{% endfor %}
