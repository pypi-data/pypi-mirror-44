...
labels = pd.read_csv("E:\\DR\\train001_filter0labels.csv",header = None)
print(labels.head())
label = labels.values
labels = label[:,1]
img_name = label[:,0]
...
print(labels[0])
print(img_name[0])
img_arr = np.zeros(shape=(len(img_name),224,224,3))
i = 0
for img in img_name:
    
    x = image.load_img(path+img+".jpeg", target_size=(224,224,3))
...
np.save("imagedata_train005_full",features_input)