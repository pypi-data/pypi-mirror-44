...
class LoggingConsumer(object):
    ...
    def __init__(self, log_directory, log_size=1024):
        ...
        # 文件分隔后缀
        self.getSplitLogSuffix(self.log_directory, self.suffix, self._fileSize)
        self.handler = self.__initLogger(self.log_directory, self.suffix, self._split_log_suffix)
        self.is_close = False

    def getSplitLogSuffix(self, directory, date_suffix, filesize):
        self._split_log_suffix = len([x for x in os.listdir(directory) if "log." + date_suffix in x])
        if self._split_log_suffix  != 0:
            self._split_log_suffix = self._split_log_suffix  - 1
        filename = directory + "log." + date_suffix + "_" + str(self._split_log_suffix)
        if (os.path.exists(filename) and self.fileSizeOut(filename)):
            pass
    ...
    def __initLogger(self, directory, date_suffix, file_suffix):
        ...
        if self.logger.handlers.__len__() != 0:
            for current_handler in self.logger.handlers:
...
class BatchConsumer(object):
    ...
    def __init__(self, server_uri, appid, batch=10, timeout=3000, interval=3):
        """
        Consumer construct
    ...
    def flush(self, throw_exception=False):
        try:
            self.__httpconsumer.send('[' + ','.join(self.__message_channel) + ']')
            self.__last_flush = time.time()
            self.__message_channel = []
...
class HttpConsumer(object):
    def __init__(self, server_uri, appid, timeout=3000):
        self.url = server_uri
        self.appid = appid
...
class AsyncFlushThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if self._consumer._queue.qsize() < self._consumer._flush_size:
