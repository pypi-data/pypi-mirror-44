...
column_annotations = {
    ...
    'URL': {
        chaise_tags.asset: {
            'md5': 'MD5',
            'url_pattern': '/hatrac/TestSchema/Foo_Asset/{{{MD5}}}.{{#encode}}{{{Filename}}}{{/encode}}',
...
column_comment = {
    ...
    'Foo_RID': 'The Foo entry to which this asset is attached',
...
column_defs = [
    ...
    em.Column.define(
        'Foo_RID', em.builtin_types['text'], nullok=False, comment=column_comment['Foo_RID'],
    ),
    ...
    '*': [
        {
            ...
            'source': [{
                'outbound': ['TestSchema', 'Foo_Asset_RCB_fkey']
            }, 'ID']
        }, {
            'source': [{
                'outbound': ['TestSchema', 'Foo_Asset_RMB_fkey']
            }, 'ID']
        }, {
            'source': [{
                'outbound': ['TestSchema', 'Foo_Asset_Catalog_Group_fkey']
            }, 'ID']
        }, {
            'source': 'URL'
        }, {
            ...
            'source': [{
                'outbound': ['TestSchema', 'Foo_Asset_Foo_fkey']
            }, 'RID']
        }
    ]
}
...
table_comment = 'Asset table for Foo'
...
key_defs = [
    em.Key.define(['RID'], constraint_names=[('TestSchema', 'Foo_Asset_RIDkey1')],
                  ),
    em.Key.define(['URL'], constraint_names=[('TestSchema', 'Foo_Asset_URLkey1')],
                  ),
]
...
fkey_defs = [
    em.ForeignKey.define(
        ...
        'TestSchema',
        'Foo', ['RID'],
        constraint_names=[('TestSchema', 'Foo_Asset_Foo_fkey')],
        acls={
    ...
    em.ForeignKey.define(
        ...
        'public',
        'Catalog_Group', ['ID'],
        constraint_names=[('TestSchema', 'Foo_Asset_Catalog_Group_fkey')],
        acls={
            'insert': [groups['test-curator']],
            'update': [groups['test-curator']]
        },
    ...
    em.ForeignKey.define(
        ...
        'public',
        'ERMrest_Client', ['ID'],
        constraint_names=[('TestSchema', 'Foo_Asset_RCB_fkey')],
        acls={
            'insert': ['*'],
            'update': ['*']
        },
    ...
    em.ForeignKey.define(
        ...
        'public',
        'ERMrest_Client', ['ID'],
        constraint_names=[('TestSchema', 'Foo_Asset_RMB_fkey')],
        acls={
            'insert': ['*'],
            'update': ['*']
        },
