...
class SkidwardModelView(ModelView):
    ...
    def confirm_message(self, id):
        ...
        message = 'Are you sure you want to delete {}: "{}"?'.format(self.name, record)
        return message

    def confirm(self):
        id = request.args.get("id")
        model_view = self.name.lower()
        message = self.confirm_message(id)
        if request.method == "POST":
            return redirect(
                url_for("{}.delete_view".format(model_view), confirm="1", id=id)
            )
        return self.render("_confirm.html", message=message)

    def delete_row(self, id, confirm):
        ...
        if self.validate_form(form):
            id = form.id.data
            return redirect(url_for("{}.confirm".format(model_view), id=id))
...
class UserAdmin(SkidwardModelView):
    ...
    def confirm(self):
        return super(UserAdmin, self).confirm()

    @expose("/delete/", methods=("GET", "POST"))
    def delete_view(self):
        id = request.args.get("id")
        confirm = request.args.get("confirm")
        return super(UserAdmin, self).delete_row(id, confirm)
...
class RoleAdmin(SkidwardModelView):
    ...
    @expose("/confirm", methods=("GET", "POST"))
    def confirm(self):
        return super(RoleAdmin, self).confirm()

    @expose("/delete/", methods=("GET", "POST"))
    def delete_view(self):
        id = request.args.get("id")
        confirm = request.args.get("confirm")
        return super(RoleAdmin, self).delete_row(id, confirm)
...
class WorkerView(SkidwardModelView):
    ...
    @expose("/confirm", methods=("GET", "POST"))
    def confirm(self):
        return super(WorkerView, self).confirm()

    @expose("/delete/", methods=("GET", "POST"))
    def delete_view(self):
        id = request.args.get("id")
        confirm = request.args.get("confirm")
        return super(WorkerView, self).delete_row(id, confirm)
...
class TaskView(SkidwardModelView):
    ...
    @expose("/confirm", methods=("GET", "POST"))
    def confirm(self):
        return super(TaskView, self).confirm()

    @expose("/delete/", methods=("GET", "POST"))
    def delete_view(self):
        id = request.args.get("id")
        confirm = request.args.get("confirm")
        return super(TaskView, self).delete_row(id, confirm)
...
class JobView(SkidwardModelView):
    ...
    @expose("/confirm", methods=("GET", "POST"))
    def confirm(self):
        return super(JobView, self).confirm()

    @expose("/delete/", methods=("GET", "POST"))
    def delete_view(self):
        id = request.args.get("id")
        confirm = request.args.get("confirm")
        return super(JobView, self).delete_row(id, confirm)
