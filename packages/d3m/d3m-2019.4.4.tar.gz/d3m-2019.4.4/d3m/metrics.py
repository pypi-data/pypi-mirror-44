...
class HammingLossMetric(Metric):
    def _to_2d_array(self, dataframe: pandas.DataFrame) -> typing.Tuple[typing.List, typing.List, bool]:
        is_multi_class = True
        ...
        return list(index_to_values.keys()), list(index_to_values.values()), is_multi_class
    ...
    def score(self, truth: Truth, predictions: Predictions) -> float:
        ...
        (truth_keys, truth_values, is_multi_class) = self._to_2d_array(pandas.concat([truth_index, truth_targets], axis=1))
        (predictions_keys, predictions_values, is_multi_class) = self._to_2d_array(pandas.concat([predictions_index, predictions_targets], axis=1))
        ...
        if is_multi_class:
            truth_label_encoded = numpy.array(truth_values).ravel()
            predictions_label_encoded = numpy.array(predictions_values).ravel()
        else:
