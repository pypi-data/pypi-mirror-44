...
class D3MProblemLoader(Loader):
    ...
    SUPPORTED_VERSIONS = {'3.0', '3.1', '3.1.1', '3.1.2', '3.2.0', '3.2.1', '3.3.0'}
    ...
    def load(self, problem_uri: str, *, problem_id: str = None, problem_version: str = None,
        ...
        parsed_uri = url_parse.urlparse(problem_uri)
        ...
        description = {
            ...
            'location_uris': [
                ...
                'file://{problem_doc_path}'.format(problem_doc_path=problem_doc_path),
            ],
            'problem': {
...
class Problem(dict):
    ...
    def _canonical_problem_description(cls: typing.Type[P], problem_description: typing.Dict) -> P:
        ...
        Currently, this is just removing any URIs the description might have.
        """

        # Making a copy.
        problem_description = dict(problem_description)

        if 'location_uris' in problem_description:
            del problem_description['location_uris']
...
def parse_problem_description(problem_doc_path: str) -> Problem:
    ...
    return Problem.load(problem_uri='file://{problem_doc_path}'.format(problem_doc_path=problem_doc_path))
...
def main(argv: typing.Sequence) -> None:
    ...
    for problem_doc_path in argv[1:]:
        try:
            problem = Problem.load('file://{problem_doc_path}'.format(problem_doc_path=os.path.abspath(problem_doc_path)))
            pprint.pprint(problem)
        except Exception as error:
            raise Exception("Unable to load problem: {problem_doc_path}".format(problem_doc_path=problem_doc_path)) from error
