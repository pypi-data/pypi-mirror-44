    ...
    def get_camera_snapshot_uri(self) -> str:
        try:
            res: requests.Response = requests.get(
                url=f'http://{self.address}:8080/?action=snapshot', timeout=2)
            return f"data:{res.headers['Content-Type']};base64,{base64.b64encode(res.content).decode('utf-8')}"
        except requests.Timeout:
            return f"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8Xw8AAoMBgDTD2qgAAAAASUVORK5CYII="
