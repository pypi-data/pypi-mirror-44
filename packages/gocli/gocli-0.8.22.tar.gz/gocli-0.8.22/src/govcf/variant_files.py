    ...
    for record in filter(bed_filter, vcf_iter):
        ...
        for sample in samples:
            ...
            allele_indices, zyg, sample_is_ref_unknown = \
                get_allele_indices_and_zyg(sample, keep_ref)
...
def create_call_dict(record, sample, alt, zyg, info, vaf_dict, build):
    call_type = "REF" if record.ref == alt else "CALL"
    return dict(
        __type__=call_type,
...
def get_allele_indices_and_zyg(sample, keep_ref):
    allele_indices = sample.allele_indices or [1]
    zyg = determine_zyg(allele_indices)

    # if keep_ref, then convert . (None) to 0
    if keep_ref:
        allele_indices = map(lambda allele: allele or 0, allele_indices)
    # else remove None and 0
    else:
        allele_indices = filter(None, allele_indices)

    allele_indices = sorted(set(allele_indices))
