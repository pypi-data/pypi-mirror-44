...
from monads import Function, Just, Maybe, Nothing
...
def test_monad_bind_lift_equivalence(value):
    """ m a >= lift == m a """
    m_a = Maybe.lift(value)

    ma_bind_lift = m_a >= Maybe.lift  # pylint: disable=comparison-with-callable
...
def test_monad_lift_bind_equivalence(value):
    """ lift a >= f == f a """
    f = Function(lambda v: Maybe.lift(v * v))
    m_a = Maybe.lift(value)
...
def test_monad_bind_composition_equivalence(value):
    """ m >= (a -> f a >>= g) == (m >= f) >= g """
    f = Function(lambda v: Maybe.lift(v * v))
    g = Function(lambda v: Maybe.lift(v + v))
    m_a = Maybe.lift(value)
