...
def convertToJavaArray(input):
    ...
    * ndarrays of any other dtype (namely *complex\*, uint\*, void\*,* or custom dtypes):
    ...
    if isinstance(input, numpy.ndarray):
        return makeJavaArray(input, 'unknown', convertUnknownToString=False)
    elif isinstance(input, list) or isinstance(input, tuple):
        ndarray = numpy.asarray(input)
        return convertToJavaArray(ndarray)
...
def convertToJavaList(input):
    ...
    jarray = _ensureBoxedArray(convertToJavaArray(input))  # it's got to be boxed, or jpy will mangle the matching
...
def convertToJavaHashMap(input1, input2=None):
    ...
    if isinstance(input1, dict):
    ...
    elif (isinstance(input1, list) or isinstance(input1, tuple) or isinstance(input1, numpy.ndarray)) and \
            (isinstance(input2, list) or isinstance(input2, tuple) or isinstance(input2, numpy.ndarray)):
        ...
        raise ValueError("Expected input1 to be a dict, or input1 and input2 to be lists/tuples/numpy.ndarrays "
                         "of the same length. "
...
def makeJavaArray(data, name, convertUnknownToString=False):
    ...
    * basic numpy primitive dtype are converted to their java analog, ``NaN`` values in floating point columns are
      converted to their respective Deephaven NULL constant values.
    * dtype ``datatime64[*]`` are converted to ``DBDateTime``
    * dtype of one of the basic string type *(unicode\*, str\*, bytes\*)* are converted to String
    * ndarrays of dtype ``numpy.object``:
...
def _parseJavaArrayType(javaArray):
    ...
    * (dimension, basic type string)
    ...
    if len(javaArray) == 0:
        # This is dumb, and should never happen
        return 0, None
