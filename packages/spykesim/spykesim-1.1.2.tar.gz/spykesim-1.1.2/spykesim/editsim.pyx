...
#     i, j = np.unravel_index(simmat.argmax(), simmat.shape)
    ...
    al1, al2 = clocal_editsim_align_alt(bp, dp_max_x, dp_max_y, mats[i].astype(np.double), mats[j].astype(np.double), flip)
    mats[i] = al1
    mats[j] = al2
    al = (al1 + al2) / 2
    processed[i] = True
    ...
    for iter_ in tqdm(range(niter)):
        if processed.sum() == 0:
            ...
            al1, al2 = clocal_editsim_align_alt(bp, dp_max_x, dp_max_y, al.astype(np.double), mats[j].astype(np.double), flip)
            al = (al1 + al2) / 2            
            mats[j] = al2
            processed[j] = True
    return mats[i]
