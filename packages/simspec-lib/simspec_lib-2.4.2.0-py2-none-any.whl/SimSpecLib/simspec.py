...
class SSutil:
    ...
    def InstallModule(modpackage):
        ...
        if modpackage[:8] == 'https://' or modpackage[:7] == 'http://':
            import requests
            from StringIO import StringIO
            print "URL supplied, Downloading file"
        ...
        for mod in sorted(next(SimSpec.os.walk(tdir))[1]):
            ...
            if SimSpec.os.path.exists( target_fullpath ):
                if raw_input(mod+": module already exist. Do you want to repalce with new one? [y/N] : ").lower()=='y':
