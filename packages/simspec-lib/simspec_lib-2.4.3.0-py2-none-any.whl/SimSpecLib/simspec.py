...
class SimSpec:
    ...
    def __init__(self, inifile='setup.conf', Object=False, DataDir='', WorkDir=''):
        ...
        if not self.os.path.isfile(inifile):
            raise self.SimSpecError("Unable to find configuration file: "+inifile)
        ...
        if WorkDir=='':
            self.WorkDir=self.os.path.join(self.DataDir,'proc')
        else:
    ...
    def parseObject(self, Object):
        ...
        if self.StdFiles:
            try:
                ...
                raise self.SimSpecError('No name defined for "'+self.StdGrp+'" group standard.')
        ...
        if not self.ObjectFiles:
            raise self.SimSpecError('No files listed for the "'+Object+'" group object.')
    ...
    def parseSetup(self):
        ...
        if self.inifile!=self.ConfDir+'default/setup.conf':
            if self.os.path.exists(self.ConfDir+self.Instrument+'/setup.conf'):
                self.param.update(SimSpec(self.ConfDir+self.Instrument+'/setup.conf').param)
            else:
                raise self.SimSpecError('Module '+self.Instrument+' does not exist')
    ...
    def ListObjects(self):
        ...
        for l in self.iniconf.sections():
            if l.lower() not in ['arc','bias','flat','std','standard','setup']:
                lis=lis+[l]
        return(lis)
    ...
    def CombineBiasFlat(self,DelOlder=False):
        ...
        if self.param['flatcor'] and self.FlatFiles and not self.os.path.isfile(self.Files['Flat']):
            ...
            self.iraf.ccdproc(self.Files['FlatSQ']+'???.fits', output="", ccdtype="", fixpix='no', overscan='no', trim='no', zerocor='yes' if self.param['biascor'] else 'no', darkcor='no', flatcor='no', zero=self.Files['Bias'], biassec='[*,*]')
            ...
            if self.param['nflat.method'] in ['movavg','savgolay']:
                from astropy.io import fits
                input_hdu = fits.open(self.Files['Flat'])
                image_data = input_hdu[0].data

                if self.param['dispaxis'].lower()!='x':
                    image_data=self.np.transpose(image_data)
                #image_data2=image_data.copy()
                
                imin_1d=self.np.median(image_data,0)
                if self.param['nflat.method']=='movavg':
                    avsz=int(self.np.floor(self.param['nflat.order/len']/2)*2)
                    base_ind=self.np.arange(-avsz/2,avsz/2+1)
                    imin_smooth=imin_1d.copy()
                    for i in range(0,len(imin_1d)):
                        ind=base_ind+i
                        ind=ind[(ind>=0) & (ind<len(imin_1d))]
                        imin_smooth[i]=self.np.mean(imin_1d[ind])
                
                elif self.param['nflat.method']=='savgolay':
                    from scipy.signal import savgol_filter
                    imin_smooth=savgol_filter(imin_1d, int(self.np.floor(self.param['nflat.order/len']/2)*2)+1, 2,mode='mirror')
                
                if self.param['nflat.interact']:
                    from matplotlib import pyplot as plt
                    plt.plot(imin_1d)
                    plt.plot(imin_smooth)
                    plt.show()
                
                image_data2=self.np.ones((len(image_data),1))*imin_smooth
                
                image_dataO=image_data/image_data2
                if self.param['dispaxis'].lower()!='x':
                    image_dataO=self.np.transpose(image_dataO)        

                output_hdu = fits.PrimaryHDU(image_dataO)
                output_hdu.header = input_hdu[0].header.copy()
                output_hdu.header['CCDMEAN']=1
                output_hdu.verify("fix+ignore")
                output_hdulist = fits.HDUList([output_hdu])

                #self.os.system("rm -f " + self.Files['FlatNorm'])
                output_hdulist.writeto(self.Files['FlatNorm'], output_verify="ignore")
                input_hdu.close()
            elif self.param['nflat.method']=='flat':
                self.iraf.imcopy(self.Files['Flat'], self.Files['FlatNorm'])
            else:
                self.iraf.hedit(self.Files['Flat'], fields="dispaxis", value=1 if self.param['dispaxis'].lower()=='x' else 2, add='yes', ver='no')
                self.iraf.longslit(_doprint=0)
                self.iraf.response.unlearn()
                self.iraf.response(self.Files['Flat'], self.Files['Flat'], self.Files['FlatNorm'], order=self.param['nflat.order/len'], function=self.param['nflat.method'], low_rej=3., high_rej=3., interactive='yes' if self.param['nflat.interact'] else 'no')
            self.iraf.imreplace(self.Files['FlatNorm'], value=1, upper=self.param['nflat.minval'])
    ...
    def PreCleanExec(self,files):
        ...
        execfile(self.param['precleanscript'],{},{'SimSpec':self})
    ...
    def Clean(self,DelOlder=False):
        ...
        if not self.os.path.isfile(self.Files['Object2D']):
            ...
            self.iraf.ccdproc(self.Files['ObjectSQ']+'???.fits', output="", ccdtype="", fixpix='no', oversca='no', trim='yes' if self.param['trimsec']!='[*,*]' else 'no', zerocor='yes' if self.param['biascor'] else 'no', darkcor='no', flatcor='yes' if self.param['flatcor'] else 'no', trimsec=self.param['trimsec'], zero=self.Files['Bias'], flat=self.Files['FlatNorm'])
            ...
            if self.param['deleteobjsingles']:
                self.iraf.imdelete(self.Files['ObjectSQ']+'???.fits')
        execfile(self.param['postcleanscript'],{},{'SimSpec':self})
    ...
    def ApExtract(self, spectype='object',DelOlder=False):
        ...
        if spectype.lower()=='object':
            ...
            raise self.SimSpecError('Invalid SpecType '+spectype)
...
class SSutil:
    ...
    def ListInstrument():
        lis=[]
        for ini in SimSpec.glob.glob(SimSpec.ConfDir+'*/setup.conf'):
            inst=SimSpec.os.path.basename(SimSpec.os.path.dirname(ini))
            if inst.lower()=='default':
                continue
            tmp = SimSpec.ConfigParser.RawConfigParser(allow_no_value=True)
            tmp.read(ini)
            try:
    ...
    @staticmethod
    def ListDefaultStds(conf=SimSpec.ConfDir+'default/setup.conf'):
        return(SimSpec(conf).LsStdDir())
    
    @staticmethod
    def ObsLog(inst='default',lsinp='*.fits', datadir=''):
        if type(lsinp)==str:
            lsinp=[lsinp]
        try:
            p=SimSpec(SimSpec.ConfDir+inst+'/setup.conf').param
        except SimSpec.SimSpecError as zz:
    ...
    def GenConf(inst='default',ofile='setup.conf'):
        try:
            ...
            print "Invalid instrument name"
            return
        iniout = SimSpec.ConfigParser.RawConfigParser(allow_no_value=True)
        iniout.optionxform = str
        iniout.add_section('setup')
        iniout.set('setup','; '+SimSpec.__name__+' template file for reduction setup.')
        iniout.set('setup','; Generated by '+SimSpec.__name__+' (lib) v'+SimSpec.__ver__)
        iniout.set('setup','; with '+inst+' Module v'+str(p['modulever']))
        iniout.set('setup','Instrument',inst)
        for req_par in p['_required_user_pars'].split(','):
            iniout.set('setup', req_par.strip(), str(p[req_par.strip().lower()])+'   ;Please ensure this parameter is correctly set')
        iniout.set('setup','')
        if p['biascor']:
        ...
        iniout.set('std','; Use "simspec -liststds" to get list of standard names available by default')
        iniout.set('std','Name','')
        iniout.set('std','')
        iniout.add_section('object')
        
        if not SimSpec.os.path.isfile(ofile):
    ...
    def RunSNID(sfile,SnidExe):
        from pyraf import iraf
        fname=SimSpec.os.path.basename(sfile).rsplit('.', 1)[0]
        iraf.wspectext(sfile+'[*,1]' ,SimSpec.os.path.join(SimSpec.os.path.dirname(sfile),fname+'.txt'), header='no')
        cwd=SimSpec.os.getcwd()
        SimSpec.os.chdir( SimSpec.os.path.dirname(sfile) )
        try:
            SimSpec.os.system(SnidExe+" '"+fname+'.txt'+"'")
        except Exception as e:
            echo ("Failed to run SNID")
        finally:
            SimSpec.os.chdir( cwd )
    
    @staticmethod
    def DeleteUnusedFiles(inifile='setup.conf',datadir=''):
        SS=SimSpec(inifile,DataDir=datadir)
        safekeep=[]        
    ...
    def InstallModule(modpackage):
        ...
        tdir=tempfile.mkdtemp(prefix='SimSpec_')
        mdir=SimSpec.ConfDir

        if not SimSpec.os.path.exists( mdir ):
            SimSpec.os.mkdir(mdir)
        ...
        if modpackage.lower()[:8] == 'https://' or modpackage.lower()[:7] == 'http://':
            ...
            repo='http://astro.subhashbose.com/simspec/modules/'
            print "Assuming SimSpec module name specified"
            ...
            try:
                ...
                raise SimSpec.SimSpecError("Specified package name not found in repository!! Ensure the package name match exactly listed in "+repo)
            modpackage = StringIO(requests.get(modpackage).content)
        ...
        if not SimSpec.os.access(mdir, SimSpec.os.W_OK):
            raise SimSpec.SimSpecError("Insufficient file permission!! You can not install/upgrade Modules, try as superuser")

        for mod in sorted(next(SimSpec.os.walk(tdir))[1]):
            target_fullpath=SimSpec.os.path.join(mdir,mod)
            if SimSpec.os.path.exists( target_fullpath ):
                if raw_input(mod+": module already exist. Do you want to replace with new one? [y/N] : ").lower()=='y':
                    ...
                    shutil.move(target_fullpath, SimSpec.os.path.join(mdir,new))
            if not SimSpec.os.path.exists( target_fullpath ):
                shutil.move(SimSpec.os.path.join(tdir,mod),  target_fullpath)
    ...
    def PackModule(moduledir,modulefile=''):
        if not SimSpec.os.path.isdir(moduledir):
            raise SimSpec.SimSpecError('Module packing error! %s is not a directory' % moduledir)
        if modulefile=='' or SimSpec.os.path.isdir(modulefile):
            iniconf = SimSpec.ConfigParser.RawConfigParser(allow_no_value=True)
            if len(iniconf.read(SimSpec.os.path.join(moduledir,'setup.conf')))==0:
                raise SimSpec.SimSpecError('Module packing error! %s is not a valid module' % moduledir)
            try:
                ver=str(iniconf.get('setup', 'modulever'))
            except:
                ver='1.0'
            modulefile=SimSpec.os.path.join(modulefile,SimSpec.os.path.basename(SimSpec.os.path.join(moduledir,'')[0:-1])+'-v'+ver+'.ssmod')
        import zipfile
        zf=zipfile.ZipFile(modulefile, 'w', zipfile.ZIP_DEFLATED)
        for root, dirs, files in SimSpec.os.walk(moduledir):
            for file in files:
                zf.write(SimSpec.os.path.join(root, file),
                         arcname=SimSpec.os.path.relpath(SimSpec.os.path.join(root, file) ,SimSpec.os.path.join(moduledir, '..')) )
        zf.close()
        if SimSpec.os.path.isfile(modulefile):
