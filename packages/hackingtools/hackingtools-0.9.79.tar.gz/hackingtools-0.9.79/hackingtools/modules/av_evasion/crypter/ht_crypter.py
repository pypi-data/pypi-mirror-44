...
"""
	...
	def crypt_file(self, filename, new_file_name, drop_file_name, iterate_count=1, print_save_stub=True, compile_exe=False):
		...
		"""
		Logger.printMessage(message='{methodName}'.format(methodName='crypt_file'), description='{filename} - {new_file_name} - {drop_file_name}'.format(filename=filename, new_file_name=new_file_name, drop_file_name=drop_file_name), debug_module=True)
