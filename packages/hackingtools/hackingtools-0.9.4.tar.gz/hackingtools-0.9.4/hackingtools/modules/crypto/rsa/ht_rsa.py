...
class StartModule():
	...
	def encrypt(self, private_key, plaintext):
		...
		key, n = private_key
		mensaje = __mensajeASCII__(plaintext)
        mensaje1 = [(ord(char) ** key) % n for char in mensaje]
        mensajeHex = __ASCII_Hex__(mensaje1)
        mensajeBase64 = __Hex_Base64__(mensajeHex)
        mensajeFinalBase64 = __unirBase64__(mensajeBase64)
		return mensajeFinalBase64

	def decrypt(self, public_key, ciphertext):
		#Unpack the key into its components
		key, n = public_key
		mensajeRecibido = __recibirBase64__(ciphertext)
        mensajeHexRecibido = __Base64_Hex__(mensajeRecibido)
        mensajeDecimalRecibido = __Hex_decimal__(mensajeHexRecibido)
        mensajeDescifrado = [chr((char ** key) % n) for char in mensajeDecimalRecibido]
        mensajeFinal = __decimal_ASCII__(mensajeDescifrado)
		return ''.join(plain)
	...
	def __mensajeASCII__(mensaje):

		__mensajeASCII__ = []
		for palabra in mensaje:
			__mensajeASCII__.append(ord(palabra))
		return __mensajeASCII__

	def __ASCII_Hex__(mensaje):
