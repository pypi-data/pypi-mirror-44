...
class StartModule():
	...
	def encrypt(self, private_key, plaintext):
		...
		encrypted_msg_joined = ''.join(map(lambda x: str(x), mensajeFinalBase64))
		return encrypted_msg_joined

	def decrypt(self, public_key, ciphertext):
		#Unpack the key into its components
		key, n = public_key
		mensajeRecibido = self.__recibirBase64__(ciphertext.encode())
