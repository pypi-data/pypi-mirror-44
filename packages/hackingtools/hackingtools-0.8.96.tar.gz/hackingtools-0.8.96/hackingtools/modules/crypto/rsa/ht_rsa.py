...
class StartModule():
	...
	def encrypt(self, private_key, plaintext):
		...
		joined = ''
		for c in cipher:
			joined = '{joined}{new}'.format(joined=joined, new=c)
		joined_cipher_base64 = base64.b64encode(joined)
		#Return the array of bytes
		return joined_cipher_base64

	def decrypt(self, public_key, ciphertext):
		#Unpack the key into its components
		key, n = public_key
		#Generate the plaintext based on the ciphertext and key using a^b mod m
		plain = [chr((char ** key) % n) for char in base64.b64decode(ciphertext)]
