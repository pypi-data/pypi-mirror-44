...
class StartModule():
	...
	def getRandomKeypair(self):
		...
		while prime_a == '':
			for x in range(100):
				num = random.randint(random.randint(0,50),random.randint(50,100))*random.randint(1,20)
				if self.__is_prime__(num):
					prime_a = num
		print('A: {a}'.format(a=prime_a))
		while prime_b == '':
			for x in range(100):
				num = random.randint(random.randint(50,100),random.randint(100,150))*random.randint(1,20)
				if self.__is_prime__(num):
					prime_b = num
