...
class StartModule():
	...
	def getRandomKeypair(self):
		...
		prime_b = ''
		while prime_a == '':
			num = random.randint(random.randint(0,30),random.randint(30,60))*random.randint(1,10)
			if self.__is_prime__(num):
				prime_a = num
		print('A: {a}'.format(a=prime_a))
		while prime_b == '':
			num = random.randint(random.randint(60,90),random.randint(90,120))*random.randint(1,10)
			if self.__is_prime__(num):
				prime_b = num
