...
class StartModule():
	...
	def clearOutputFolder(self, stub_name):
		output_folder = os.path.abspath(os.path.dirname(stub_name))
		onlyfiles = [f for f in os.listdir(output_folder) if os.path.isfile(os.path.join(output_folder, f))]
		for filename in onlyfiles:
			if filename not in stub_name:
				os.remove(filename)
...
"""
		self.clearOutputFolder(save_name)
