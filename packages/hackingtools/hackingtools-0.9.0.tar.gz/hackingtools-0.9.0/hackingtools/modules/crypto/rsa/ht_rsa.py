...
class StartModule():
	...
	def encrypt(self, private_key, plaintext):
		...
		cipher_to_base = []
		for c in cipher:
			cipher_to_base.append(base64.b64encode(str(c)))
		#Return the array of bytes
		return ''.join(cipher_to_base)
	...
	def decrypt(self, public_key, ciphertext):
		...
		cipher_no_base = []
		for c in ciphertext:
			cipher_no_base.append(base64.b64decode(str(c)))
		plain = [chr((char ** key) % n) for char in cipher_no_base]
