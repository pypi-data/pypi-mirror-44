    ...
    for modu in modules:
        for submod in modules[modu]:
            for files in modules[modu][submod]:
                ...
                module_import_string = '{package}.{modules}.{category}.{tool}.{toolFileName}'.format(package=package, modules=modu, category=submod, tool=files, toolFileName=module_name)
...
def createModule(moduleName, category):
    ...
    if not os.path.isdir('modules/{category}/{moduleName}'.format(category=category, moduleName=moduleName)):
        os.makedirs('modules/{category}/{moduleName}'.format(category=category, moduleName=moduleName))
    if not os.path.exists('modules/{category}/{moduleName}/ht_{moduleName}.py'.format(category=category, moduleName=moduleName)):
        f = open('modules/{category}/{moduleName}/ht_{moduleName}.py'.format(category=category, moduleName=moduleName), "w")
        f.write(default_template_modules_ht)
    if not os.path.exists('modules/{category}/{moduleName}/__init__.py'.format(category=category, moduleName=moduleName)):
        f = open('modules/{category}/{moduleName}/__init__.py'.format(category=category, moduleName=moduleName), "w")
        f.write('')
    
def createCategory(categoryName):
    categories = getCategories()
    if categoryName not in categories:
        if not os.path.isdir('modules/{category}/'.format(category=categoryName))
            os.makedirs('modules/{category}'.format(category=categoryName))
