...
class MongoFactory():
    ...
    def __new__(cls, *_args, **_kwargs):
        if cls._collection_dict is None:
            cls._mongo_init()
            print('collection_init')
        instance = super(MongoFactory, cls).__new__(cls)
        instance.__init__()
        return instance
    ...
    def _mongo_init(cls):
        ...
        cls._mongo_client = MongoClient(T_T.client).__getattr__(T_T.db)
        cls._motor_client = motor.MotorClient(T_T.client).__getattr__(T_T.db)
        cls._collection_dict = {(scls.__alias__ or scls.__collection_name__):
                                scls for scls in MongoBase.__subclasses__()}

        if T_T.run_init:
            for scls in MongoBase.__subclasses__():
                scls.__collection_init__(cls._mongo_client)

    def __init__(self, sync=False):
        if MongoFactory._collection_dict is None:
            self._mongo_init()
            print('collection_init')
        if sync:
