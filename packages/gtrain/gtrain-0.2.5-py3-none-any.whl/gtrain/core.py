...
gtrain implements general purpouse training algorithm
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            #optimizer = tf.train.MomentumOptimizer(lr_var, mu, use_nesterov=use_nesterow)

            # Accumulative
            with tf.name_scope("Accumulate"):
                tvs = tf.trainable_variables()
                accum_vars = [tf.Variable(tf.zeros_like(tv.initialized_value()), trainable=False) for tv in tvs]    
                accum_loss = tf.Variable(0.0, trainable=False)
                accum_hits = tf.Variable(0.0, trainable=False)
                accum_count = tf.Variable(0.0, trainable=False)
