...
provider = Web3.IPCProvider(config.Ethereum['internal']['ipc'])
web3 = Web3(provider)
...
def sign_tx(sender_address, tx, password):
    ...
    files = glob.glob(r'/Users/YCB/.ethereum/keystore/*--{}'.format(sender_address.lower()[2:]))
    if len(files) > 0:
...
def sign_msg(sender_address, msg, password):
    ...
    files = glob.glob(r'{}/keystore/*--{}'.format(config.Ethereum['path'], sender_address.lower()[2:]))
    if len(files) > 0:
        with open(files[0]) as key_file:
            try:
