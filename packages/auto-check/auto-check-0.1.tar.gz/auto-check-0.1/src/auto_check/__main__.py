...
from src.auto_check.checker.auto_checker import AutoChecker
from src.auto_check.bean.property import Property
from src.auto_check.common.parse_params import parse
...
def main(**kwargs):
    ...
    # 优先从网络获取配置参数
    if params.url != '':
        resp = str(requests.get(params.url).content).replace('b\'', '').replace(']\'', ']')
        # print(resp)
        ...
        for conf in configs:
            ...
            if params.suffix != '':
                if not conf['username'].__contains__(params.suffix):
                    ...
                    t = threading.Thread(target=AutoChecker(Property(conf), params.test, not params.immediately).check,
                                         name=conf['username'])
                    t.start()
                    break
            else:
