...
try:
    ...
    raise ImportError("The bcrypt library is needed for using Blowfish encryption.")
...
class Field(base.Field):
    ...
    place_after: Name of a field that this one should be placed
    after in field order, or None if it should not be reordered.
...
class HashedPassword(HashedValue):
    ...
    def __str__(self):
        return super(HashedPassword, self).__str__()
...
class String(Field):
    ...
    data_type = str # detect py3k here ?
    monospace = False
    multiline = False
    ...
    #    if value is UNASSIGNED:
    #        return value
    #    return type.__str__(value).encode('utf8')
    ...
    def validate(self, value):
        ...
        if not self.allow_empty and value == '':
            ...
            self._raise(ValueError, msg)
        #if self.multiline:
        #    if not value == UNASSIGNED and '\n' in value:
        #        msg = '%s value must be a single line.' % self._name
        #        self._raise(ValueError, msg)
        #assert value != UNASSIGNED, 'String value is unassigned'
        self._validate_min_max_size(value)

    #def __repr__(self):
    #    #return "'%s'"%str(super(String, self).__str__().encode('utf8'))
    #    s = str(super(String, self).get().encode('utf8'))
    #    return s
...
class Boolean(Field):
    ...
    def __unicode__(self):
        v = self.get()
        if v is UNASSIGNED and self.unassigned_label is not None:
            return str(self.unassigned_label)
        elif v is UNASSIGNED:
            return Field.__unicode__(self)
        else:
            if v:
...
class _EntityBase(Field):
    ...
    def reversible(self, value=None):
        ...
        if value is UNASSIGNED:
            ...
            return '%s-%i' % (value.s.extent.name, value.s.oid)
        ...
        if value is UNASSIGNED:
            ...
            if value in stop_entities:
                ...
                values = tuple(
                    ...
                    for field in list(field_map.values())
                    )
                return values
