...
def _key(*args):
    ...
    defined."""
    clsLocals = inspect.currentframe().f_locals
...
def finish(db, schema_module=None):
    ...
    for entity_name in schema_def.E:
        ...
        for FieldClass in EntityClass._field_spec.values():
    ...
    for entity_name in schema_def.E:
        EntityClass = schema_def.E[entity_name]
        for v in EntityClass.__dict__.values():
            if (inspect.isclass(v)
                and issubclass(v, schevo.transaction.Transaction)):
                v._db = db
    ...
    for view_name in schema_def.V:
        ...
        for v in ViewClass.__dict__.values():
            if (inspect.isclass(v)
                and issubclass(v, schevo.transaction.Transaction)):
                v._db = db
    # Add relationship metadata to each Entity class.
    for parent, spec in schema_def.relationships.items():
        E = schema_def.E
        # Catch non-existence errors.
        parentClass = getattr(E, parent, None)
        if parentClass is None:
            # Check for dbname.Entity. If found,
            # create the related entity in the external
            # db using RelationProxy. Parent must
            # a string and a valid Entity. 
            if '.' in parent:
                dbname, clsname = parent.split('.')
                l = sync_lock.acquire()
                db = schevo.database.open(dbname)
                parentClass = db.extents[clsname]
                l.release()
            else:
        ...
        spec = []
        for other_extent_name, other_extent_field_set in list(other_map.items()):
    ...
    q = schema_def.q
    for name, func in schema_module.__dict__.items():
...
def _field_info_extract(module):
    ...
    f_set = schevo.namespace.SCHEMADEF.f._set
    for FieldClass in list(module.__dict__.values()):
