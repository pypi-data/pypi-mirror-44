...
class Param(Query):
    ...
    __slots__ = ['_on', '_field_map', '_label', '_f', '_h', '_s']
...
class Exact(Param):
    ...
    def __init__(self, extent, **kw):
        ...
        for name, FieldClass in extent.field_spec.items():
    ...
    def _criteria(self):
        criteria = odict()
        for name, field in self.s.field_map().items():
            if field.assigned:
    ...
    def __unicode__(self):
        ...
        if criteria:
            ...
            criteria = [
                ...
                (label(field_spec[name]), str(self.f[name]))
...
class Group(Query):
    ...
    def _results(self):
        ...
        for result in self.query():
            ...
            L.append(result)
        return results(values for values in groups.values())
...
class Min(Query):
    ...
    def __unicode__(self):
        field = self.FieldClass(self, self.field_name)
        return 'results that have the minimum %s in each (%s)' % (
...
class Max(Query):
    ...
    def __unicode__(self):
        field = self.FieldClass(self, self.field_name)
        return 'results that have the maximum %s in each (%s)' % (
            label(field), self.query
            )
