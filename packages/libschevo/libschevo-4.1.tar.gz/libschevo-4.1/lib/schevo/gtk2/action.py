    ...
    if namespace_id == 't' and method_name in DEFAULT_T_METHODS:
        ...
        if other_found:
            ...
            if isinstance(instance, Entity):
                ...
                action.label = '%s %s...' % (method_label, label(instance))
...
def get_view_actions(db, entity):
    ...
    if entity is not None:
        if (entity._hidden_views is not None
            ...
            return actions
        options = [False]
        for name, FieldClass in list(entity._field_spec.items()):
...
def get_view_action(db, entity, include_expensive):
    if include_expensive:
        text = 'View (including expensive fields)...'
    else:
        text = 'View...'
    action = Action()
    action.db = db
