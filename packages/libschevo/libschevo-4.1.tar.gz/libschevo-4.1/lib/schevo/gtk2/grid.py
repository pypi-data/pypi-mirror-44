...
from schevo.schema import UNASSIGNED
from schevo.error import EntityDoesNotExist

import gtk
from gtk import gdk

from schevo.gtk2.utils import gproperty, gsignal, type_register
...
class Column(object):
    ...
    def __init__(self, grid, attribute, title, data_type,
        ...
        if type == 'pixbuf':
            ...
            # Set fixed height for cells to avoid filling all the
            # available window space.
            self.cell.set_fixed_height_from_font(True)
            self.cell_prop = 'text'
        elif type == 'toggle':
    ...
    def cell_data(self, column, cell, model, row_iter):
        ...
        if self.call:
            data = data()
        prop = self.cell_prop
        if not isinstance(data, UNASSIGNED):
            if prop == 'text':
                try:
                    data = str(data)
                except (TypeError, EntityDoesNotExist):
                    #data = None
                    raise
            elif prop == 'pixbuf':
...
class Grid(gtk.VBox):
    ...
    def __init__(self, columns=[]):
        ...
        # ListStore is responsible for storing data in
        # list form.
...
##         return hash(instance)
    ...
    def set_rows(self, instances):
        ...
        textview = self._view
        textview.freeze_notify()
        textview.set_model(None)
        self.unselect_all()
        self.clear()
        gc.collect()
        model = self._model # ListStore 
        ...
        for instance in instances:
            ...
            n += 1
        if self._sorter is not None:
            textview.set_model(self._sorter)
        else:
            textview.set_model(model)
        textview.thaw_notify()
        self.set_cursor()
...
def model_sort(model, row_iter1, row_iter2, xxx_todo_changeme):
    (column, attr_name) = xxx_todo_changeme
    instance1 = model[row_iter1][OBJECT_COLUMN]
    instance2 = model[row_iter2][OBJECT_COLUMN]
