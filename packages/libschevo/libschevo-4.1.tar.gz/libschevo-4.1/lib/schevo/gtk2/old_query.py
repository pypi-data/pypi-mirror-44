...
class Query(gtk.VBox):
    ...
    def __init__(self, db, extent, method_name):
        ...
        text_label = gtk.Label('<b>%s</b>' % label(extent))
...
class Results(gtk.VBox):
    ...
    def rerun(self):
        ...
        for class_, results in list(class_results.items()):
            ...
            if count != 1:
                extent_label = gtk.Label(
                    '%i %s found:' % (count, plural(extent)))
            else:
                extent_label = gtk.Label(
                    '%i %s found:' % (count, label(extent)))
...
class ResultsListView(ObjectList):
    ...
    def __init__(self, ResultClass, results):
        columns = []
        sorted = True
        for field_name, FieldClass in list(ResultClass._field_spec.items()):
...
class IntersectionForm(gtk.Frame):
    ...
    def __init__(self, db, query):
        gtk.Frame.__init__(self, 'Intersection of')
        self._db = db
        self._query = query
    ...
    def update_query(self):
        for widget in list(self._query_widget.values()):
            widget.update_query()
