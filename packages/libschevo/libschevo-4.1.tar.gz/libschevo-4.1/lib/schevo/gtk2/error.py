...
from schevo.gtk2.constants import MONO_FONT
    ...
    try:
        ...
        if markup is None:
            if issubclass(exc_type, schevo.error.DatabaseFileLocked):
                markup = [
                    'The file or library you are trying to open is already\n'
                    'in use by another application.  Please close the file\n'
                    'in the other application to open it here.\n'
                    ]
            elif issubclass(exc_type, schevo.error.DeleteRestricted):
                markup = [
                    'The object was not deleted from the database.\n'
                    '\n'
                    'It is referred to by the following types of objects.\n'
                    'Delete them first before deleting this object.\n'
                    '\n'
                    ]
                ...
                markup = [
                    'The <b>%s</b> field is readonly and cannot be changed.'
                    % escape(label(exc_val.field))
                    ]
            elif issubclass(exc_type, schevo.error.FieldRequired):
                markup = [
                    'The <b>%s</b> field is required. Please provide a value.'
                    % escape(label(exc_val.field))
                    ]
                ...
                markup = [
                    ...
                    '\n'
                    'There is already an object of this type that has\n'
                    'the following values, which must be unique:\n'
                    '\n'
                    ]
                ...
                markup = [
                    ...
                    '\n'
                    'The most common reason for this is performing\n'
                    'another transaction that results in this object\n'
                    'being updated.\n'
                    '\n'
                    'Cancel this transaction, then re-open it, to\n'
                ...
                markup = [
                    'Your changes were not saved.\n'
                    '\n'
                    'The following transaction rule was violated when\n'
                    'attempting to save:\n'
                    '\n'
                    ]
                ...
                markup = [
                    'No fields changed.\n'
                    '\n'
                    'Change at least one field to update this object,\n'
                    'or click <b>Cancel</b> to leave it unchanged.'
                    ]
            else:
