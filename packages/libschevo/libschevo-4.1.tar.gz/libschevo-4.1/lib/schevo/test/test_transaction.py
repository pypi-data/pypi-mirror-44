...
class BaseTransaction(CreatesSchema):
    ...
    def test_create_simple(self):
        ...
        assert list(tx.s.field_map().keys()) == ['name', 'age']
    ...
    def test_create_copy(self):
        ...
        except error.DatabaseMismatch as e:
    ...
    def test_create_with_fget(self):
        ...
        assert list(tx.s.field_map().keys()) == ['code', 'name', 'count']
...
##         assert not result2.f.age.changed
    ...
    def test_update_cannot_skip_revisions(self):
        ...
        try:
            db.execute(tx2)
        except error.TransactionExpired as e:
    ...
    def test_delete_simple(self):
        ...
        try:
            name = result1.name
        except error.EntityDoesNotExist as e:
            assert e.extent_name == 'User'
    ...
    def test_delete_restrict(self):
        ...
        except error.DeleteRestricted as e:
    ...
    def test_clone_simple(self):
        ...
        try:
            db.execute(tx)
        except error.KeyCollision as e:
            assert e.extent_name == 'User'
            assert e.key_spec == ('name',)
            assert e.field_values == ('User 1',)
    ...
    def test_execute_only_once(self):
        ...
        try:
            db.execute(tx)
        except error.TransactionAlreadyExecuted as e:
    ...
    def test_undo(self):
        ...
        try:
            tx._undo()
        except error.TransactionNotExecuted as e:
            assert e.transaction == tx
    ...
    def test_f_delattr(self):
        ...
        assert list(tx.s.field_map().keys()) == ['name']
        tx.name = 'foo'
        ...
        tx = db.User.t.create()
        del tx.f.name
        assert list(tx.s.field_map().keys()) == ['age']
        tx.age = 5
...
##         assert label(tx.f.extra) == 'Extar'
    ...
    def test_relaxed_index(self):
        ...
        tx.count = 3
        first = db.execute(tx)
        assert first.next.next.__next__ == first
