...
class BaseEvolveIntraVersion(CreatesDatabase):
    ...
    def test_add_entity_field(self):
        ...
        """)
        self.sync(schema1)
        foo = db.execute(db.Foo.t.create(name='baz'))
        foo_oid = foo.s.oid
        self.sync(schema2)
        foo = db.Foo[foo_oid]
        assert foo.name == 'baz'
        assert foo.bar is UNASSIGNED
        bar = db.execute(db.Bar.t.create(name='bof'))
...
class BaseEvolveInterVersion(CreatesDatabase):
    ...
    def test_wrong_version(self):
        ...
        except error.DatabaseVersionMismatch as e:
    ...
    def test_remove_extent_restricted(self):
        ...
        try:
            self.evolve(schema2, version=2)
        except error.ExtentDoesNotExist as e:
...
class BaseEvolvesSchemataNoSkip(EvolvesSchemata):
    ...
    def test(self):
        ...
        expected = set(['one', 'two', 'three', 'five', 'fob'])
...
class BaseEvolvesSchemataSkip(EvolvesSchemata):
    ...
    def test(self):
        assert db.version == 2
        names = set(foo.name for foo in db.Foo)
        expected = set(['one', 'two', 'three', 'four', 'fob'])
        assert names == expected
