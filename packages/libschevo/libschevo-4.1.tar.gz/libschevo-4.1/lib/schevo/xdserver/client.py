...
from schevo.xdserver.server import DEFAULT_HOST, DEFAULT_PORT, PROTOCOL
...
class Client(object):
    ...
    def __init__(self, host=DEFAULT_HOST, port=DEFAULT_PORT):
        ...
        assert self.socket is not None, 'Could not connect to %s' % (self.address)
...
class ClientStorage(Storage):
    ...
    def __init__(self, client, db_name):
        ...
        assert self.socket is not None, 'Invalid client socket: %s' % self.socket
        self.oid_pool = []
