...
class EntityFieldIdsRepair(object):
    ...
    def perform(self):
        ...
        try:
            try:
                items = list(self._extents_extraneous.items())
                for (extent_name, extraneous_field_ids) in items:
                    ...
                    if db.format == 2:
                        for entity_map in extent_map['entities'].values():
    ...
    def perform(self):
        ...
        try:
            try:
                ...
                for extent_id, extent_map in extent_maps_by_id.items():
                    ...
                    for oid, entity_map in extent_map['entities'].items():
                        ...
                        len_links = sum(
                            len(v) for v in entity.s.links().values())
