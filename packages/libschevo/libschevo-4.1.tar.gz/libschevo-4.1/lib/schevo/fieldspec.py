...
def field_spec_from_class(cls, class_dict, slots=False):
    ...
    if cls._field_spec:
        ...
        for name, BaseFieldClass in cls._field_spec.items():
            field_spec[name] = new_field_class(BaseFieldClass, slots)
    specs = []
    for name, field_def in list(class_dict.items()):
