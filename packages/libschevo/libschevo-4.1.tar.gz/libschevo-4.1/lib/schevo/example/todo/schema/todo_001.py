...
class Priority(E.Entity):
    ...
    @f.integer(label='# Open Items')
    def open(self):
        return len([item for item in list(self.m.items()) if not item.done])

    @f.integer(label='# Done Items')
    def done(self):
        return len([item for item in list(self.m.items()) if item.done])
