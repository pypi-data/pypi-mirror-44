...
def convert_format(url, backend_args={}, format=None):
    ...
    try:
        for new_format in range(original_format + 1, format + 1):
...
def copy(src_url, dest_url, src_backend_args={}, dest_backend_args={}):
    ...
    dest_SCHEVO['extent_name_id'] = d_pdict(
        iter(src_SCHEVO['extent_name_id'].items()))
    ...
    def copy_btree(src):
        ...
        for key, value in src.items():
            ...
            copy[key] = value
        return copy
    for extent_id, src_extent in src_SCHEVO['extents'].items():
        ...
        dest_extent['field_id_name'] = d_pdict(
            iter(src_extent['field_id_name'].items()))
        dest_extent['field_name_id'] = d_pdict(
            iter(src_extent['field_name_id'].items()))
        ...
        for entity_oid, src_entity in src_extent['entities'].items():
            ...
            for key, value in src_links.items():
                ...
                for k, v in src_links[key].items():
                    links[k] = v
            dest_entity['related_entities'] = d_pdict(
                iter(src_entity['related_entities'].items()))
        assert log(2, 'Copying indices for', extent_name)
        dest_extent['index_map'] = d_pdict(
            (k, d_plist(v))
            for k, v
            in src_extent['index_map'].items()
            )
        dest_extent['normalized_index_map'] = d_pdict(
            (k, d_plist(v))
            for k, v
            in src_extent['normalized_index_map'].items()
            )
        dest_indices = dest_extent['indices'] = d_btree()
        for index_spec, src_index_data in src_extent['indices'].items():
...
def create(url, backend_args={},
           ...
           format=None, label='Schevo Database'):
...
def new_backend(url, backend_args=None):
    ...
    if url.backend_name == 'zodb':
        db = url.backend_class()(url.database)
        return db
    else:
...
def open(url, backend_args=None, dbclass=2):
    ...
    Database = format_dbclass[dbclass]
