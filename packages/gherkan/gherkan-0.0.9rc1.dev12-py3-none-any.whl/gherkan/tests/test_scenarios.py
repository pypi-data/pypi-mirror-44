...
class TestNLScenarios(unittest.TestCase):
    ...
    def get_files(self, codename, level, lang):
        nl_filename = f"{codename}_NL_{level}_{lang}.json"
        signal_filename = f"{codename}_signals_{level}_{lang}.txt"
    ...
    # =============== TESTS ===============
    def test_montrac_basic_en(self):
        nl_filename, signal_filename = self.get_files("Montrac", "basic", "en")
    ...
    def test_bgscenarioDif_easy_en(self):
        #TODO connected issue Robot actions not detected from background plus wrong detection
