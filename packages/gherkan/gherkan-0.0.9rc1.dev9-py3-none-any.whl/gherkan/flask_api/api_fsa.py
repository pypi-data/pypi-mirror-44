...
class API_FSA():
    ...
    # =====>>> API initialization <<<===== #
    @classmethod
    def init_fsa(cls, address, port, debugMode=False):
        global DEBUG_MODE
        ...
        class ConsoleFormater(logging.Formatter):
            ...
            def format(self, record):
                ...
                string = template.format(severity=record.levelname, time=self.formatTime(record), message=record.message)
                string += ""
                return string
        ...
        queueHandler.setFormatter(ConsoleFormater())
        ...
        logging.info("Variable checking done, checking state from previous run...")
    ...
    def requestNLText(cls):
        ...
        logging.debug("SignalToNL: ", signalFilePath)
    ...
    def receiveNLText(cls, data):
        ...
        logging.debug("nlToSignal: ", basepath, data)
    ...
    def receiveAudio(cls, audioPath, language):
        ...
        logging.debug("speechToText: ", audioPath, lang)
