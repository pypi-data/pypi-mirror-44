...
class Finetuner():
    ...
    def get_txt_info(self, text):
        text_info = self.morpho.get_pos_info(text)
        return text_info
    ...
    def find_verb(self, lang, sentence, head_only=True):
            ...
            for word in word_list:
                ...
                if analysis == []:
                    ...
                    try:
                        ...
                        pass
            if not verb:
                sent_lst = self.morpho.get_pos_info(sentence)
                for x in sent_lst:
                    if x["pos"] == "V":
                        verb.append(x["word"])

        elif lang == "en":
            t = parsetree(self.lemmatize_sentence(
                lang, sentence), tokenize=True,  tags=True)
        ...
        with pkg_resources.resource_stream("gherkan", "utils/verbs.yaml") as stream:
            verb_dic = yaml.load(stream)
        if verb:
            verb_copy = verb
            for v in verb_copy:
              if v in verb_dic[lang]["not verbs"]:
                verb.remove(v)
              if not verb:
        ...
        if not verb:
            for word in sentence_l.split():
                if word in verb_dic[lang]["verbs"]:
                    verb = [word.replace("-", "ne")]
        if not verb:
            logging.debug("Warning: did not detect any verb in sentence: {}".format(sentence))
        return verb
