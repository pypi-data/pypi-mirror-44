...
class Parser:
    ...
    def parseLanguage(self, line: str):
        ...
        if not directive:
            logging.warning("Missing language in the input file, using 'en'",
                            extra={"type": logging_types.W_SHIT_HAPPENED, "phrase" : line })
            return "en"
        ...
        if scenarioName:
            ...
            logging.warning(
                "Missing name for a scenario on line {}, using random name".format(
                    lineNumber), extra={"type": logging_types.W_SHIT_HAPPENED, "phrase" : line})

    def parseAnd(self, nlBatch, lineNumber, textlines):
        # TODO implement
        logging.warning(
            "Skipping And section, on line {} not implemented yet.".format(
                lineNumber), extra={"type": logging_types.W_SHIT_HAPPENED, "phrase": textlines[lineNumber]})
        ...
        if section == g.GIVEN:
            ...
            logging.warning(
                "Unknown section '{}' encountered on line {}".format(section, lineNumber),
                    extra={"type" : logging_types.W_SHIT_HAPPENED, "phrase" : textlines[lineNumber]})
    ...
    def parse(self, textlines: list):
        ...
        # TODO exceptions
        # TODO just use the official parser...
