...
CZ_PROGRAM_DICT_DIR = os.path.join(c.PROJECT_ROOT_DIR, "gherkan/utils", "RobotPrograms_cs.json")
...
class FillActions:
    def __init__(self):
        self.finetune = Finetuner(lang="cs")
        self.lang = None
    ...
    def ParseMainAction(self, data):
        ...
        for key, value in varz.items():  # match varz with template
            ...
            if self.data["inclinations"][idx] != 0:
                value = self.finetune.conjugate(
                    self.lang, value, self.data["inclinations"][idx])
            to_replace = re.compile(r"\(\?P\<{}\>[^\)]+\)".format(key))
    ...
    def ParseBool(self, phrase, value):
        ...
        if value:
            ...
            if verb:
                if self.lang == "cs":
                    phrase_bool = re.sub(
                        verb[0], "".join(["ne", verb[0]]), phrase)
                elif self.lang == "en":
                    verb_lemma = self.finetune.lemmatize(self.lang, verb[0])
                    if verb_lemma in exceptions:
                        phrase_bool = re.sub(r'\b{}\b'.format(
                            verb[0]),  " ".join([verb_lemma, "not"]), phrase)
                    else:
                        phrase_bool = re.sub(r'\b{}\b'.format(
                            verb[0]), " ".join(["does not", verb_lemma]), phrase)
            else:
                phrase_bool = phrase
        return phrase_bool
